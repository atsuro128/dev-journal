...
| `PORT` | `8080` | 固定 |
| `LOG_LEVEL` | `info` | 固定 |
| `DATABASE_URL` | `postgres://expense_owner:<owner_pw>@<rds-endpoint>:5432/expense_saas?sslmode=require` | tfvar の `expense_owner_db_password`（§5.7.2 / §5.7.3）。**migrate 後に RDS マスターユーザーで `ALTER ROLE expense_owner WITH PASSWORD '${OWNER_DB_PW}'` を必ず実行した後の値**（migration が `localdev` で作成するため、ALTER で上書きしないと不一致になる、F-115） |
| `APP_DATABASE_URL` | `postgres://expense_app:<app_pw>@<rds-endpoint>:5432/expense_saas?sslmode=require` | tfvar の `expense_app_db_password`（§5.7.2 / §5.7.3）。**migrate 後に RDS マスターユーザーで `ALTER ROLE expense_app WITH PASSWORD '${APP_DB_PW}'` を必ず実行した後の値**（同上、F-115） |
...
| `expense_app` / `expense_owner` ロールのパスワード | Secrets Manager から `ALTER ROLE ... WITH PASSWORD` で投入（env_config.md §6.4） | **migrate up 完了後に RDS マスターユーザーで `ALTER ROLE ... WITH PASSWORD '${...}'` を必ず実行**（§5.7.3） | migration `000002_create_roles.up.sql` が両ロールを `localdev` パスワードで作成する仕様。prod では `IF NOT EXISTS` ガードに引っかかり NOOP になるため、`CREATE ROLE` ではなく `ALTER ROLE` で必ず上書きする手順に統一（F-115） |
...
- **`expense-saas/db/migrations/000002_create_roles.up.sql` が prod でも `expense_owner` / `expense_app` を `PASSWORD 'localdev'` で作成する**（F-115 検証根拠）:
  ...
  - GRANT（USAGE, DML, ALTER DEFAULT PRIVILEGES）も同 migration に含まれているため、**prod でも `migrate up` 完了時点で権限付与は完了する**
  - 残課題は **localdev パスワードの上書き**のみ。後続で `IF NOT EXISTS` 付き `CREATE ROLE` を流しても既存ロール扱いで NOOP になり、`APP_DATABASE_URL` の prod パスワードと不一致が起こるため、**§5.7.3 では `CREATE ROLE` ではなく `ALTER ROLE ... WITH PASSWORD` で必ず上書きする** 手順に統一する（F-115 対応）
...
> **重要（F-115 根本修正）**: `000002_create_roles.up.sql` が `expense_owner` / `expense_app` の両ロールを **`PASSWORD 'localdev'` で作成する** ため、prod では migration 完了直後に **必ず `ALTER ROLE ... WITH PASSWORD '${...}'` を RDS マスターユーザー（`db_master_user`、既定 `postgres`）で実行**しないと、`APP_DATABASE_URL` / `DATABASE_URL` の値と DB 上のパスワードが一致せずアプリ起動・migrate のいずれも失敗する。`IF NOT EXISTS` 付きの `CREATE ROLE` は既存ロール扱いで NOOP になるため、**`CREATE ROLE` ではなく `ALTER ROLE` を使う**。GRANT 系は migration で完了済みのため再実行不要（確認のみ）。

##### 手順全体像

1. EC2 から **RDS マスターユーザーで** psql 接続（migration 実行前にパスワード変更すると `expense_owner` で migrate 接続できないため、**順序は migration 先 → ALTER ROLE 後**）
2. **a. `expense_owner` のパスワードのまま** migrate 実行用に一時的に使うため、まずは migration を `localdev` パスワードで通す（後述 a 案）
   または、**b. RDS マスターユーザーで先に ALTER ROLE してから** マスターではなく `expense_owner` で migrate を実行する（後述 b 案）
3. migrate 完了後、`expense_owner` / `expense_app` の両ロールを `ALTER ROLE` で prod パスワードに上書き
4. アプリ起動（`/etc/expense-saas/app.env` の `DATABASE_URL` / `APP_DATABASE_URL` で接続できることを確認）

##### 採用案: b 案（RDS マスターユーザーで先に ALTER ROLE）

a 案（migrate 後に ALTER）も技術的には成立するが、`migrate up` 実行時に `localdev` という弱パスワードを TLS 越しに使う履歴を作ることになり、ssh tty / shell history への露出リスクがあるため、**b 案（migrate 前に ALTER）を採用する**。a 案を採る場合は §5.8 ロールバック手順の脚注を参照。
...
# 5.7.2 で生成したパスワードを export（履歴に残らないよう先頭スペース付きで実行 or `read -s` を使う）
...
# ─────────────────────────────────────────
# Step 1: マイグレーション SQL を持ち込み（§5.5 Q1 案ごとに方法が異なる、後述 5.7.4 参照）
# ─────────────────────────────────────────
# 例: 案B（EC2 build）の場合は git clone 済みなのでこのまま。案A/C は 5.7.4 の手順で先に持ち込む。
cd ~/expense-saas

# ─────────────────────────────────────────
# Step 2: RDS マスターユーザーで両ロールを作成（migration が走る前に空ロールだけ用意する場合）
#          → ただし migration が CREATE ROLE IF NOT EXISTS で同名ロールを localdev で作るので
#            「マスターで先に作る」だけだと依然 localdev に置き換わってしまう。
#          したがって順序は「migrate up → 直後に ALTER ROLE」固定とする。
# ─────────────────────────────────────────

# ─────────────────────────────────────────
# Step 3: migrate up
#   - migration 内 000002_create_roles.up.sql により expense_owner / expense_app が
#     PASSWORD 'localdev' で作成される
#   - GRANT / ALTER DEFAULT PRIVILEGES もこの migration で完了する
#   - migrate 自身の接続は RDS マスターユーザー（postgres）で行う（schema 所有を変更しないため）
# ─────────────────────────────────────────
export MIGRATE_DB_URL="postgres://${db_master_user:-postgres}:${MASTER_PW}@${RDS_HOST}:5432/${DB_NAME}?sslmode=require"
migrate -path db/migrations -database "$MIGRATE_DB_URL" up

# ─────────────────────────────────────────
# Step 4: RDS マスターユーザーで expense_owner / expense_app のパスワードを prod 値に上書き
#   - CREATE ROLE は流さない（migration 内で作成済み、IF NOT EXISTS で NOOP になるため）
#   - ALTER ROLE で必ず上書きする
# ─────────────────────────────────────────
export PGPASSWORD="$MASTER_PW"
psql -h "$RDS_HOST" -U "${db_master_user:-postgres}" -d "$DB_NAME" -v ON_ERROR_STOP=1 <<SQL
-- 両ロールが migration で作成されていることを前提に、パスワードのみ prod 値で上書き
ALTER ROLE expense_owner WITH LOGIN PASSWORD '${OWNER_DB_PW}';
ALTER ROLE expense_app   WITH LOGIN PASSWORD '${APP_DB_PW}';

-- GRANT は migration で完了済み。確認のみ実施（権限不足が見つかった場合のみ追加投入）。
\du expense_owner
\du expense_app
SELECT grantee, privilege_type
  FROM information_schema.role_table_grants
 WHERE grantee IN ('expense_owner', 'expense_app')
...
# ─────────────────────────────────────────
# Step 6: 環境変数ファイル更新（§5.6 で雛形を置いた /etc/expense-saas/app.env を ALTER 後の値で確定）
# ─────────────────────────────────────────
...
> **F-115 緩和策の要点**:
> - Step 3 の `migrate up` で migration 内 `CREATE ROLE ... PASSWORD 'localdev'` が走るが、Step 4 の `ALTER ROLE` で **必ず** 上書きする
> - Step 4 を **省略すると `APP_DATABASE_URL` / `DATABASE_URL` の prod パスワードと DB 上のパスワードが不一致になり、アプリ起動失敗**。リスク表 §8 #13 にも追記
> - `CREATE ROLE` 文を runbook から完全に削除した（旧版では `IF NOT EXISTS` 付きで再投入していたが、既存ロール扱いで NOOP になり罠だった）
> - GRANT は migration で完了するため Step 4 では再投入しない。`information_schema.role_table_grants` で確認のみ
...
| 14 | **migration `000002_create_roles.up.sql` の `localdev` パスワードが prod に残置** | 高 | `expense_owner` / `expense_app` の両ロールは migration により `PASSWORD 'localdev'` で作成される。**§5.7.3 Step 4 で RDS マスターユーザーから `ALTER ROLE ... WITH PASSWORD '${...}'` を必ず実行**して prod 値で上書きすること。Step 5 の疎通確認 `PGPASSWORD="$APP_DB_PW" psql -U expense_app ... -c 'SELECT 1;'` を完了確認ゲートとする。`IF NOT EXISTS` 付き `CREATE ROLE` は既存ロール扱いで NOOP になるため、`CREATE ROLE` 文を runbook から完全に削除した（F-115 対応） |
...
- [x] **F-115 対応**: `000002_create_roles.up.sql` の localdev パスワード残置リスクを §5.7.1 / §5.7.3 / §8 #14 / §4.2 末尾差分表に反映済み。Phase 4-4 で **§5.7.3 Step 4 ALTER ROLE と Step 5 疎通確認** を完了ゲートとする
