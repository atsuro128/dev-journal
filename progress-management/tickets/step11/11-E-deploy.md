...
| インフラ | AWS リソース構築（Phase 2-4） | 完了（2026-05-18〜20） | state バケット + 26 リソース apply 完了、DB bootstrap 全 Step 1-7 PASS、app.env 確定値書き込み済み |
| インフラ | AWS リソース更新（SSM + CloudWatch + CloudFront） | 完了（2026-05-26） | 1 段階目: 5 add (CloudFront / Log Group / IAM 3) / 3 change / 1 destroy。create_before_destroy 対応で SG replace 順序問題解消。2 段階目: CORS_ALLOWED_ORIGINS を CloudFront ドメインへ、restrict_alb_to_cloudfront=true で SG プレフィックスリスト限定 |
| 設定 | 環境変数・シークレット | 完了（2026-05-26、SSM 移行） | SSM Parameter Store 4 件 SecureString 投入: `/expense-saas/portfolio/{database/url,database/app_url,jwt/private_key,jwt/public_key}`。EC2 user_data が起動時 fetch して `/etc/expense-saas/app.env` (0640 root:appgroup) + `/etc/expense-saas/keys/{private,public}.pem` (0600) に配置 |
| デプロイ | Docker build / S3 持ち込み / systemd 起動 | 完了（2026-05-26） | ホストで `docker build -t expense-saas:portfolio` (85.7MB → save+gzip 24.6MB) → S3 経由で EC2 持ち込み (`s3://...-receipts-.../_temp/...`) → `docker load` → `systemctl enable --now expense-saas.service` → 8080 listen + DB 接続成功 |
| ヘルスチェック | API（CloudFront 経由 / ALB 直接 / SPA） | 完了（2026-05-26） | CloudFront `/health` 200 (`{"status":"ok","checks":{"database":"ok"}}`)、ALB 直接 = タイムアウト（SG が CloudFront プレフィックスリスト限定）、HSTS/X-Frame/X-Content-Type 全設定確認 |
| スモーク | 申請 → 承認 → 支払完了 | 完了（2026-05-26） | seed 投入後、test-member submit → test-approver approve → test-accounting pay → status=paid 確認（全 CloudFront 経由）。詳細は §実施ログ - Phase 5-7 実 apply セッション を参照 |
| 時刻ドリフト smoke | 11-D CONDITIONAL #3 | 完了（2026-05-26） | Host/Container 完全一致、chronyc System time 25 マイクロ秒（AWS Time Sync Service 169.254.169.123、Stratum 4）。JWT leeway 60s で完全カバー |
...
### 実施ログ - Phase 5-7 実 apply セッション（2026-05-26）

issue #187/#188/#185 T4/T6 + 11-D CONDITIONAL #3 を実 AWS 環境で全消化し、Step 11-E を完了状態に到達させた。

#### apply セッションの全工程

| Phase | 内容 | 所要 | 結果 |
|-------|------|------|------|
| 5-1 | SSM Parameter Store 4 件 SecureString 投入 | <1 分 | PASS（database/url, database/app_url, jwt/private_key, jwt/public_key）|
| 5-2 | terraform.tfvars 更新（SSH 変数削除 / trusted_proxy_count=2 / restrict_alb_to_cloudfront=false）+ `TF_VAR_cloudfront_origin_verify_secret` 環境変数化 | <1 分 | PASS |
| 5-3 | 1 段階目 apply (CloudFront / CloudWatch Log Group / SSM IAM / 新 EC2) | 約 8 分 | PASS（2 ラウンド: 初回 SG DependencyViolation で FAIL → create_before_destroy 修正 → 5 add / 3 change / 1 destroy 完了）|
| 5-4 | EC2 上で S3 経由 image 持ち込み + systemctl start | <1 分 | PASS（24.6MB → docker load → 58.2MB → systemd active）|
| 6 | ヘルスチェック（CloudFront /health 200・ALB 直接 403 = B-1-b 1 層目） | <1 分 | PASS |
| 7-A | 受け入れ基準 #187 6 件 | <2 分 | 全 PASS（SSM start-session 接続 / SSH 22 不在 / app.env 配置 / CloudTrail SendCommand ログ / userData 平文不在 / アプリ /health 疎通）|
| 7-B | 受け入れ基準 #188 2 件 | <1 分 | 全 PASS（新 LogGroup `/expense-saas/portfolio/api` stream `i-051ca0c9129854b10/api` 転送 / 旧 `/ecs/expense-saas/*` 不在 = 放置 OK）|
| 7-C | #185 T6 一部（HSTS / X-Frame / CORS / レート制限） | <1 分 | 全 PASS |
| 7-D | 2 段階目 apply（CORS_ALLOWED_ORIGINS → CloudFront ドメイン / restrict_alb_to_cloudfront=true）| 約 3 分 | PASS（2 add / 1 change / 2 destroy、SG description 残り日本語 fix 必要） |
| 7-E | EC2 上で S3 経由 image 再持ち込み + systemctl start（2 段階目 EC2 再作成） | <1 分 | PASS |
| 7-F | #185 T6 残り（CloudFront 経由 CORS / B-1-b 2 層目 = ALB 直接タイムアウト） | <1 分 | 全 PASS |
| 7-G | seed 投入 + 申請→承認→支払 golden path | <2 分 | PASS（test-member submit → test-approver approve → test-accounting pay → status=paid）|
| 7-H | 時刻ドリフト smoke（11-D CONDITIONAL #3） | <1 分 | PASS（host/container 一致、chronyc 25μs ズレ、AWS Time Sync Service 同期）|

#### 緊急 fix

apply blocker 解消のため `expense-saas/infra/terraform/security_groups.tf` を master 直接編集（5/18 rds.tf 緊急 fix と同パターン）:
1. **B-01**: `ingress.description` の L46/L58 日本語 → ASCII 英語（AWS SG validation regex 違反、PR #151 漏れ）
2. **B-02**: `aws_security_group.alb` に `lifecycle { create_before_destroy = true }` + `name` → `name_prefix` 切替（SG replace 順序問題）

緊急 fix は後追いで PR #154 を起票（reviewer + codex レビュー実施）+ 副次 issue #189 起票（再発防止 lint 検討は post-MVP）。
...
| 新 EC2 Public IP | `18.181.175.57` → `13.230.121.95`（2 段階目 apply で再変更） |
| ALB DNS | `expense-saas-portfolio-alb-290554356.ap-northeast-1.elb.amazonaws.com`（CloudFront プレフィックスリスト限定で外部アクセス不可）|
| 新 LogGroup | `/expense-saas/portfolio/api` |
| SSM Parameter Store パス | `/expense-saas/portfolio/{database,jwt}/*` |

#### Phase 5-7 判定: **PASS**

Step 11-E 完了条件全達成:
- ✓ デプロイ済み環境で第三者がアクセスできる状態 (`https://djhmwtrr79jdq.cloudfront.net/`)
- ✓ ヘルスチェックエンドポイントが応答する (`/health` 200)
- ✓ 主要フロー1本（申請 → 承認 → 支払完了）がデプロイ済み環境で通る (golden path 完走)
...
### 公開 URL
- **`https://djhmwtrr79jdq.cloudfront.net/`** （CloudFront 経由 HTTPS、UAT 担当はこの URL でアクセス）
- ALB DNS `expense-saas-portfolio-alb-290554356.ap-northeast-1.elb.amazonaws.com` への直アクセスは B-1-b 2 層目（SG 限定）でタイムアウト = アクセス不可

### UAT 用アカウント
seed 投入済み（パスワード共通: `TestPass1!`）
...
| Member（空状態） | `test-member-empty@example.com` |

### デプロイ日時・コミット
- 2026-05-26: 実 apply 完了（CloudFront / SSM / awslogs / CORS / B-1-b 2 層目）
- expense-saas master HEAD: `420f0ea` + PR #154 マージ後の HEAD（PR #154 は本セッション中に作成、レビュー中）
- インフラ: terraform/ + AWS state 同期済み

### 既知の非ブロッカー
- ADR-0007 で受容した逸脱:
  - B-3: CloudFront〜ALB 間 HTTP（CloudFront origin-facing でセグメント保護）
  - B-5: viewer 側 TLS 最小バージョン TLSv1 固定（CloudFront デフォルト証明書の制約）
- Secrets Manager 不採用（SSM Parameter Store SecureString で代替、無料枠維持）
- 緊急 fix で起票した issue #189（SG description ASCII 化、再発防止 lint 検討は post-MVP）

### 11-F UAT 着手前の前提条件
- ✅ **issue #190 (CloudFront default_root_object regression) 解消済み**: PR #155 マージ + apply 完了で `https://djhmwtrr79jdq.cloudfront.net/` 200 OK。

### UAT で重点確認すべき箇所
- CloudFront 経由の HTTPS 配信・HSTS・CORS が正しく機能すること（本セッション T6 検証で疎通確認済み）
- 申請→承認→支払の主要フロー以外のサブフロー（却下・差戻し・添付ダウンロード等）
- 各ロールの認可制御（test-member は他人のレポート閲覧不可、test-accounting は承認操作不可 等）
- レスポンシブ表示（スマホ幅・タブレット幅）

### 復旧 runbook（EC2 が再作成された場合）
1. SSM 経由で aws s3 cp `s3://expense-saas-portfolio-receipts-d8ed055a/_temp/expense-saas-portfolio.tar.gz` /tmp/
2. `sudo docker load -i /tmp/expense-saas-portfolio.tar.gz`
3. `sudo systemctl enable --now expense-saas.service`
4. `/health` 200 を確認
