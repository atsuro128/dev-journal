...
| Q2 | TLS / カスタムドメイン（Q7 を統合） | 後述 3 案 | 採用面接の評価者等に見せないなら案1。見せるなら案2 | **案 1: HTTP のみ、ALB DNS**（UAT は社内デモ限定。ADR-0004 prod TLS との乖離を 11-F 引き継ぎに明記） |
...
#### Q2 案 1 採用に伴う §4.2 / §3.2 / §7.2 / リスク表の整合

- §4.2 `CORS_ALLOWED_ORIGINS=http://<alb-dns>` を採用（変更なし）
- §3.2 alb.tf は `aws_lb_listener` 80 のみ（443 / ACM は不要）
- §7.2 月額にドメイン費・ホストゾーン費を加算しない
- リスク表 #11（HTTP 運用受容）はそのまま採用、UAT は社内デモ限定で運用

##### Q2 詳細: TLS / カスタムドメイン

> ※2026-05-20 issue #185 / ADR-0007 で C 案（CloudFront 経由 HTTPS 化）に方針変更。以降の TLS 構成は ADR-0007（`dev-journal/deliverables/docs/30_arch/adr/0007-cloudfront-https.md`）を正とする。本セクションの「案1（HTTP のみ）採用」記述は当時の判断履歴として残す。

`security.md` §11 / `env_config.md` §3.1 prod が prod の TLS（ACM 終端）と CORS（`https://...` 固定）を定める。本計画はこれと一定の差分を許容する代わりに、選定理由と差分を 11-F 引き継ぎに記録する。
