...
上流・下流すべてのファイルで `file_id` → `attachment_id` に統一。

| ファイル | 修正内容 |
|----------|----------|
| `.claude/rules/security-policy.md` §3 | S3 キー `file_id` → `attachment_id` |
| `requirements.md` ATT-014 | S3 パス `file_id` → `attachment_id` |
| `04_business-rules.md` ATT-014 | 同上 |
| `security.md` | 上流修正済みのため「読み替え」注記・差分テーブル行・品質チェック項目を削除 |
| `files.md` | 上流修正済みのため「用語統一（issue 036）」セクション・差分テーブル行を削除 |
| `04_business-rules.md` ATT-014 | 文言「ファイル名に〜」→「S3パスに〜」に修正（codex 指摘） |
| `implementation-guide.md` | 旧パス形式 `tenants/{tenant_id}/users/{user_id}/receipts/{file_id}` → `{tenant_id}/{report_id}/{attachment_id}` に修正（codex 指摘） |
