...
| 184 | SPA fallback ハンドラが HEAD リクエストで 405 を返す（GET のみ登録） | 2026-05-20 | 起票のみ（post-MVP。Phase 6 検証中に curl -I で検出、ブラウザは GET のため SPA 表示に実害なし） |
| 185 | ALB が HTTP 平文で HTTPS 未対応（security.md §11 と乖離） | 2026-05-20 | 起票のみ（post-MVP。Phase 6 で検出、認証情報平文 + HSTS 矛盾。11-E §2.1/§3.5 のポートフォリオ簡略化判断との整合は対応方針決定時に判断） |
