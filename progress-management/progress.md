...
| 147 | per_page UI セレクタ + ページネーションフッター（再々オープン: SMK-081 で見栄え崩れ発覚 → AppPaginationFooter / PageSizeSelector の見た目を MUI 標準に寄せる A2 案。border-top + minHeight + 件数表示追加など共通コンポーネント側の調整） | 2026-04-25（初回）/ 2026-04-27（再オープン）/ 2026-04-27（再々オープン） | 未着手（SMK-081 ブロッカー） |
