...
| ID | タイトル | 状態 |
|----|---------|------|
| 118 | ItemForm のリアルタイムバリデーションが動作しない（useForm の mode 未指定） | 作業中 |
| 119 | ReportForm 日付フィールドで onBlur バリデーションが発火しない（Controller が field.onBlur を渡していない） | PR レビュー中 |
| 120 | 日付フィールドを空にすると Zod デフォルトエラー「Invalid input: expected string, received null」が露出 | PR レビュー中 |
| 121 | 422 レスポンスに details 配列が欠落（OpenAPI 契約違反） | 工程 A 完了・工程 B 未着手 |
