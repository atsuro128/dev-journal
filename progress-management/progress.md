...
| ID | タイトル | 状態 |
|----|---------|------|
| 108 | フォーム編集中の操作整合性（並行操作 + ItemForm 破棄確認） | 実装完了、PR #68 draft（Backend テスト + マージ待ち） |
| 114 | 添付の永続化タイミング仕様明示（即時保存方式の明確化） | 実装完了、PR #68 draft（Backend テスト + マージ待ち） |
| 115 | 新規明細での添付方法（ローカル保持 + 保存時順次アップロード方式） | 実装完了、PR #68 draft（Backend テスト + マージ待ち） |
...
| 119 | ReportForm 日付フィールドで onBlur バリデーションが発火しない（Controller が field.onBlur を渡していない） | 起票のみ |
| 120 | 日付フィールドを空にすると Zod デフォルトエラー「Invalid input: expected string, received null」が露出 | 起票のみ |
| 121 | 422 レスポンスに details 配列が欠落（OpenAPI 契約違反） | 起票のみ |
| 123 | smoke_check.md SMK-026 期待文言が state-management.md と不整合（docs 監査） | 起票のみ |
| 124 | API クライアントで SERVER_ERROR_MESSAGES マッピングが未適用（共通層の実装不備） | 起票のみ |
| 125 | smoke_check.md SMK-028 期待文言が state-management.md に未定義（docs 不整合） | 起票のみ |
| 126 | レポート作成・編集画面で成功トーストが表示されない（実装漏れ） | 起票のみ |
