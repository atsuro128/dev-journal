...
| 186 | 設計書のコンピュート層表記が ECS Fargate のまま（実装は EC2 t3.micro 単一） | 2026-05-20 | 対応中（2026-05-24: スコープを案D「EC2 を正本、Fargate は ADR-0004 だけに残す」に拡大、影響度を 中 に格上げ。architect 実装計画 v2 reviewer PASS、Phase 0 ユーザー判断 7 項目（UD-1〜7）+ SSM 別 issue 起票タイミング 確定。連動して issue #187 起票。次は Phase 1（小規模ファイル群シリアル）+ Phase 2-3（monitoring/env_config/runbook/release 並列）+ Phase 4（diagrams 構成図差し替え）） |
| 187 | terraform/ec2.tf: SSM Session Manager + SSM Parameter Store 対応（issue #186 設計確定に追従） | 2026-05-24 | 起票のみ（issue #186 UD-1=A + UD-5=B 確定に伴う派生。`expense-saas/infra/terraform/` の SSH 鍵認証→SSM 移行 + Terraform variable シークレット注入→SSM Parameter Store 移行。Step 11-E 実デプロイ時に同期実施想定） |
