...
| 11-D | 横断レビュー | reviewer (codex) | 11-B, 11-C | 作業中（2026-05-07: チケットレビュー PASS with NOTE → codex 横断レビュー実行中） | `tickets/step11/11-D-cross-review.md` |
...
| 173 | JWT 検証にクロックスキュー許容（leeway）を追加（CRS-066 真因 = Docker クロックドリフト） | 2026-05-07 | 再 open（11-D 横断レビューで PR #141 の対応漏れ判明: middleware 経路の `internal/pkg/jwt.Verifier` には leeway 未適用） |
