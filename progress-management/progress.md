...
| 109 | seed.go を業務的に妥当 + 設計書整合な状態に再整備（テナント B Admin 不在 + test_strategy.md §4.2 乖離） | 2026-05-29 | 起票のみ（post-MVP。Phase 1 README レビュー中に発覚） |
...
| ops-108 | ポートフォリオ用 GitHub 公開戦略の未決（README 内 dev-journal リンクの整合性 + 用語統一） |
| ops-110 | ai-dev-framework/operations/ 配下の旧エージェント名を現行 8 体に現行化（実体対応済み、状態遷移は次セッション判定） |
