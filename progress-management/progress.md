...
| 188 | 監視設計のロググループ命名残置をリネーム（monitoring.md + 実装側 docker awslogs-group 設定の同期更新） | 2026-05-24 | resolved（2026-05-25: PR #153 マージ済み squash `420f0ea`。当初想定の「リネームのみ」から設計 vs 実装の重大乖離が判明し awslogs 完全実装スコープに拡大。architect 計画書 v2 で reviewer PASS → 内部レビュー 1 ラウンド PASS → codex 1 ラウンド APPROVE 相当。確定ユーザー判断 P-0=c / P-1=b / P-2=a / P-3=a / P-4=b / P-5=a / P-6=a（P-1 は issue 起票案 B から `/expense-saas/{env}/api` に変更、SSM 階層整合）。AWS 実 apply 系の受け入れ基準 #3/#4 は Step 11-E 実 apply セッションで消化。dev-journal コミット: 1d1352b / 2e6c327 / 本コミット） |
