...
| 147 | per_page UI セレクタ + ページネーションフッター（再オープン: AppPaginationFooter を `AppDataGrid` の `slots.footer` 経由で DataGrid フッターコンテナに統合する設計書改訂 + 4 画面実装修正。確定方針 D-1 + ②a） | 2026-04-25（初回）/ 2026-04-27（再オープン） | 未着手（SMK-081 ブロッカー） |
