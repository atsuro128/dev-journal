...
| 目的 | セッション終了時に未コミット変更を検知し、commit-message.md の参照を指示 |
| 動作 | ブロック（exit 2）。3回連続発火でループ回避（exit 0） |
| 確認対象 | root-project, expense-saas, ai-dev-framework, dev-journal |
| ログ出力先 | `dev-journal/logs/hooks/hook-warnings.log` |
| カウンター | `%TEMP%/.claude-stop-check-counter`（git管理外） |

**設計意図**: 未コミット変更がある場合、`ai-dev-framework/rules/commit-message.md` の参照を
メッセージに含めてブロックする。commit-message.md にはセッションログ記録・リポジトリ移動・
メッセージ形式等のルールが集約されており、参照させることで全ルールの遵守を促す。
3回連続発火時はループ回避のため警告のみで通過させる。
...
Claude 停止試行 → exit 2 ブロック
  ...
  → Claude がコミット作業を実行
  → 新たな変更が発生（hook-warnings.log 等）
  → Claude 停止試行 → exit 2 ブロック → ...
```

この問題はカウンター方式で解決した。連続発火回数をカウンターファイル（`%TEMP%` に配置、
git管理外）で追跡し、3回連続で発火した場合は exit 0 で通過させる。
未コミット変更が解消されるとカウンターはリセットされる。
