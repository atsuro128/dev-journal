...
- **独立ファイルへの出力**: 同時実行可（例: db_schema.md と security.md）
- **同一ファイルへの出力**: 順序制御が必要（後述の共有ファイル調整を参照）
...
複数エージェントが同一ファイルに書き込む場合の制御:

| ファイル | 初回作成 | 追記 |
|---------|---------|------|
| `screens.md` | basic-designer（4+5-I: 全体構成） | basic-designer + detail-designer（機能別に順次追記） |
| `openapi.yaml` | detail-designer（4+5-C: 認証） | detail-designer（機能別に順次追記） |
| `ui_flow.md` | basic-designer（4+5-I: 初版） | design-architect（4+5-G: 最終統合） |

**原則**: 同一ファイルへの書き込みは直列化する。指揮役が先行タスクの完了を確認してから次のエージェントを起動する。

### コミットタイミング

```
Wave 完了 → reviewer PASS → ユーザーにコミット確認 → /commit
```

Wave 単位でコミットする。機能単位のこまかいコミットはしない（手戻り時に revert しやすくするため）。
...
```
指揮役:
  1. 4 エージェントの出力を確認（ファイル存在・基本的な完全性）
  2. design-unit-reviewer で基盤成果物の上流整合性をスポットチェック
  3. ユーザーにコミット確認 → /commit（Wave 1）
```
...
**共有ファイルの衝突防止**: screens.md / openapi.yaml は機能別セクションが明確に分かれるため、
指揮役がセクション名を指定することで機能間の並列書き込みが可能。
