...
Step 4+5（基本・詳細設計）以降の開発フェーズにおいて、**役割（ロール）** に対応したカスタムサブエージェント（全15体）を `.claude/agents/` に配置する。
...
|---|-------|------|-------|------|------------|
| 1 | `design-architect` | 計画者・統合者 | opus | Write（計画ファイル + Wave 4 統合） | `.claude/agents/design-architect.md` |
...
- **Write（計画ファイル + 統合成果物）**: architect（タスク計画ファイル作成 + Wave 4 統合）
...
├── step4-5.md    # design-architect が作成・更新
...
```
design-architect: Wave構成・タスク定義・受け入れ基準を策定
    ↓
Wave 1（並列 — 独立ファイルへの出力）:
  db-designer → db_schema.md
  detail-designer → security.md, monitoring.md
  basic-designer → screens.md, ui_flow.md
    ↓
design-unit-reviewer × 各機能（並列）: 基本/詳細/DB 横断 + 上流チェック
    ↓
Wave 2-3（機能間は並列、機能内は直列）:
  各機能ごと: basic-designer → detail-designer の順で画面詳細 → API 定義
    ↓
design-unit-reviewer × 各機能（並列）: 同上
    ↓
Wave 4（統合 — design-architect が担当）:
...
- **機能間は並列**: 認証と経費CRUD は同時に進行可能
- **機能内は直列**: 1 機能について basic-designer（画面）→ detail-designer（API）の順で実行
  - API 設計は画面仕様を参照するため、画面設計の完了が前提
...
全成果物が揃った後に実行する（Wave 4 / 実装フェーズ最終）。
...
```
Agent(subagent_type="design-architect", prompt="Step 4+5 の Wave 構成を分析し...")
Agent(subagent_type="basic-designer", prompt="認証関連画面の詳細仕様を作成して...")
Agent(subagent_type="design-unit-reviewer", prompt="認証機能の設計整合性をレビューして...")
```
