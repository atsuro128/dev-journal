> **[統合済み]** 本書の内容は `policies.md` に統合されました。正本は `policies.md` SS3（RBAC）を参照してください。
...
| テナント内メンバー名一覧取得（フィルタ用） | ○ | × | × | ○ | 読み取り専用。SCR-ADM-001 の申請者フィルタドロップダウンで使用。メンバー管理（Phase 3）とは異なり、名前の一覧のみ返却する |
| メンバー招待（Phase 3） | ○ | × | × | × | |
| ロール変更（Phase 3） | ○ | × | × | × | |
| メンバー削除（Phase 3） | ○ | × | × | × | |
| 監査ログ閲覧（Phase 3） | ○ | × | × | × | |
...
| ID | ルール | 対象操作 |
|----|--------|---------|
| RBC-010 | 申請系操作（作成/編集/削除/提出）は、ロール（Member/Approver/Admin/Accounting）に関わらず所有者のレポートに限定 | CRUD、提出 |
| RBC-011 | Approver は同テナントの submitted レポートを承認/却下可能 | 承認、却下 |
| RBC-012 | Accounting は同テナントの approved レポートのうち、自分以外が作成したものに限り支払完了を記録可能（自己処理禁止） | 支払完了 |
| RBC-013 | Admin は同テナントの全レポートを閲覧可能（編集不可） | 閲覧 |
| RBC-014 | Admin は自分が作成したレポートに限り申請者として操作可能（作成・編集・削除・提出）。他者のレポートは閲覧のみ | Admin の申請系操作 |
| RBC-015 | MVP では Approver は同一テナント内の submitted 状態のレポートを全件閲覧・承認・却下できる（全件承認型。担当者割当は Phase 3 以降の拡張）。加えて、自分が承認/却下したレポートは状態変更後も追跡閲覧可能 | Approver の承認スコープ |
| RBC-016 | Approver は自分が作成したレポートを承認・却下できない（自己承認・自己却下禁止） | 承認、却下 |

### 4.3 補足・解釈

以下は SS4.1〜4.2 の正式ルールに対する補足説明である。独立した新規ルールではない。
