...
**更新不可**: 添付ファイルのメタデータは作成後に変更しない。修正が必要な場合は削除→再アップロードで対応する。
...
| ID | 不変条件 | 実装責務 |
|----|---------|---------|
| TNT-001 | 全テーブルに tenant_id が存在する | DB スキーマ |
...
| RBC-014 | Approver の自己承認・自己却下は禁止 | ドメイン層 |
...
- [x] テナント分離（tenant_id 冗長保持・RLS）が反映されているか → 全エンティティに tenant_id
