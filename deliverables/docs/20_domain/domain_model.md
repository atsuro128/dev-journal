...
| DAT-003 | AuditLog は INSERT ONLY（更新・削除禁止）。Phase 3 で実装するが、不変条件としてここで定義する | ドメイン層 + DB制約 |
...
| エラー | 意味 | HTTP 変換 | 対応 UC-ID |
|--------|------|----------|-----------|
| InvalidStateTransition | 禁止された状態遷移 | 422 Unprocessable Entity | UC-M06, UC-A02, UC-A03, UC-AC02 |
| SelfApprovalNotAllowed | 自己承認の試行 | 403 Forbidden | UC-A02, UC-A03 |
| SelfPaymentNotAllowed | 自己処理（自分のレポートの支払完了記録）の試行 | 403 Forbidden | UC-AC02 |
| EmptyReportSubmission | 明細0件での提出 | 422 Unprocessable Entity | UC-M06 |
| InvalidPeriod | period_start > period_end | 422 Unprocessable Entity | UC-M01, UC-M04 |
| InvalidAmount | amount ≦ 0 | 422 Unprocessable Entity | UC-M02, UC-M05 |
| ReportNotEditable | draft 以外での編集 | 422 Unprocessable Entity | UC-M04, UC-M02, UC-M05, UC-M03, UC-M03a |
| NoApproverInTenant | テナントに Approver が0人の状態での提出 | 422 Unprocessable Entity | UC-M06 |
| ReportNotDeletable | draft 以外での削除 | 422 Unprocessable Entity | UC-M07 |
| ResourceNotFound | リソースが見つからない（テナント境界越え含む） | 404 Not Found | 全UC（横断） |
| Forbidden | ロール不足・所有権不足 | 403 Forbidden | 全UC（横断） |
| InvalidFileType | 許可されていないMIMEタイプ | 422 Unprocessable Entity | UC-M03 |
| FileTooLarge | 5MB 超過 | 413 Payload Too Large | UC-M03 |
| MissingRejectionReason | 却下理由が空での却下 | 422 Unprocessable Entity | UC-A03 |
| ConflictError | 楽観的ロック競合（別ユーザーが先に更新済み） | 409 Conflict | 全UC（横断、更新系） |
...
- [x] audit_logs の INSERT ONLY 制約が明記されているか → セクション6.4 不変条件テーブルに DAT-003 として定義 + セクション9.1 で詳細記載
