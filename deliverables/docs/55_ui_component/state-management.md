...
- 永続化: sessionStorage（タブを閉じると破棄）。アクセストークン・リフレッシュトークンを sessionStorage に保存し、F5 リロード後もログイン状態を維持する。XSS リスクは React の自動エスケープと将来の CSP 設定で軽減し、本番運用では httpOnly Cookie への移行を検討する
...
### 3.1 ページネーション URL クエリ ⇔ Hook ⇔ API URL 連動仕様
...
| URL → Hook 反映 | `useSearchParams` で `per_page` / `page` を読み取り、Hook に渡す。`per_page` を `parseInt` で解釈し、NaN または負数の場合は `20` にフォールバックする（FE 側 fail-soft） |
| 範囲内不正値の扱い | 範囲内の不正値（例: `0`, `101`）は FE で事前クランプせず、API クエリにそのまま乗せて BE のバリデーション（`per_page: 1〜100`）に委ねる |
| UI → URL 反映 | `PageSizeSelector` の `onPerPageChange` で URL クエリ `per_page` を更新する |
| per_page 変更時の page リセット | `per_page` 変更時は同時に `page=1` にリセットする（既存 status / from / to フィルタ変更時と同一パターン）。`setSearchParams` は **1 回のコールに集約**して `per_page` と `page` を同時更新する（race 回避） |
...
| フッター表示 | `AppPaginationFooter` は常時表示（`totalPages <= 1` でも非表示にしない） |
| 件数表示の経路 | API レスポンス `pagination.total_count` を `AppPaginationFooter` の `totalCount` prop にそのまま渡し、フッター左側に「{start} - {end} / 全 {total} 件」を描画する。`start` / `end` の算出は `AppPaginationFooter` 内部（`currentPage` / `perPage` / `totalCount` から計算）で行うため、画面側で算出ロジックを持たない（`common-components.md` §AppPaginationFooter §件数表示フォーマット参照） |
...
#### AllReportsPage（SCR-ADM-001）の URL 駆動化

`AllReportsPage` の `page` 状態は `useState` から `useSearchParams` 駆動に移行する。`per_page` 配線の前提として `page` も URL 駆動化が必要なためスコープに含める。他 3 画面（`ReportListPage` / `ApprovalListPage` / `PaymentListPage`）は既に `useSearchParams` で `page` を管理済み。
...
export const ERROR_MESSAGES: Record<string, string> = {
  ...
  // INVALID_PERIOD: 同一の業務ルール（RPT-003）を 2 経路から表示する。フィールド別文言は
  //   FE 側の Zod refine（path: ['periodStart'] / ['periodEnd']）で個別出力するため、
  //   サーバー由来の汎用フォールバックは開始日側文言（多数派）を採用する。
  //   フィールド別表示は FE のバリデーション層で担保する（report-create.md §4 V5-S / V5-E 参照）。
  INVALID_PERIOD:
    '開始日は終了日以前を指定してください',
  INVALID_AMOUNT:
    '金額は正の整数で入力してください',
...
| RATE_LIMIT_EXCEEDED | しばらく待ってから再試行してください | 汎用メッセージ（画面固有の上書きは §6.5.7 参照） |
...
| INVALID_PERIOD | 開始日は終了日以前を指定してください（開始日フィールド側）／ 終了日は開始日以降を指定してください（終了日フィールド側） | report-create.md §4 V5-S / V5-E、report-edit.md §4 V5-S / V5-E。FE の Zod refine が `path: ['periodStart']` / `['periodEnd']` で個別出力する。サーバー由来の汎用フォールバックは開始日側文言を使用 |
...
#### 6.5.3 onError での err.message 使用方針

> **重要**: `client.ts` の `handleErrorResponse` で `SERVER_ERROR_MESSAGES` によるマッピングが導入され、`ApiClientError.message` は throw の時点で**既に正しい日本語文言にマッピング済み**になっている。
...
**廃止（旧パターン）**: 以下の `getErrorMessage(error)` ヘルパー呼び出し方式は、`client.ts` 層のマッピング導入以前のパターンである。二重変換になるため使用しないこと。

```typescript
// 廃止: client.ts 層のマッピング前の旧パターン。
...
| 成功メッセージ | 「レポートを作成しました」「承認しました」「ファイルをアップロードしました」「添付ファイルを削除しました」 | 各 Hook の `onSuccess` 内の定数オブジェクト、または操作ハンドラ内（add モードの pending file 操作のように Hook を介さないケース） |
| クライアントサイドバリデーションメッセージ | 「タイトルを入力してください」等 | Zod スキーマ定義（`src/lib/validations/*.ts`） |

##### 添付ファイル成功トーストの発火条件（add / edit 両モード共通）

明細スライドパネルの添付ファイル操作については、追加（add）モード・編集（edit）モードの両方で同一文言のトーストを発火する。これは「ローカル保留方式」という実装詳細を UI 文言に露出させない方針（`50_detail_design/screens/report-detail.md` §6・§7）に準拠する。

| 文言 | edit モードの発火条件 | add モードの発火条件 |
|------|---------------------|--------------------|
| 「ファイルをアップロードしました」 | `useUploadAttachment` の `onSuccess`（POST 成功 = 即時 S3 + DB 保存完了） | ファイル選択 → クライアントサイドバリデーション通過 → ローカル state への保留が完了した時点（API 呼び出しは行わないため `useUploadAttachment` を介さず、`AttachmentArea` / `AttachmentUploader` のハンドラ内でトーストをトリガする） |
| 「添付ファイルを削除しました」 | `useDeleteAttachment` の `onSuccess`（DELETE 成功 = 論理削除完了） | pending file の「削除」ボタン押下 → ローカル state からの除去完了時点（API 呼び出しなし、確認ダイアログなし。`AttachmentArea` / `AttachmentUploader` のハンドラ内でトーストをトリガする） |

**実装上の注意**:

- add モードのトリガは Hook（mutation）の `onSuccess` ではなく、`AttachmentArea` / `AttachmentUploader` 内のハンドラから直接トースト表示をトリガする（`AppToast` の state を更新する、または `SnackbarContext` の `showSnackbar` を呼ぶ）
- 文言定数はモード別の出し分けをせず、`ATTACHMENT_UPLOAD_SUCCESS = 'ファイルをアップロードしました'`、`ATTACHMENT_DELETE_SUCCESS = '添付ファイルを削除しました'` のような共通定数として 1 箇所に定義する（add モード固有の文言を新設しない）
- 部分失敗時のトーストは別ルール: 追加モードの「保存する」押下後の順次アップロードで一部添付が失敗した場合のみ、警告トースト「N 件の添付ファイルがアップロードに失敗しました。再試行してください」を発火する（`50_detail_design/screens/report-detail.md` §6「部分失敗時のロールバック方針」参照）

#### 6.5.6 警告メッセージ定義（確認ダイアログ用）

エラーではなく警告レベルのユーザー通知（ポリシー `ITM-007` 等）に対応するメッセージを定義する。これらのメッセージは `ERROR_MESSAGES`（エラー用）とは別に `src/lib/warningMessages.ts` に定数として配置し、**API エラーコードには対応しない**（クライアント側の条件判定で表示する）。

##### 表示方式

警告メッセージの表示方式は **Snackbar ではなく ConfirmDialog（確認ダイアログ）** を使用する。ユーザーが確認ボタンを押下するまで処理を進めないようにし、入力ミスの検出機会を確保するためである。common-components.md の `ConfirmDialog` を流用し、Snackbar による一時通知は行わない。

##### 警告メッセージ一覧

| キー | 文言 | 表示タイミング | 対応ポリシー |
|------|------|--------------|--------------|
| `ITEM_DATE_OUTSIDE_PERIOD_WARNING` | 明細日付がレポートの対象期間外です。入力を確認してください。 | 明細追加・編集の保存ボタン押下時に `expense_date` がレポートの `period_start` / `period_end` の範囲外の場合、保存処理の前に ConfirmDialog で表示する。View mode（閲覧時）では保存操作が存在しないため表示しない | `policies.md` ITM-007（警告のみ、エラーにしない） |
...
##### ConfirmDialog 呼び出し方針（ITM-007）

- ダイアログは `common-components.md` の `ConfirmDialog` を使用する。入力フィールド（`inputField`）は不要
- タイトルは「入力内容の確認」等の汎用文言でよい（本文で文脈を示すため）
- 確認ボタンラベルは「保存する」、キャンセルボタンラベルはデフォルト（「キャンセル」）
- 確認押下 → 保存処理を継続（期間外でも保存は許可）
- キャンセル押下 → ダイアログを閉じてフォームに戻る（保存しない、フォームの入力値は維持）
- 期間内日付の場合は ConfirmDialog を表示せず、そのまま保存処理を実行する
...
| `src/lib/warningMessages.ts` | 警告メッセージ定数（ITM-007 等、エラー扱いしない確認用メッセージ）|
