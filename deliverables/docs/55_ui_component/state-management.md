...
export const ERROR_MESSAGES: Record<string, string> = {
  ...
  //   フィールド別表示は FE のバリデーション層で担保する（issue #141 / report-create.md §4 V5-S / V5-E 参照）。
  INVALID_PERIOD:
    '開始日は終了日以前を指定してください',
  INVALID_AMOUNT:
    '金額は正の整数で入力してください',
...
| INVALID_PERIOD | 開始日は終了日以前を指定してください（開始日フィールド側）／ 終了日は開始日以降を指定してください（終了日フィールド側） | report-create.md §4 V5-S / V5-E、report-edit.md §4 V5-S / V5-E。FE の Zod refine が `path: ['periodStart']` / `['periodEnd']` で個別出力する。サーバー由来の汎用フォールバックは開始日側文言を使用（issue #141） |
...
##### 添付ファイル成功トーストの発火条件（add / edit 両モード共通、issue #143）

明細スライドパネルの添付ファイル操作については、追加（add）モード・編集（edit）モードの両方で同一文言のトーストを発火する。これは「ローカル保留方式」という実装詳細を UI 文言に露出させない案 A の方針（`50_detail_design/screens/report-detail.md` §6・§7）に準拠する。
