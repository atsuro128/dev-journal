...
- 配置: `components/report/ReportForm.tsx`（report-create.md と共通）
...
Props 型定義は `report-create.md §ReportForm` を参照。編集画面では以下の Props 値で使用する。
...
- 配置: `components/ui/FormAlert.tsx`（auth-login.md、report-create.md と共通）
...
Props 型定義は `report-create.md §FormAlert` を参照。

### ReportPeriodField（共通）

- 配置: `components/report/ReportPeriodField.tsx`（report-create.md と共通）
- 責務: 対象期間の開始日・終了日を横並びに配置する
- 対応セクション: `50_detail_design/screens/report-edit.md` &sect;3

Props 型定義は `report-create.md §ReportPeriodField` を参照。

### ReportFormActions（共通）

- 配置: `components/report/ReportFormActions.tsx`（report-create.md と共通）
- 責務: フォーム下部のアクションボタン群。編集画面では submitLabel が「保存する」、キャンセルは詳細画面に戻る
- 対応セクション: `50_detail_design/screens/report-edit.md` &sect;7

Props 型定義は `report-create.md §ReportFormActions` を参照。
