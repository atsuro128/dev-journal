...
- 配置: `components/ui/SelfLabel.tsx`（workflow-pending.md と共通）
...
- 配置: `components/ui/FilterResetButton.tsx`（workflow-pending.md と共通）
...
| Props | 型 | 必須 | 説明 | データソース |
|-------|---|------|------|------------|
| `onReset` | `() => void` | Yes | フィルタリセットコールバック | PayableFilterBar |
| `isFiltered` | `boolean` | Yes | フィルタ適用中フラグ | PayableFilterBar |
...
| Props | 型 | 必須 | 説明 | データソース |
|-------|---|------|------|------------|
| `title` | `string` | Yes | ページタイトル | 固定値（「支払待ち一覧」） |
