...
| &sect;4 バリデーションルール | ReportForm（reportCreateSchema） | V1-V4, V5-S, V5-E: クライアントサイド Zod バリデーション。V5（対象期間の相関）は `path: ['periodStart']`（V5-S）と `path: ['periodEnd']`（V5-E）の 2 経路で個別出力し、両フィールドの onBlur で `trigger(['periodStart', 'periodEnd'])` を呼んで再評価する |
