...
- **件数 0 時の最小高さ確保**: 件数 0（`rows.length === 0`）時のみ、`NoRowsOverlay` 内の `EmptyState`（アクションボタン付きを含む）が画面内に収まるよう、共通基盤側で `minHeight: 300`（px）を**条件付きで**適用する（実装パターン: `minHeight: rows.length === 0 ? 300 : undefined`）。1 件以上のときは `minHeight` を適用せず DataGrid の自然高さに任せる（テーブル末尾の不要な余白を防ぐ）。各画面側で minHeight を個別設定する必要はない（全 4 一覧画面で統一挙動とする）。issue #154 対応（PR #107 一律 200 → 再オープン後 案A 条件付き 300）
