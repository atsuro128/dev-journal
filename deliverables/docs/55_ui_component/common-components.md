...
- **件数 0 時の NoRowsOverlay auto-fit**: 件数 0 時に `NoRowsOverlay`（内部に `EmptyState` を配置）の領域を `EmptyState` の自然高さに追従させるため、共通基盤側で MUI X v8 の CSS 変数 `--DataGrid-overlayHeight: 'auto'` を sx で指定する。これにより親要素（`MuiDataGrid-overlayWrapperInner`）が子要素（`EmptyState`）の content-box サイズにフィットし、centering による不要な上下余白が出ない。`EmptyState` のメッセージ・アクションボタンが将来変わっても自動追従するため hardcoded な高さ調整は不要。1 件以上のときは変数が無視され DataGrid の自然高さに任せる（テーブル末尾の不要な余白を防ぐ）。各画面側で個別設定する必要はない（全 4 一覧画面で統一挙動とする）。issue #154 対応（PR #107 一律 minHeight 200 → 条件付き 300 → 400 → CSS 変数 auto）
- **横スクロール対応**: ルート Box に `overflowX: 'auto'` を付与し、列幅合計（各列の `width` 合計）が画面幅を超える場合に DataGrid 領域内で横スクロールできるようにする。ページ全体の横スクロールにはせず、テーブル領域内に閉じ込める。スマホ幅（375px）で列内容が ellipsis 省略される問題（issue #160）を回避するため、各画面側で各列に **`flex` を使わず `width`（絶対値）を必須で** 設定する。`flex` を使うと MUI X DataGrid がコンテナ幅（`Box width: 100%`）に合わせて列幅を縮小するため `minWidth` が効かなくなる（MUI X DataGrid の仕様）。`width` 絶対値固定により列幅合計が画面幅を超えてルート Box の `overflowX: 'auto'` が発火する（候補 A 採用、issue #160 再オープン対応）。標準値は申請者名 120px / タイトル 200px / 金額 110px / ステータス 110px / 日付 110px を基準とする（各列の最終値は画面仕様書に従う）
