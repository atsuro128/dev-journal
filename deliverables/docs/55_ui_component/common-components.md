...
- **件数 0 時の最小高さ確保**: 件数 0（`rows.length === 0`）時のみ、`NoRowsOverlay` 内の `EmptyState`（アクションボタン付きを含む）と `AppPaginationFooter` の間に視覚的余白を確保するため、共通基盤側で `minHeight: 400`（px）を**条件付きで**適用する（実装パターン: `minHeight: rows.length === 0 ? 400 : undefined`）。400 の根拠: ColumnHeader 56 + EmptyState 必要量 236（Inbox アイコン 48 + メッセージ 24 + アクションボタン 36 + gaps 32 + py 上下 96）+ AppPaginationFooter 53 + ボタンとフッター間の視覚余白 ~30 = 約 375px を丸めた値。1 件以上のときは `minHeight` を適用せず DataGrid の自然高さに任せる（テーブル末尾の不要な余白を防ぐ）。各画面側で minHeight を個別設定する必要はない（全 4 一覧画面で統一挙動とする）。issue #154 対応（PR #107 一律 200 → 条件付き 300 → 400 へ調整）
