...
- **件数 0 時の最小高さ確保**: 件数 0（`rows.length === 0`）時のみ、`NoRowsOverlay` 内の `EmptyState`（アクションボタン付きを含む）が画面内に収まるよう、共通基盤側で `minHeight: 361`（px）を**条件付きで**適用する（実装パターン: `minHeight: rows.length === 0 ? 361 : undefined`）。361 の根拠（overlayWrapperInner と EmptyState の高さを一致させ centering slack を 0 にする計算済み最小値）: ColumnHeader 56 + EmptyState 必要量 236.5（Inbox アイコン 48 + メッセージ 24 + アクションボタン 36 + gaps 32 + py 上下 96）+ filler 15 + AppPaginationFooter 53 = 360.5px を丸めた値。button-footer 間の視覚余白は EmptyState 自身の `py: 6` の py-bottom（48px）で確保される。1 件以上のときは `minHeight` を適用せず DataGrid の自然高さに任せる（テーブル末尾の不要な余白を防ぐ）。各画面側で minHeight を個別設定する必要はない（全 4 一覧画面で統一挙動とする）。CSS 変数 `--DataGrid-overlayHeight: 'auto'` による真の auto-fit は MUI X v8 で機能しない（`auto` が数値計算文脈で無効値となり fallback `calc(var(--height) * 2) = 104px` に落ちる）ため、計算済み hardcoded 値を採用。issue #154 対応（PR #107 一律 minHeight 200 → 条件付き 300 → 400 → CSS 変数 auto 失敗 → 361 へ調整）
- **横スクロール対応**: ルート Box に `overflowX: 'auto'` を付与し、列幅合計が画面幅を超える場合に DataGrid 領域内で横スクロールできるようにする。ページ全体の横スクロールにはせず、テーブル領域内に閉じ込める。各画面側の列定義は `flex` + `minWidth` を併用する（PC 幅では flex で右側余白を埋め、スマホ幅では minWidth が下限となる方針）。標準 minWidth はタイトル 200px / 対象期間 180px / 申請者名 120px / 金額 130px / ステータス 120px / 処理結果 120px / 現在ステータス 170px / 日付 110px を基準とする（issue #160 再対応で金額・ステータス系を引き上げ。横スクロールは AppDataGrid の Box `minWidth: 0` + `overflowX: auto` で発火するが、column 自体の minWidth が小さいと横スクロールしても ellipsis で見切れるため。値の根拠: 合計金額は `¥9,999,999`（9文字）+ ヘッダ「合計金額」+ ソートアイコン、ステータスは StatusChip 最長「承認済み/支払済み」+ Chip padding + cell padding、現在ステータスはヘッダ「現在ステータス」(6文字) + ソートアイコン。SMK-101 再々検証で現在ステータスが 140 では不足（ヘッダ「現在ステータス」6 文字 + ソートアイコンで切れる）と判明し 170 に再引き上げ。）
- **Box ラッパーの `minWidth: 0` 必須規定**（issue #160 再対応）: ルート Box の sx には `minWidth: 0` を**必ず**指定すること。理由: 親コンポーネントが `display: flex` の場合、flex item の `min-width` 既定値は `auto`（コンテンツ幅に追従）であるため、Box が DataGrid 内部幅（実測 726px）に追従して膨張し、`overflowX: 'auto'` が発火しない CSS Flexbox の罠が存在する。`minWidth: 0` により Box を flex container 内でビューポート幅（375px）に収め、`overflowX: 'auto'` を正しく機能させる。実機計測値: columnsTotalWidth=690 / hasScrollX=0 / Box=726px / 親=375px / 親 display=flex。過去 PR #107（overflowX 追加）/ PR #114（width 絶対値固定）が空振りした構造的理由はこの `min-width: auto` の罠で説明できる。実装例: `<Box sx={{ width: '100%', minWidth: 0, overflowX: 'auto' }}>`
...
- **TextField への `autoFocus` 禁止**（issue #162 再対応）: `inputField` ありの `TextField` に `autoFocus` を付与してはならない。理由: MUI Dialog の FocusTrap 機構と競合し、ダイアログ開画時に「autoFocus でフォーカス付与 → FocusTrap が別要素へフォーカス移動 → TextField から blur 発火 → `onBlur` で `setTouched(true)`」という連鎖が起き、初回 paint 時点で `touched=true` となりエラー文言が表示される regression を引き起こす。UX への影響: Approver が TextField を一度クリックしてから入力する仕様になるが、却下操作は Approver の意識的な判断を伴う操作であるため影響は軽微と判断する。プログラム的フォーカス制御が必要になった場合は `TransitionProps.onEntered` + `useRef` を使用すること（autoFocus は再追加しないこと）
