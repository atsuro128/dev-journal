...
- **横スクロール対応**: ルート Box に `overflowX: 'auto'` を付与し、列幅合計が画面幅を超える場合に DataGrid 領域内で横スクロールできるようにする。ページ全体の横スクロールにはせず、テーブル領域内に閉じ込める。各画面側の列定義は `flex` + `minWidth` を併用する（PC 幅では flex で右側余白を埋め、スマホ幅では minWidth が下限となる方針）。標準 minWidth はタイトル 200px / 対象期間 180px / 申請者名 120px / 金額 130px / ステータス 120px / 処理結果 120px / 現在ステータス 140px / 日付 110px を基準とする（issue #160 再対応で金額・ステータス系を引き上げ。横スクロールは AppDataGrid の Box `minWidth: 0` + `overflowX: auto` で発火するが、column 自体の minWidth が小さいと横スクロールしても ellipsis で見切れるため。値の根拠: 合計金額は `¥9,999,999`（9文字）+ ヘッダ「合計金額」+ ソートアイコン、ステータスは StatusChip 最長「承認済み/支払済み」+ Chip padding + cell padding、現在ステータスはヘッダ「現在ステータス」(6文字) + ソートアイコン）
