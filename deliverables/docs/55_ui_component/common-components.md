...
- **横スクロール対応**: ルート Box に `overflowX: 'auto'` を付与し、列幅合計が画面幅を超える場合に DataGrid 領域内で横スクロールできるようにする。ページ全体の横スクロールにはせず、テーブル領域内に閉じ込める。各画面側の列定義は `flex` + `minWidth` を併用する（PC 幅では flex で右側余白を埋め、スマホ幅では minWidth が下限となる方針）。標準 minWidth はタイトル 200px / 対象期間 180px / 申請者名 120px / 金額 100px / ステータス 100px / 日付 110px を基準とする
