...
| test | go test ./...（PostgreSQL サービスコンテナ） | vitest run |
| build | go build ./cmd/server | npm run build |

各ステージは前ステージ成功時のみ実行（fail-fast）。バックエンドとフロントエンドは同一ステージ内で並列実行。

### 8.2 main マージパイプライン（main ブランチへのプッシュ時）
...
| ルール | 設定 |
|--------|------|
| main への直接プッシュ | 禁止（Step 9 以降。Step 8 は許可） |
| PR マージ条件 | CI 全ステージ成功必須 |
| ステータスチェック最新化 | 必須 |

### 8.4 テスト実行環境

GitHub Actions の service container として PostgreSQL 16 を起動する。テスト前にマイグレーションを実行する。
...
| カーソルベースページネーション（デフォルト20件） | NFR-PERF-004 | - | §5.3 ページネーション |

### 9.2 セキュリティ（§4.2）

| 要件概要 | 関連ルールID | 反映先 ADR | 反映先セクション |
|----------|-------------|-----------|----------------|
| JWT RS256 認証 | NFR-SEC-001, SEC-003, SEC-004 | ADR-0001（golang-jwt 選定） | §3.3 認証フロー, §6.1 多層防御 [3] |
...
| RDS 自動バックアップ 7日間保持 | NFR-AVAIL-003 | ADR-0004（RDS 選定） | §2 システム全体構成 |
...
- [x] CI/CD パイプライン構成が定義されているか → §8 で PR パイプライン・main マージパイプライン・ブランチ保護ルール・テスト実行環境を定義
