...
| `adr/0004-infra.md` | インフラ選定（コンピュート / DB / ストレージ） |
...
| デプロイ | EC2 上の `docker pull` + `systemctl restart`（ECR pull デプロイ、UD-6=A） | 枠のみ（現時点では手動デプロイ） |
...
| 同時接続ユーザー数 100 | NFR-PERF-003 | ADR-0002（Shared DB 選定理由）, ADR-0004（EC2 t3.micro スペック、§ポートフォリオ対応） | §2 システム全体構成 |
