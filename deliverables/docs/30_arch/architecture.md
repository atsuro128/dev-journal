...
│   │   ├── authorizer.go        # 認可ロジック
│   │   ├── interfaces.go        # サービスインターフェース
│   │   ├── params.go            # サービスパラメータ型
│   │   └── not_implemented.go   # 未実装サービスメソッドのスタブ
│   ├── domain/                   # ドメイン層（エンティティ・値オブジェクト直置き）
│   │   ├── entity.go            # User, Tenant, TenantMembership 等
│   │   ├── report.go            # ExpenseReport 集約ルート
│   │   ├── item.go              # ExpenseItem
│   │   ├── auth.go              # 認証関連（RefreshToken 等）
│   │   ├── actor.go             # Actor（認可主体）
│   │   ├── dto.go               # レイヤー間転送オブジェクト
│   │   ├── enum.go              # ReportStatus, Role, Category
│   │   ├── errors.go            # ドメインエラー
...
│   │   ├── reports.ts
│   │   ├── types.ts              # API レスポンス型
│   │   └── adminTypes.ts         # 管理者向け API 型
│   ├── stores/                   # クライアント状態管理
│   │   └── auth.ts               # 認証状態（トークン保持）
│   ├── contexts/                 # React Context
│   │   └── SnackbarContext.ts    # 通知用コンテキスト
│   ├── lib/                      # ユーティリティ
│   │   ├── constants.ts
...
各ディレクトリ直下に `__tests__/` サブディレクトリを配置し、コンポーネント・Hook・ユーティリティのユニットテストを格納する（コロケーションパターン）。
