...
```mermaid
graph TD
    AUTH002[ログイン] --> DASH001[ダッシュボード<br/>Member情報 + 承認待ち数]
...
```
ダッシュボード → 承認待ち一覧 → レポート詳細（明細・領収書確認） → 承認
```
...
```mermaid
graph TD
    AUTH002[ログイン] --> DASH001[ダッシュボード<br/>支払待ち数]
...
```mermaid
graph TD
    AUTH002[ログイン] --> DASH001[ダッシュボード<br/>テナント全体の概況]
