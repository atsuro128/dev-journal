...
> レポート詳細画面（SCR-RPT-004）では、Admin は閲覧のみ可能（編集・承認・却下ボタンは非表示）。Accounting は approved 状態かつ自分が作成したレポートでない場合に限り「支払完了」ボタンが表示される。これらの制御は SCR-RPT-004 側の仕様（screens/report-detail.md）で定義する。
