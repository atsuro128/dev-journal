...
コンピュート基盤は EC2 t3.micro（ADR-0004 §ポートフォリオ対応）を採用しており、本書のメトリクス収集・ログ転送方式はこの構成を前提とする。
...
| `30_arch/adr/0004-infra.md` | インフラ選定（コンピュート / DB / 監視基盤） |
...
| ログ出力形式 | JSON（構造化ログ） |
| ログ出力先 | stdout → CloudWatch Logs（Docker awslogs ログドライバ、UD-2=B） |
...
| 用途 | 方式 | 説明 |
|------|------|------|
| インフラメトリクス | AWS/EC2 標準メトリクス + CWAgent メモリ・ディスクメトリクス / RDS 標準メトリクス | EC2 標準は追加設定不要。CWAgent はメトリクスのみモードで導入（UD-3=A） |
...
AWS/EC2 標準メトリクス + CWAgent（メトリクスのみモード）+ RDS 標準メトリクスにより自動収集されるメトリクス。

#### EC2（AWS/EC2 + CWAgent）

| メトリクス | 説明 | 名前空間 | 収集元 |
|-----------|------|----------|--------|
| `CPUUtilization` | EC2 CPU 使用率 | `AWS/EC2` | 標準（追加 Agent 不要） |
| `NetworkIn` / `NetworkOut` | ネットワーク I/O | `AWS/EC2` | 標準 |
| `DiskReadBytes` / `DiskWriteBytes` | ディスク I/O | `AWS/EC2` | 標準 |
| `StatusCheckFailed` | インスタンスヘルスチェック失敗 | `AWS/EC2` | 標準（1 分間隔） |
| `mem_used_percent` | メモリ使用率 | `CWAgent` | CloudWatch Agent（メトリクスのみモード） |
| `disk_used_percent` | ディスク使用率（パーティション別） | `CWAgent` | CloudWatch Agent |

CloudWatch Agent はメトリクス収集のみを担当し、ログ転送は §2.1/§設計方針サマリーで定義した Docker awslogs ログドライバが担う（UD-2=B / UD-3=A）。
...
| 可用性 | インスタンス健全性 | EC2 `StatusCheckFailed` | > 0（連続 2 回） | Critical |
| リソース | EC2 CPU 使用率 | `AWS/EC2` `CPUUtilization` | > 80% | Warning |
| リソース | EC2 メモリ使用率 | `CWAgent` `mem_used_percent` | > 80% | Warning |
...
| `EC2-StatusCheckFailed` | `AWS/EC2` `StatusCheckFailed` | > 0 | 1 分 | 2/2（連続 2 回） |
| `High-5xx-Rate-Critical` | Math: `5xxCount/RequestCount*100` | > 5% | 1 分 | 3/5（5 分中 3 回） |

`EC2-StatusCheckFailed` は ALB ヘルスチェックと検知契機が重複しうるが、SG / NACL / インスタンス OS レイヤの障害は ALB 経路を通さずに検知できるため、別系統として併設する（UD-4=A、1 分間隔の早期検知を目的とする）。
...
| `High-EC2-CPU` | `AWS/EC2` `CPUUtilization` | > 80% | 5 分 | 3/3 |
| `High-EC2-Memory` | `CWAgent` `mem_used_percent` | > 80% | 5 分 | 3/3 |
