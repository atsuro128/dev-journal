...
**設計判断**: 保留 → 確定方式（temp テーブル / status カラム等）は DB スキーマ改修を要し MVP スコープ外。ロールバック方式（キャンセル時に DELETE API を呼ぶ）はネットワーク断時に S3 孤児ファイルが残るリスクがあり、既存添付削除のロールバック UI 状態管理も複雑化するため不採用。MVP では即時保存方式に統一し、UI 側で案内文を常時表示してユーザー期待値と整合させる（UI 仕様は `screens/report-detail.md` §7 参照）。
