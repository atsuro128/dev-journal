...
**設計判断**: 保留 → 確定方式（temp テーブル / status カラム等）は DB スキーマ改修を要し MVP スコープ外。ロールバック方式（キャンセル時に DELETE API を呼ぶ）はネットワーク断時に S3 孤児ファイルが残るリスクがあり、既存添付削除のロールバック UI 状態管理も複雑化するため不採用。MVP では即時保存方式に統一する。UI 側の案内文は当初常時表示としていたが、UI ノイズとなるため撤去した（issue 108 追加対応、2026-04-20）。削除確認ダイアログ（「元に戻せません」文言）でユーザーへの注意喚起は担保する（UI 仕様は `screens/report-detail.md` §7 参照）。
