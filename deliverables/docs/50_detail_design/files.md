...
**EC2 インスタンスプロファイル（IAM ロール）のポリシー**:
...
- S3 操作は全て EC2 インスタンスプロファイル経由。IAM ユーザーのアクセスキーは使用しない
...
添付ファイルは**ファイル選択時点で S3 + DB に永続化される**（即時保存方式）。明細フォームの保存ボタンとは独立しており、以下の性質を持つ。

| 観点 | 挙動 |
|------|------|
| アップロード確定 | POST `/api/reports/:id/items/:itemId/attachments` のレスポンス（201 Created）受領時点で S3 と DB の両方に永続化済み |
| 明細フォームの保存ボタン | 添付の永続化には関与しない。フォーム未保存・キャンセル時も添付は残る |
| 明細フォームのキャンセル | 追加済みの添付ファイルは削除されない（ロールバックしない） |
| ブラウザクローズ / セッション切断 | 追加済みの添付ファイルは残る |
| 削除操作 | DELETE `/api/reports/:id/items/:itemId/attachments/:attId` も同様に即時実行される。明細フォームのキャンセルでは復活しない（論理削除状態のまま） |

**設計判断**: 保留 → 確定方式（temp テーブル / status カラム等）は DB スキーマ改修を要し MVP スコープ外。ロールバック方式（キャンセル時に DELETE API を呼ぶ）はネットワーク断時に S3 孤児ファイルが残るリスクがあり、既存添付削除のロールバック UI 状態管理も複雑化するため不採用。MVP では即時保存方式に統一する。UI 側の案内文は当初常時表示としていたが、UI ノイズとなるため撤去した。削除確認ダイアログ（「元に戻せません」文言）でユーザーへの注意喚起は担保する（UI 仕様は `screens/report-detail.md` §7 参照）。

**追加モード（新規明細）の補足**: 新規明細追加時は `itemId` が未確定であるため、添付ファイルはフロント側で `File` オブジェクトをローカル state に保留し、明細作成（POST `/api/reports/:id/items`）でのレスポンスから取得した `itemId` に対して順次 POST `/api/reports/:id/items/:itemId/attachments` を呼び出す。API / バックエンド側の処理内容は本節の即時保存方式と同一であり、**バックエンド側の挙動変更は不要**（フロント側の呼び出しタイミングが明細作成後に後ろ倒しになるのみ）。順次アップロード中の部分失敗時も明細自体はロールバックせず、失敗した添付のみ再アップロードを促す（UI 挙動は `screens/report-detail.md` §6「モード別の保存処理」および §7「モード別の添付挙動」参照）。
...
署名付き GET URL を発行し、クライアントが S3 から直接ダウンロードまたはインライン表示する。API サーバーはプロキシしない。

ダウンロードとプレビューは別エンドポイントで提供し、署名付き URL の `ResponseContentDisposition` で挙動を切り替える。

| エンドポイント | Content-Disposition | ブラウザ挙動 |
|--------------|--------------------|----|
| `GET .../attachments/:attId/download` | `attachment; filename="..."` | ファイルダウンロード |
| `GET .../attachments/:attId/preview` | `inline; filename="..."` | ブラウザ内でインライン表示（JPEG/PNG/PDF） |

両エンドポイントは同一の認可ロジックを使用する（service 層の共通関数 `GeneratePresignedURL(disposition string)` で `disposition` パラメータにより分岐）。`StorageClient` インターフェースの `PresignGetObject` メソッドには `disposition` パラメータを追加し、署名付き URL 生成時に `ResponseContentDisposition` を設定する。
...
```
GET /api/reports/:id/items/:itemId/attachments/:attId/download
GET /api/reports/:id/items/:itemId/attachments/:attId/preview
```

**レスポンス（200 OK）** --- 両エンドポイント共通スキーマ `AttachmentAccess`:

```json
{
  "data": {
    "url": "https://expense-saas-attachments-prod.s3.ap-northeast-1.amazonaws.com/...",
    "file_name": "receipt_20260315.jpg",
    "mime_type": "image/jpeg",
    "file_size": 245760,
...
| パラメータ | ダウンロード (`/download`) | プレビュー (`/preview`) | 説明 |
|-----------|--------------------------|------------------------|------|
| 有効期限 | 15分（ATT-012） | 15分（ATT-012） | `time.Duration(15 * time.Minute)` |
