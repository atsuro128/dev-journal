...
| 403 | `PERMISSION_DENIED` | "You do not have permission to access this resource" | 所有権不足・リソースアクセス権なし |
...
| サービス層（Authorizer） | 所有権・ビジネスルール | 所有者判定（RBC-010）、自己承認禁止（RBC-016）、自己支払処理禁止（RBC-012）。詳細は `authz.md` SS3〜SS8 を参照 |
...
| `POST /api/auth/logout` | 不要（リフレッシュトークン） | なし |
...
| `GET /api/categories` | 認証済み（全ロール） | なし |
| `GET /api/tenant` | Admin | なし |
| `GET /api/tenant/members` | Admin, Accounting | なし（テナント内全メンバー名一覧） |
