...
| ローテーション周期 | 90日（四半期ごと） |
| ローテーション方式 | 二鍵並行方式 |
| 手順 | 1. 新しい鍵ペアを生成 2. 新鍵で署名開始 3. 旧公開鍵での検証を猶予期間（7日）継続 4. 猶予期間後に旧鍵を廃棄 |
| 鍵識別 | JWT ヘッダーの `kid`（Key ID）で署名鍵を識別 |
| MVP での運用 | 手動ローテーション。自動化は Phase 3 で検討 |
...
- `security-policy.md` SS3, SS5 の `file_id` は `attachment_id` に読み替え（issue 036）
...
| `style-src` | `'self' 'unsafe-inline'` | Tailwind CSS のインラインスタイルに対応 |
| `img-src` | `'self' data: https://*.amazonaws.com` | 同一オリジン + data URI + S3 署名付き URL からの画像読み込み |
| `connect-src` | `'self'` | API 通信は同一オリジン。S3 ダウンロードはブラウザの直接アクセスではなく API 経由の署名付き URL リダイレクト |
...
| 401 | `UNAUTHORIZED` | "Authentication required" | 認証なし・トークン無効 |
| 401 | `TOKEN_EXPIRED` | "Token has expired" | アクセストークン期限切れ |
| 403 | `FORBIDDEN` | "Insufficient permissions" | ロール不足 |
...
認可の詳細設計は `authz.md`（Phase 3 で作成）に定義する。本セクションではセキュリティ設計としての認可チェックの方針を記載する。

### 9.1 認可チェックの層別責務

| 層 | 責務 | チェック内容 |
|----|------|------------|
| RBAC ミドルウェア | ロールの存在チェック | 「このエンドポイントには指定ロール以上が必要」（RBC-001） |
| ハンドラ層 | 所有権チェック | 「自分のレポートか？」（RBC-010） |
| ドメイン層 | ビジネスルール | 「自己承認ではないか？」（RBC-016）「自己支払処理ではないか？」（RBC-012） |
| リポジトリ層 | テナント分離 | WHERE tenant_id = ? の強制（TNT-002, TNT-003） |
| DB（RLS） | テナント分離の二重保証 | アプリ層が漏れた場合の防御（TNT-004） |

### 9.2 エンドポイント別の認可要件

| エンドポイント | 必要ロール | 追加チェック |
|-------------|-----------|------------|
| `POST /api/auth/signup` | 不要（未認証） | なし |
| `POST /api/auth/login` | 不要（未認証） | なし |
| `POST /api/auth/refresh` | 不要（リフレッシュトークン） | なし |
| `POST /api/auth/logout` | 認証済み（全ロール） | なし |
| `GET /api/auth/me` | 認証済み（全ロール） | なし |
| `POST /api/auth/password-reset` | 不要（未認証） | なし |
| `PUT /api/auth/password-reset/:token` | 不要（リセットトークン） | なし |
| `GET /api/reports` | 認証済み（全ロール） | 自分のレポートのみ |
| `POST /api/reports` | 認証済み（全ロール） | なし |
| `GET /api/reports/:id` | 認証済み（全ロール） | 所有権 or Approver: 同テナント submitted 状態 or Accounting: 同テナント全件 or Admin: 同テナント全件 |
| `PUT /api/reports/:id` | 認証済み（全ロール） | 所有権 + draft 状態 |
| `DELETE /api/reports/:id` | 認証済み（全ロール） | 所有権 + draft 状態 |
| `POST /api/reports/:id/submit` | 認証済み（全ロール） | 所有権 |
| `GET /api/reports/all` | Admin, Accounting | なし（テナント内全件） |
| `POST /api/reports/:id/items` | 認証済み（全ロール） | 所有権 + draft 状態 |
| `PUT /api/reports/:id/items/:itemId` | 認証済み（全ロール） | 所有権 + draft 状態 |
| `DELETE /api/reports/:id/items/:itemId` | 認証済み（全ロール） | 所有権 + draft 状態 |
| `POST /api/reports/:id/items/:itemId/attachments` | 認証済み（全ロール） | 所有権 + draft 状態 |
| `GET /api/reports/:id/items/:itemId/attachments` | 認証済み（全ロール） | 所有権 or 管理/承認/経理権限 |
| `GET /api/reports/:id/items/:itemId/attachments/:attId` | 認証済み（全ロール） | 所有権 or 管理/承認/経理権限 |
| `DELETE /api/reports/:id/items/:itemId/attachments/:attId` | 認証済み（全ロール） | 所有権 + draft 状態 |
| `GET /api/workflow/pending` | Approver | なし |
| `POST /api/workflow/:id/approve` | Approver | 自己承認禁止 |
| `POST /api/workflow/:id/reject` | Approver | 自己承認禁止 |
| `GET /api/workflow/payable` | Accounting | なし |
| `POST /api/workflow/:id/pay` | Accounting | 自己支払処理禁止 |
| `GET /api/dashboard` | 認証済み（全ロール） | ロール別のデータ範囲 |
| `GET /api/tenant` | Admin | なし |
| `GET /health` | 不要（未認証） | なし |
...
| 項目 | 上流の記載 | 本書での対応 |
|------|-----------|------------|
| `security-policy.md` SS3 S3 キー | `{tenant_id}/{report_id}/{file_id}` | `{tenant_id}/{report_id}/{attachment_id}` に読み替え（issue 036） |
...
| `requirements.md` ATT-014 S3 パス | `{tenant_id}/{report_id}/{file_id}` | `{tenant_id}/{report_id}/{attachment_id}` に読み替え（issue 036） |
...
- [x] `security-policy.md` の `file_id` が `attachment_id` に読み替えられているか
