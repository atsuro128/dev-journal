...
[4] RS256 署名検証
  | NG -> 401
  v
[5] exp クレームの検証（有効期限切れチェック。leeway 60 秒を許容）
...
JWT クレームの時刻検証には、クライアント・サーバー間およびサーバー間のクロックずれを吸収するための leeway（許容誤差）を設定する。

| 項目 | 仕様 |
|------|------|
| leeway 値 | 60 秒 |
| 適用範囲 | `exp` / `iat` / `nbf` の全時刻クレーム |
| 対象トークン | アクセストークン / リフレッシュトークン共通 |
| 実装参照 | `internal/domain/auth.go` の `parseToken`（service 経路 = AuthService の TokenVerifier）および `internal/pkg/jwt/jwt.go` の `Verifier.Verify`（middleware 経路 = リクエスト毎の JWT 検証）の両系統で `gojwt.WithLeeway(60 * time.Second)` を全クレーム検証に適用（issue #173 + 11-D 横断レビュー対応漏れ補完） |
| 根拠 | RFC 7519 §4.1.4（leeway の使用を明示的に許可）。Auth0 / AWS Cognito 等の業界実装も既定 60 秒を採用 |

**目的**:

- クライアント・サーバー間のクロックずれを吸収する（モバイル端末、Docker Desktop on WSL2 等の NTP 未同期環境では数秒〜数十秒のドリフトが現実的）
- NTP 同期前後の数秒のずれによる過剰な 401 を防止する
- RFC 7519 §4.1.4 に基づく業界標準的な扱いに合わせる

**セキュリティ影響の評価**:

| クレーム | leeway 適用による影響 | リスク評価 |
|---------|----------------------|-----------|
| `exp` | 期限切れトークンを最大 60 秒だけ余分に使用可能になる | アクセストークン TTL = 15 分の運用において、有効期間の延長率は約 6.7% に留まりリスク影響度は微小 |
| `iat` | サーバー時刻より最大 60 秒未来の `iat` を持つトークンを許容する | トークン発行は自サーバーのみで行うため、攻撃者が「未来発行」トークンを作ることは署名鍵を握らない限り不可能。攻撃面は実質ゼロ |
| `nbf` | `nbf` 到達前のトークンを最大 60 秒早く使用可能になる | 本システムでは `nbf` を発行しないため実質的な影響なし |

リフレッシュトークンについては DB の `is_revoked` チェック（SS2.1「リフレッシュトークンの DB 管理」）により二重防御されており、leeway 経由のリプレイ攻撃も revoke 検知で防止される。
...
| IP 取得 | `X-Forwarded-For` の右から `trusted_proxy_count` 段目の IP（prod=1・CloudFront 1 段経由。lean 構成・ADR-0007 / issue #197）。ヘッダー不在時は `RemoteAddr`（dev は `trusted_proxy_count=0` で `RemoteAddr` を採用）。実装は `remoteIP`（`internal/middleware`） |
...
### 4.5 環境変数による上書き（開発・テスト用途）

未認証 IP ベースのレート制限値（SS4.1 の「未認証リクエスト」「ログイン試行」）は、環境変数で上書き可能とする。本番（AWS デプロイ環境）では env 未設定でデフォルト値が適用され、開発・E2E テスト環境のみ `docker-compose.yml` 等で緩和する運用とする。

| 環境変数 | 対象 | デフォルト値 | 用途 |
|---------|------|------------|------|
| `LOGIN_RATE_LIMIT_PER_MINUTE` | `/api/auth/login` の IP ベースレート制限 | 5 | ログインエンドポイント専用 |
| `UNAUTH_RATE_LIMIT_PER_MINUTE` | 未認証リクエスト全般の IP ベースレート制限 | 20 | 未認証グローバル |

**環境別の運用方針**:

| 環境 | LOGIN_RATE_LIMIT_PER_MINUTE | UNAUTH_RATE_LIMIT_PER_MINUTE | 備考 |
|------|-----------------------------|------------------------------|------|
| dev / E2E | 100（推奨） | 200（推奨） | 複数ロールを連続でログインする E2E テストの実行性確保のため緩和。`docker-compose.yml` の api サービス env で指定 |
| stg / prod | 未設定（デフォルト 5） | 未設定（デフォルト 20） | 本番値は変更しない。env 上書きは開発・テスト用途のみ |

**セキュリティ的観点**:

- 本番（stg/prod）の値は変更しない。env 上書きはブルートフォース対策（SEC-012）の有効性を損なうため、開発・E2E テスト環境に限定する
- 認証済みリクエスト（100 req/min/user）およびファイルアップロード（10 req/min/user）の制限値は env による上書き対象外（実装内ハードコード）
- env_config.md §4.6 に環境変数一覧として記載する
...
| `connect-src` | `'self'` | API 通信は同一オリジン。S3 プレビューは `window.open` で新タブを開く方式（`about:blank` -> `location.href` 差し替え）、S3 ダウンロードは動的生成した `<a download>` 要素のクリック方式であり、いずれも `connect-src` の制約対象外。CSP は `window.open` による新タブ遷移および `<a>` 要素のナビゲーションを制限しない |
...
> lean 構成（issue #197）により ALB を除去し、CloudFront を EC2(EIP):8080 へ直結する。TLS 終端は CloudFront が担う。判断の正本は ADR-0007。

| 項目 | 仕様 |
|------|------|
| TLS 終端 | CloudFront で終端（`*.cloudfront.net` デフォルト証明書）。EC2 との通信は CloudFront〜EC2(EIP):8080 間で HTTP（B-3 受容逸脱・ADR-0007） |
| TLS バージョン | viewer 側は CloudFront のデフォルト証明書により最小 TLSv1（本表「TLS 1.2 以上」の理想値とは B-5 受容逸脱として乖離。ADR-0007）。独自ドメイン+ACM 導入時に TLS 1.2 以上を強制可能 |
| 証明書管理 | CloudFront デフォルト証明書（独自ドメイン・ACM 不要）。独自ドメイン採用時は AWS Certificate Manager (ACM) |
| HTTP リダイレクト | CloudFront の viewer プロトコルポリシー `redirect-to-https` で HTTP を HTTPS へリダイレクト |
| HSTS | `Strict-Transport-Security: max-age=31536000; includeSubDomains` でブラウザに HTTPS を強制 |

### 11.1 オリジン保護

> lean 構成（issue #197）でのオリジン保護方針。判断の正本は ADR-0007「なぜ多層から SG 一枚へ簡素化したか」。

| 項目 | 仕様 |
|------|------|
| オリジン保護 | **SG（CloudFront prefix list 限定）1 層に簡素化**。EC2 SG inbound 8080 を CloudFront マネージドプレフィックスリスト `com.amazonaws.global.cloudfront.origin-facing` 限定にし、CloudFront 経由以外の :8080 到達を遮断 |
| 廃止した保護 | X-Origin-Verify カスタムヘッダ検証（旧 ALB リスナールール）。ALB 除去に伴い廃止し、Go への移設も行わない |
| 安全網 | アプリのレート制限（§4。未認証 20/min・IP / ログイン 5/min・IP / 認証済 100/min・user）がブルートフォース・DoS の安全網 |
| 残存リスク（受容） | 他者が自前 CloudFront を踏み台に prefix list 経由で EC2:8080 へ到達しうるが、ダミーデータのみ・標的型限定・レート制限・ポートフォリオ用途で受容（詳細・将来の再導入余地は ADR-0007） |
