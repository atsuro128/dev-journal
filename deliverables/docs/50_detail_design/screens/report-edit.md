...
SCR-RPT-002（report-create.md）と同一（V1 ~ V5（V5-S/V5-E））。
...
> V5-S / V5-E は同一の業務ルール（RPT-003: `period_start <= period_end`）を 2 経路から表示する設計。開始日・終了日のいずれを編集しても両方のバリデーションが再評価され、違反していれば該当フィールド直下にフィールド主語の文言を即時表示する。Zod の `refine` を 2 つに分割し、`path: ['periodStart']` と `path: ['periodEnd']` で個別エラーを出力する。両フィールドの `onBlur` で `trigger(['periodStart', 'periodEnd'])` を呼び、片側だけの編集でも他方のエラー表示が更新されるようにする。
