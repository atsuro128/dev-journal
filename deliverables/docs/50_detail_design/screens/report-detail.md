...
|          | | 合計金額: \XX,XXX                            |      |
|          | | 作成者: XXXX       作成日: YYYY/MM/DD HH:mm  |      |
|          | | (再申請元表示: 元レポートへのリンク ※該当時)   |      |
|          | |                                              |      |
|          | | --- 承認/却下/支払情報（該当状態の場合のみ）--- |      |
|          | | 承認者: XXXX  承認日: YYYY/MM/DD HH:mm        |      |
|          | | 承認コメント: XXXX                            |      |
|          | | 却下者: XXXX  却下日: YYYY/MM/DD HH:mm        |      |
|          | | 却下理由: XXXX                                |      |
|          | | 支払処理者: XXXX  支払完了日: YYYY/MM/DD HH:mm |      |
|          | +----------------------------------------------+      |
|          |                                                     |
|          | +--- アクションボタンエリア -------------------+      |
|          | | [編集] [提出する] [削除] [再申請]             |      |
...
| # | 項目 | ラベル表記（UI 上の表示文言） | 表示形式 |
|---|------|---------------------------|---------|
| 1 | タイトル | （ラベルなし。見出し `<h2>` として表示） | テキスト（見出し表示） |
| 2 | ステータス | （ラベルなし。バッジで表現） | ステータスバッジ（screens.md 4.8 準拠） |
| 3 | 対象期間 | `対象期間:` | `YYYY/MM/DD ~ YYYY/MM/DD` |
| 4 | 合計金額 | `合計金額:` | `¥` プレフィックス + 3桁カンマ区切り（例: ¥12,500）。明細0件時は `¥0` |
| 5 | 作成者 | `作成者:` | ユーザー名 |
| 6 | 作成日 | `作成日:` | `YYYY/MM/DD HH:mm` |

> **ラベル表示要件**: 上記の項目 #3〜#6（対象期間・合計金額・作成者・作成日）は、値の前に「ラベル表記」列の文字列（末尾コロン `:` 含む）を必ず表示する。値だけを羅列するレイアウトは禁止（issue #144 の差異検出を契機にラベル付き表記を正本化）。条件付き表示項目（#7〜#16）の表記規則とも揃える。
>
> **「作成日」の命名規則**: ワークフロー系日時（提出日・承認日・却下日・支払完了日）と同じく、項目名の末尾は「日」で統一する（時刻まで表示する場合も「作成日時」とは表記しない）。実装側のラベル文言と本表の項目名は一致させること（既存正本のドメインモデル `paid_at` / openapi `paid_at` の慣例に合わせ「支払完了日」を採用）。
...
|---|------|------|-----------|------------|------------|
| 1 | 保存する（追加） | バリデーション通過 AND 期間外判定通過（§「保存時の期間外警告（ITM-007）」参照） | POST /api/reports/:id/items | パネルを閉じる。明細一覧を再読み込み。合計金額を更新。トーストで「明細を追加しました」 | パネル上部にエラー表示 |
| 2 | 保存して続けて追加 | バリデーション通過 AND 期間外判定通過 | POST /api/reports/:id/items | フォームをクリアし追加モードを維持。明細一覧を再読み込み。合計金額を更新。トーストで「明細を追加しました」 | パネル上部にエラー表示 |
| 3 | 保存する（編集） | バリデーション通過 AND 期間外判定通過 | PUT /api/reports/:id/items/:itemId | パネルを閉じる。明細一覧を再読み込み。合計金額を更新。トーストで「明細を更新しました」 | パネル上部にエラー表示 |
...
ポリシー `policies.md` ITM-007（明細日付が対象期間外の場合、警告を表示するが保存は許可する）の実装として、追加モード / 編集モードの「保存する」「保存して続けて追加」押下時に **ConfirmDialog で警告を表示**する。View モード（閲覧時）は保存操作が存在しないため本仕様の対象外（警告表示なし）。

#### 判定条件

| 条件 | 内容 |
|------|------|
| トリガー | ItemForm の「保存する」「保存して続けて追加」ボタン押下時、クライアントサイドバリデーション（V1〜V7）通過後、API 呼び出し前 |
| 期間外判定式 | `expenseDate < report.period_start` OR `expenseDate > report.period_end` |
| 比較形式 | すべて `YYYY-MM-DD` の文字列比較（日付のローカルタイム比較で問題ないことを保証するため、ISO 8601 日付文字列を想定） |
| 対象モード | 追加（add）、編集（edit）のみ。閲覧（view）は対象外 |

#### ConfirmDialog の仕様

| 項目 | 値 |
|------|------|
| 共通コンポーネント | `ConfirmDialog`（`55_ui_component/common-components.md §ConfirmDialog`） |
| タイトル | 「入力内容の確認」 |
| 本文 | 「明細日付がレポートの対象期間外です。入力を確認してください。」（`state-management.md §6.5.6 WARNING_MESSAGES.ITEM_DATE_OUTSIDE_PERIOD_WARNING` を参照） |
| 確認ボタンラベル | 「保存する」 |
| 確認ボタン色 | `primary` |
| キャンセルボタンラベル | 「キャンセル」（デフォルト） |
| 入力フィールド（inputField） | なし |

#### 挙動フロー

| # | 操作 | 挙動 |
|---|------|------|
| 1 | 期間内日付で「保存する」押下 | ConfirmDialog は表示せず、そのまま保存処理（POST / PUT）を実行する |
| 2 | 期間外日付で「保存する」押下 | ConfirmDialog を表示。保存処理は未実行。フォームの入力値は保持 |
| 3 | 上記 2 の後、確認ボタン押下 | ConfirmDialog を閉じ、保存処理（POST / PUT）を実行する |
| 4 | 上記 2 の後、キャンセルボタン押下 / ダイアログ外クリック / ESC キー | ConfirmDialog を閉じ、フォームに戻る。保存処理は実行しない。フォームの入力値は維持 |
| 5 | 「保存して続けて追加」押下（追加モード） | 上記 1〜4 と同じロジック。確認押下後は「保存して続けて追加」の挙動（フォームリセット + 追加モード維持）を実行 |

#### 非対象ケース

- **閲覧モード（view）**: 保存操作が存在しないため ConfirmDialog は表示しない
- **サーバー側でのブロック**: ITM-007 は警告のみで保存は許可するため、バックエンド側では期間外判定によるエラー返却を行わない（フロントエンド責務）
- **日付が未入力 / 不正フォーマット**: V1（日付必須）のバリデーションエラーを優先表示し、期間外判定には進まない

### モード別の保存処理（追加モード時の順次アップロード）

追加モードでは未保存の明細に対して `itemId` が未確定のため、添付ファイルはフォームの**ローカル state に保留**し、「保存する」ボタン押下時に明細作成 → 添付アップロードの順で処理する。編集モードは挙動変更なし（添付は即時アップロード、§7「永続化タイミング」参照）。

#### 追加モードの「保存する」押下時シーケンス

| # | 処理 | API | 備考 |
|---|------|-----|------|
| 1 | 明細作成 | POST `/api/reports/:id/items` | レスポンスから `itemId` を取得 |
| 2 | 保留中の添付を順次アップロード | 取得した `itemId` に対して POST `/api/reports/:id/items/:itemId/attachments` を保留ファイル件数分ループ呼び出し | 1 件ずつ順次実行。並列実行はしない（レート制限 10 req/min/user との整合、files.md §9） |
| 3a | 全成功 | - | パネルを閉じる。明細一覧・合計金額を再読み込み。トーストで「明細を追加しました」 |
| 3b | 部分失敗（1 件以上の添付が失敗） | - | 明細は作成済みのまま維持（ロールバックしない）。まず失敗添付の警告トースト「N 件の添付ファイルがアップロードに失敗しました。再試行してください」を表示し、その直後にパネルを閉じて明細一覧を再読み込みする（トーストはパネルクローズ後の画面上で視認できるよう、画面右上等の Snackbar 領域に表示し続ける）。ユーザーは作成された明細を編集モードで開き直し、失敗ファイルを再アップロード可能 |

#### 順次アップロード中の UI 表示

| 項目 | 内容 |
|------|------|
| ボタン表示 | 「保存する」ボタンを disabled + スピナー、ラベルを「アップロード中... (N/M 件完了)」に切り替える（N: 完了件数、M: 保留ファイル総数） |
| フォーム編集 | 明細作成後の順次アップロード中はフォームフィールドを readonly とする（二重送信・整合性崩れ防止） |
| 閉じる操作 | 「キャンセル」「× ボタン」「明細外クリック」は §7「アップロード・削除中の並行操作制御」の「パネル閉じ時のアップロード中断」と同じ方針を準用する。すなわち、保留中の順次アップロードを AbortController で中断し、「アップロードを中止しました」の警告トーストを表示する（明細自体は作成済みなのでロールバックしない。未アップロード分は編集モードで再アップロード可能） |

#### 部分失敗時のロールバック方針

- 作成済みの明細は**削除しない**（既存の即時アップロード方式と同じ挙動に揃える）
- 失敗した添付ファイルはユーザーに警告トーストで通知し、編集モードで再アップロードできる旨を示唆する
- 明細ロールバックを行わない理由: 明細自体は正常に作成されており、添付のみの再試行で UX を維持できるため。ロールバック方式は DELETE API 呼び出しの失敗時に DB 不整合を招くリスクがある

#### 編集モードとの差異

| 観点 | 追加モード | 編集モード |
|------|----------|----------|
| 添付の保持方法 | 選択時点でローカル state に保留 | 即時 S3 + DB 保存 |
| 添付の確定タイミング | 「保存する」押下後の順次アップロード | ファイル選択時点（フォーム保存は不要） |
| 「保存する」押下時の API 呼び出し | POST 明細 → POST 添付（N 件） | PUT 明細のみ |

### 編集中の破棄確認ダイアログ

ItemForm（明細編集フォーム）のフィールド編集内容が未保存のままパネルを閉じようとした場合、誤操作による破棄を防ぐための確認ダイアログを表示する。

#### dirty 判定対象

| 区分 | 対象 | 備考 |
|------|------|------|
| dirty 判定対象 | ItemForm のフィールド変更（日付・金額・カテゴリ・摘要） | フォーム内の入力値が初期値と差異がある状態を dirty とする |
| dirty 判定対象（追加モードのみ） | 保留中の添付ファイルが 1 件以上ある状態 | 追加モードでは添付はローカル state に保留されており、パネルを閉じると保留中のファイル選択がすべて失われるため dirty 扱いとする（§6「モード別の保存処理」と整合） |
| dirty 判定対象外 | 編集モードでの添付ファイルの追加・削除 | 編集モードは即時保存方式のため Form の dirty とは独立（§7「永続化タイミング」と整合） |

補足: 追加モードの dirty 判定は「フィールド変更が 1 件以上」OR「保留中添付が 1 件以上」で成立する。編集モードの dirty 判定は従来通り「フィールド変更が 1 件以上」のみで成立し、添付操作は dirty 判定に含めない。

#### 閉じパターン別の挙動

| # | 閉じパターン | dirty 時の挙動 | クリーン時の挙動 |
|---|------------|--------------|---------------|
| 1 | パネル右上の × ボタン | MUI Dialog を表示（「変更を破棄しますか？」+ 「破棄」「キャンセル」ボタン） | パネルを即時クローズ |
| 2 | キャンセルボタン | 同上（MUI Dialog） | パネルを即時クローズ |
| 3 | 明細外クリック（Drawer の onClose） | 同上（MUI Dialog） | パネルを即時クローズ |
| 4 | F5 リロード / タブ閉じ / ブラウザ閉じ | `beforeunload` によるブラウザ標準ダイアログ（dirty 時のみイベント登録） | 何も表示しない |

#### 破棄確認ダイアログの仕様（MUI Dialog）

| 項目 | 内容 |
|------|------|
| タイトル | 「変更を破棄しますか？」 |
| 本文 | 「編集内容は保存されていません。破棄するとこれまでの変更が失われます。」 |
| 確認ボタン | 「破棄」（危険スタイル）→ パネルを閉じ、編集内容を破棄 |
| キャンセルボタン | 「キャンセル」→ パネルは閉じず、編集内容を保持 |

#### F5 / タブ閉じ時の技術制約（補足）

F5・タブ閉じ・ブラウザ閉じのケースはブラウザ標準ダイアログに依存する。以下の技術制約を前提とする。

- `beforeunload` のカスタム文言表示は現代ブラウザでは不可（Chrome 51 / Firefox 4 / Safari 9.1 以降で廃止。セキュリティ上の乱用対策）。表示有無のみ `event.preventDefault()` で制御可能
- `beforeunload` 中の MUI Dialog / alert 表示は不可（仕様上ブロックされる）
- keydown F5 / Ctrl+R 傍受による独自ダイアログは、ツールバーの再読み込みやタブ閉じを拾えず網羅性が欠けるため不採用
- SPA 内遷移のブロックは React Router の仕組みで対応可能だが、本画面のスライドパネルは画面遷移を伴わないため対象外

これらの制約により、F5・タブ閉じ等のハード離脱についてはブラウザ標準ダイアログで最低限の保護を行う方針とする。
...
**UI 案内表示**: 挙動の案内文は表示しない（issue 108 追加対応、2026-04-20）。当初は AttachmentArea に常時表示する方針だったが、UI ノイズとなるため撤去した。即時保存仕様自体は本節および `files.md` §3 で維持する。削除確認ダイアログ（「元に戻せません」文言）でユーザーへの注意喚起は担保する。

**設計判断**: 添付の永続化を明細フォーム保存ボタンと独立させる方式（即時保存）は、保留 → 確定方式（DB スキーマ改修が必要）とロールバック方式（ネットワーク断時の S3 孤児ファイルリスク、既存添付削除のロールバック UI 状態管理複雑化）を比較した上で MVP 方針として採用済み。

### モード別の添付挙動（追加モードはローカル保持方式）

追加モードでは未保存の明細に対して `itemId` が未確定であるため、添付は即時 API 呼び出しせず**フロントのローカル state に保留**し、明細フォームの「保存する」押下後に順次アップロードする（順次アップロードの詳細は §6「モード別の保存処理」参照）。編集モードは上記「永続化タイミング（即時保存方式）」を維持する。

#### モード別サマリ

| モード | 添付選択時 | 添付確定タイミング | AttachmentArea 表示 |
|--------|----------|-----------------|-----------------|
| 追加（新規明細） | ローカル state に保留 | 「保存する」ボタン押下後の順次アップロード完了時 | 表示 |
| 編集（既存明細） | 即時 S3 + DB 保存 | 選択時点（即時、issue 114 / §7「永続化タイミング」） | 表示 |
| 閲覧 | - | - | 表示（操作不可） |
...
| 観点 | 挙動 |
|------|------|
| ファイル選択 | `File` オブジェクトをフロントの state に保持。API 呼び出しは行わない |
| 一覧表示 | 保留中のファイルを編集モードと**同一の UI 構造**で表示する。具体的には `<ul><li>` 構造のリストでファイル名（`<Button variant="text">`）・ファイルサイズ（`<span>`）・削除ボタン（`<Button variant="text" color="error">削除</Button>`、テキストボタン）をレンダリングし、編集モードの `AttachmentList` のマークアップと一致させる。「保留中」「保存後にアップロード予定」等の実装詳細を示すラベル / バッジは表示しない（issue #129 / #143、案 A: 実装詳細を UI に露出させない方針） |
| プレビュー | ファイル名クリックで `URL.createObjectURL(file)` で生成したブラウザ内プレビュー URL を新タブで開く（編集モードの署名付き URL プレビューと同等 UX。issue #129） |
| ダウンロード | ダウンロードアイコン（編集モードの `<IconButton>↓</IconButton>`）は **add モードでのみ非表示**。pending file はサーバー未保存のため署名付き URL が存在せず、ダウンロード操作が成立しないため（issue #129 / #143）。それ以外の要素（ファイル名・サイズ・削除ボタン）は編集モードと同一構造で表示する |
| クライアント側バリデーション | MIME タイプ（JPEG / PNG / PDF）・サイズ（5MB 以下）は選択時点でチェックし、違反時はファイル一覧に追加しない + エラー表示（§7「アップロードのバリデーション」の V1 / V2 と同一ルール） |
| サーバー側 MIME 検証 | 「保存する」押下後の POST 時点で実施（従来通り、V3 / ATT-013） |
| 保留中添付の削除 | 保留中のファイルに編集モードと同一の `<Button variant="text" color="error">削除</Button>`（テキストボタン）を表示する。押下でローカル state から除去（API 呼び出しなし、確認ダイアログなし）。削除時に `URL.revokeObjectURL` でプレビュー URL を解放する。削除完了時のトースト文言は編集モードと完全同一の「添付ファイルを削除しました」を表示する（issue #143、案 A: ローカル保留方式という実装詳細を UI 文言に露出させない） |
| アップロード成功時のトースト | ファイル選択 → クライアントサイドバリデーション通過 → ローカル state への保留が完了した時点で、編集モードと完全同一の「ファイルをアップロードしました」トーストを発火する。フロント上ではアップロード操作が完了している（ユーザー視点でアップロード成功）ため文言の不整合は生じない（issue #143、案 A） |
| メモリ管理 | `URL.createObjectURL` で生成した Blob URL は unmount 時・ファイル削除時・保存成功時に `URL.revokeObjectURL` で解放する（メモリリーク防止） |

#### 部分失敗時の挙動（追加モード）

順次アップロード中に 1 件以上の添付がアップロード失敗した場合の扱いは §6「モード別の保存処理 → 部分失敗時のロールバック方針」を参照。要点のみ再掲する。

- 明細は作成済みのまま維持（ロールバックしない）
- 失敗添付の警告トースト「N 件の添付ファイルがアップロードに失敗しました。再試行してください」を表示
- ユーザーは作成された明細を編集モードで開き直し、失敗ファイルを通常の即時アップロード方式で再アップロードできる
- 既存の編集モード（即時アップロード）と整合した復旧導線となる

### 添付ファイル一覧

#### アップロード済みファイル（編集・閲覧モード）

リスト全体は `<ul><li>` 構造でレンダリングする。

| # | 項目 | 表示形式 |
|---|------|---------|
| 1 | ファイル名 | `<Button variant="text">`。クリックでプレビュー（サーバー署名付き URL で新タブ） |
| 2 | ↓ アイコン | `<IconButton>↓</IconButton>` ダウンロードボタン。クリックでダウンロード |
| 3 | ファイルサイズ | `<span>` で `XX KB` / `X.X MB` |
| 4 | 削除ボタン | `<Button variant="text" color="error">削除</Button>`（テキストボタン）。status == draft かつ所有者の場合のみ表示 |

#### 保留中ファイル（追加モード・ローカル state のみ）（issue #129 / #143）

UI 構造は**編集モードの「アップロード済みファイル」と完全同一**にし、唯一の差分は「ダウンロードアイコンの有無」のみとする。リストは `<ul><li>` 構造、ファイル名は `<Button variant="text">`、サイズは `<span>`、削除ボタンは `<Button variant="text" color="error">削除</Button>`（テキストボタン）でレンダリングする。実装詳細を示すラベル（例: 「保留中」「保存後にアップロード予定」等）は表示しない（issue #143、案 A）。

| # | 項目 | 表示形式 | 編集モードとの差分 |
|---|------|---------|-------------------|
| 1 | ファイル名 | `<Button variant="text">`。クリックで `URL.createObjectURL` のブラウザ内プレビュー URL を新タブで開く | 構造は同一（クリック先 URL の取得元のみ差異: 編集モードはサーバー署名付き URL、追加モードは Blob URL） |
| 2 | ↓ アイコン | **add モードのみ非表示**（pending file = サーバー未保存のため署名付き URL が存在しない） | **add モードのみ削除**（編集モードでは表示） |
| 3 | ファイルサイズ | `<span>` で `XX KB` / `X.X MB` | 構造は同一 |
| 4 | 削除ボタン | `<Button variant="text" color="error">削除</Button>`（テキストボタン）。押下でローカル state から除去（確認ダイアログなし・API 呼び出しなし）。削除完了時のトーストは編集モードと共通の「添付ファイルを削除しました」 | 構造は同一（押下時の挙動のみ差異: 編集モードは確認ダイアログ + DELETE API、追加モードはローカル state からの除去のみ。ユーザー視点のトースト文言は同一） |
...
> **add / edit 両モード共通**（issue #143、案 A）: 上記「ファイルをアップロードしました」トーストは編集モード（即時 S3 + DB 保存の成功時）・追加モード（ローカル state への保留完了時）の両方で発火する。文言は完全同一（モード別の文言出し分けはしない）。追加モードでローカル保留方式を採用していることを UI 文言に露出させないための統一ルール。
...
| メッセージ | 確認ボタン | 対応UC | 適用モード |
|-----------|-----------|--------|----------|
| 「この添付ファイルを削除しますか?」 | 「削除する」 | UC-M03a | 編集モードのみ（DELETE API 呼び出しを伴うため） |

- 削除成功時: ファイル一覧から削除。トーストで「添付ファイルを削除しました」

> **add / edit 両モード共通のトースト文言**（issue #143、案 A）: 上記「添付ファイルを削除しました」トーストは編集モード（DELETE API 成功時）・追加モード（ローカル state からの pending file 除去時）の両方で発火する。文言は完全同一。追加モードでは確認ダイアログを挟まず即時にローカル state から除去するが、ユーザー視点の完了通知文言は編集モードと揃えてローカル保留方式という実装詳細を UI に露出させない。

### アップロード・削除中の並行操作制御

添付ファイルのアップロード / 削除が進行中に、明細フォームの保存やパネルクローズ等の並行操作が行われた場合のトースト座礁・反映取りこぼしを防ぐため、以下の制御を行う。
...
| 項目 | 内容 |
|------|------|
| 中断手段 | `useUploadAttachment` / `useDeleteAttachment` hook に AbortController を組み込む。コンポーネント unmount 時（パネルクローズ時）に進行中の mutation を cancel する。別明細を開いた場合もパネルが再マウントされ、進行中の mutation は cancel される |
| 中断トースト | mutation のエラーハンドリング経由で「アップロードを中止しました」（削除時は「削除を中止しました」）のトーストを表示する |
| 中断時の反映 | S3 / DB への書き込みが完了していない場合は添付一覧への反映は行わない。中断前に通信が完了していた場合は次回パネルオープン時の添付一覧再読み込みで反映される |

#### 削除 mutation への適用

`useDeleteAttachment` も `useUploadAttachment` と同パターンを適用する（状態伝播・AbortController・中断トースト）。削除処理中も保存ボタンは disabled となる。
...
| チェック | エラー時の挙動 |
|---------|-------------|
| レポートが存在しない | メインコンテンツ領域に EmptyState で「指定されたデータが見つかりません。」と「レポート一覧」リンクを表示（§11 EmptyState パターン準拠） |
...
> **実装者注記（issue #134）**: 下表の「表示方法」欄に記載されている日本語文言は `frontend/src/lib/error-messages.ts` の `SERVER_ERROR_MESSAGES` の期待値であり、コンポーネント内でハードコードするものではない。onError ハンドラでは `err.message` をそのまま使うこと（`.claude/rules/implementation-workflow.md` 「FE エラーハンドリング」参照）。
...
sequenceDiagram
    ...
    F->>F: クライアントバリデーション（V1〜V7）実行
    F->>F: 期間外判定（ITM-007）<br/>expense_date が report.period_start〜period_end の範囲外か?
    alt 期間外
        F->>F: ConfirmDialog を表示<br/>（「明細日付がレポートの対象期間外です。入力を確認してください。」）
        alt キャンセル押下
            F->>F: ConfirmDialog を閉じ、フォームに戻る（入力値保持）<br/>※ここで処理中断
        end
        F->>F: 確認ボタン押下 → ConfirmDialog を閉じ、保存処理続行
    end
