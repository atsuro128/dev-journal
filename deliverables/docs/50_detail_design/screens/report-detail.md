...
> **ラベル表示要件**: 上記の項目 #3〜#6（対象期間・合計金額・作成者・作成日）は、値の前に「ラベル表記」列の文字列（末尾コロン `:` 含む）を必ず表示する。値だけを羅列するレイアウトは禁止。条件付き表示項目（#7〜#16）の表記規則とも揃える。
>
...
| D4（却下） | 却下理由 | 空でないこと | 「却下理由を入力してください」（onBlur で touched 状態を立て、`!value.trim()` のとき helperText に赤字表示。確認ボタンの disabled 併用も可。誤送信防止の二段構え。issue #159 対応） | WFL-012 |
...
**UI 案内表示**: 挙動の案内文は表示しない。当初は AttachmentArea に常時表示する方針だったが、UI ノイズとなるため撤去した。即時保存仕様自体は本節および `files.md` §3 で維持する。削除確認ダイアログ（「元に戻せません」文言）でユーザーへの注意喚起は担保する。
...
| 編集（既存明細） | 即時 S3 + DB 保存 | 選択時点（即時、§7「永続化タイミング」） | 表示 |
| 閲覧 | - | - | 表示（操作不可） |
...
| 一覧表示 | 保留中のファイルを編集モードと**同一の UI 構造**で表示する。具体的には `<ul><li>` 構造のリストでファイル名（`<Button variant="text">`）・ファイルサイズ（`<span>`）・削除ボタン（`<Button variant="text" color="error">削除</Button>`、テキストボタン）をレンダリングし、編集モードの `AttachmentList` のマークアップと一致させる。「保留中」「保存後にアップロード予定」等の実装詳細を示すラベル / バッジは表示しない（実装詳細を UI に露出させない方針） |
| プレビュー | ファイル名クリックで `URL.createObjectURL(file)` で生成したブラウザ内プレビュー URL を新タブで開く（編集モードの署名付き URL プレビューと同等 UX） |
| ダウンロード | ダウンロードアイコン（編集モードの `<IconButton>↓</IconButton>`）は **add モードでのみ非表示**。pending file はサーバー未保存のため署名付き URL が存在せず、ダウンロード操作が成立しないため。それ以外の要素（ファイル名・サイズ・削除ボタン）は編集モードと同一構造で表示する |
...
| 保留中添付の削除 | 保留中のファイルに編集モードと同一の `<Button variant="text" color="error">削除</Button>`（テキストボタン）を表示する。押下でローカル state から除去（API 呼び出しなし、確認ダイアログなし）。削除時に `URL.revokeObjectURL` でプレビュー URL を解放する。削除完了時のトースト文言は編集モードと完全同一の「添付ファイルを削除しました」を表示する（ローカル保留方式という実装詳細を UI 文言に露出させない） |
| アップロード成功時のトースト | ファイル選択 → クライアントサイドバリデーション通過 → ローカル state への保留が完了した時点で、編集モードと完全同一の「ファイルをアップロードしました」トーストを発火する。フロント上ではアップロード操作が完了している（ユーザー視点でアップロード成功）ため文言の不整合は生じない |
...
#### 保留中ファイル（追加モード・ローカル state のみ）

UI 構造は**編集モードの「アップロード済みファイル」と完全同一**にし、唯一の差分は「ダウンロードアイコンの有無」のみとする。リストは `<ul><li>` 構造、ファイル名は `<Button variant="text">`、サイズは `<span>`、削除ボタンは `<Button variant="text" color="error">削除</Button>`（テキストボタン）でレンダリングする。実装詳細を示すラベル（例: 「保留中」「保存後にアップロード予定」等）は表示しない。
...
> **add / edit 両モード共通**: 上記「ファイルをアップロードしました」トーストは編集モード（即時 S3 + DB 保存の成功時）・追加モード（ローカル state への保留完了時）の両方で発火する。文言は完全同一（モード別の文言出し分けはしない）。追加モードでローカル保留方式を採用していることを UI 文言に露出させないための統一ルール。
