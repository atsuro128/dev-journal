...
| 一覧表示 | 保留中のファイルを通常の添付一覧と同じ UI で「保存後にアップロード予定」ラベル付きで表示 |
...
| 保留中添付の削除 | 保留中のファイルに「× 削除」ボタンを表示。押下でローカル state から除去（API 呼び出しなし、確認ダイアログなし） |
| ダウンロード / プレビュー | 保留中（未アップロード）のため表示しない。アップロード完了後に通常の添付として操作可能 |
