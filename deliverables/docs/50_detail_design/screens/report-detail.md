...
| 一覧表示 | 保留中のファイルを編集モードと同一の UI で表示（ファイル名・ファイルサイズ）。ラベル付きでは表示しない（issue #129） |
| プレビュー | ファイル名クリックで `URL.createObjectURL(file)` で生成したブラウザ内プレビュー URL を新タブで開く（編集モードの署名付き URL プレビューと同等 UX。issue #129） |
| ダウンロード | ダウンロードアイコンは非表示。保留中（未アップロード）のためサーバー署名付き URL が存在しないため（issue #129） |
...
| 保留中添付の削除 | 保留中のファイルに「× 削除」ボタンを表示。押下でローカル state から除去（API 呼び出しなし、確認ダイアログなし）。削除時に `URL.revokeObjectURL` でプレビュー URL を解放する |
...
| # | 項目 | 表示形式 |
|---|------|---------|
| 1 | ファイル名 | テキスト。クリックでプレビュー（サーバー署名付き URL で新タブ） |
| 2 | ↓ アイコン | ダウンロードボタン。クリックでダウンロード |
| 3 | ファイルサイズ | `XX KB` / `X.X MB` |
| 4 | 削除ボタン | [x] アイコン。status == draft かつ所有者の場合のみ表示 |

#### 保留中ファイル（追加モード・ローカル state のみ）（issue #129）

| # | 項目 | 表示形式 |
|---|------|---------|
| 1 | ファイル名 | テキスト。クリックで `URL.createObjectURL` のブラウザ内プレビュー URL を新タブで開く |
| 2 | ↓ アイコン | **非表示**（保留中はサーバー未保存のためダウンロード操作なし） |
| 3 | ファイルサイズ | `XX KB` / `X.X MB` |
| 4 | 削除ボタン | × ボタン。押下でローカル state から除去（確認ダイアログなし・API 呼び出しなし） |
