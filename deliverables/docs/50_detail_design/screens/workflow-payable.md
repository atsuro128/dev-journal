...
件数表示は §7 ページネーションのフッター左「{start} - {end} / 全 {total} 件」に統合された（issue #147 再々オープン A2 案）。テーブル上部の独立した件数表示は提供しない（4 画面で表示パターンを統一、業務文脈はページタイトル「支払待ち一覧」で明示）。フィルタ適用中の 0 件メッセージは §9 空状態を参照。
