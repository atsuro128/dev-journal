...
- **Grid 配置**: PC 幅（md ≥ 900px）で 5 等分（`md:2.4`、`12 / 5 = 2.4`）、タブレット幅（sm 600〜899px）で 2 列折返し（`sm:6`）、モバイル（xs < 600px）で縦積み（`xs:12`）。`md: 'auto'` のような自然幅指定は使用しない（コンテンツ依存で幅が縮み、他ロールカードと不均等になるため）
...
> Grid 配置の詳細は §4.4（5 等分: `md:2.4`）/ §4.5（1/3 幅: `sm:4`）を参照。
