...
以下のフローチャートは、ログインリクエスト受信からレスポンス返却までの分岐を示す。SEC-011 に基づき、「ユーザー不存在」と「パスワード不一致」は同一のエラーレスポンスに合流する。
...
graph TD
    ...
    C -->|No| E2[401 認証失敗<br/>SEC-011: 存在を推測させない]
...
> **注記**: ノード E2 に「ユーザー不存在」(C の No) と「パスワード不一致」(D の No) の両方のエラーパスが合流している。これは SEC-011 の要件（認証失敗時のレスポンスでユーザーの存在を推測させない）を設計レベルで保証するものである。クライアントにはいずれの場合も「メールアドレスまたはパスワードが正しくありません」という同一メッセージが返却される。
