...
ロールは Admin・Approver・Member・Accounting の 4 種類。Member は全レポートフィクスチャの作成者である Test Member（`report_draft_empty` を含むテナントA レポートを所有）と、空状態表示確認用のレポート 0 件ユーザー Test Member Empty（SMK-084 用）の 2 名、Approver は SMK-105（同テナント内の別 Approver 検証）用に第二 Approver を加えた 2 名で、合計 6 ユーザー。
...
`s3_key` は前者が `{TenantAID}/{ReportSubmittedID}/{AttachmentSubmittedID}`、後者が `{TenantAID}/{ReportDraftID}/{AttachmentDraftID}`。投入は `ON CONFLICT (attachment_id) DO NOTHING` による冪等投入のため、未投入環境には添付レコードを追加する。ただし SMK-038 で論理削除（`deleted_at` 設定）済みの同一 `attachment_id` は、seed 再実行では `deleted_at` を戻す UPDATE を行わないため復元されない。削除前状態に戻すには DB のクリーンアップ（該当行の物理削除または `deleted_at` リセット）後に seed を再実行する。
...
**補足**: ローカルテスト実行結果は PR body の Local CI セクションに記載し、手動確認で担保する。
