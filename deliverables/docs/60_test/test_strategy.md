...
|------|------|
| テストレベル | 統合テスト（軽量スモーク） |
| 実施タイミング | main ブランチへのマージ後（E2E テストと同タイミング、または nightly） |
...
**備考**: MVP では本格的な負荷テストは行わない。CI での軽量スモークテストで閾値超過を検知するレベルとする。
...
| `report_paid` | `paid` | `cccccccc-0005-0005-0005-000000000005` | テスト支払済みレポート | 終端状態テスト用 |
...
| タイミング | 実行内容 | 目的 |
|----------|---------|------|
| **PR 作成・更新時** | 単体テスト + 統合テスト（レート制限テストを含む） | 変更の安全性を素早く確認 |
| **main ブランチへのマージ後** | E2E テスト + レスポンスタイム スモークテスト | 主要フローの回帰保証 + 非機能閾値の監視 |

**補足**: パイプライン YAML の具体設計は Step 7（実装・運用）で行う。本書では方針のみ定義する。
...
本節は Step 7 の実装者向けの補足ガイドである。詳細な実装手順は Step 7 で定義する。
