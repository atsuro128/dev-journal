...
| TNT-FE-054 | 単体 | AllReportsFilterBar | filters: AllReportsFilterValues, onFilterChange, members, membersLoading | 正常系 | ADM-F01 | 55_ui_component/screens/admin-all-reports.md §AllReportsFilterBar | `test_AllReportsFilterBar_container_and_four_filters_present` | `filters = defaultFilters`、`members = [{ id: "u1", name: "User1" }, { id: "u2", name: "User2" }]`、`membersLoading = false` | `data-testid="all-reports-filter-bar"` のコンテナが描画され、ステータス・開始日・終了日・申請者の 4 つのフィルタ要素が含まれること（flex-wrap レイアウト検証） |
