...
| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| DSH-FE-003b | 単体 | DashboardPage | useCurrentUser (role: approver) | 正常系 | DASH-F01, DASH-002 | 55_ui_component/screens/dashboard.md §DashboardPage | `test_DashboardPage_Approver_ActionCardsGrid` | useCurrentUser が `{ role: 'approver' }` を返す。useDashboard が mockDashboardApprover を返す | Approver の承認待ちカードが Grid レイアウトコンテナ（data-testid="approver-cards-row"）内に配置される。MyReportCountCards 3 枚（下書き / 提出中 / 却下）も同 Grid 内に含まれること |
| DSH-FE-004b | 単体 | DashboardPage | useCurrentUser (role: accounting) | 正常系 | DASH-F01, DASH-003 | 55_ui_component/screens/dashboard.md §DashboardPage | `test_DashboardPage_Accounting_ActionCardsGrid` | useCurrentUser が `{ role: 'accounting' }` を返す。useDashboard が mockDashboardAccounting を返す | Accounting の支払待ちカードが Grid レイアウトコンテナ（data-testid="accounting-cards-row"）内に配置される。4 枚カード（MyReportCountCards 3 枚 + ActionCards 1 枚）が単一 Grid container 内で 3+1 折り返しレイアウトとなること |
...
| 中 | DSH-FE-002, DSH-FE-009, DSH-FE-011, DSH-FE-017, DSH-FE-018, DSH-FE-019, DSH-FE-022, DSH-FE-023, DSH-FE-029, DSH-FE-030, DSH-FE-035, DSH-FE-037 |
| 低（ベストエフォート） | DSH-FE-010, DSH-FE-014, DSH-FE-015, DSH-FE-020, DSH-FE-021, DSH-FE-024, DSH-FE-025, DSH-FE-026, DSH-FE-031, DSH-FE-032, DSH-FE-033, DSH-FE-036, DSH-FE-038 |
