...
**備考**: DashboardPage 側のリダイレクトトースト表示テスト（issue-088 対応）は、元の DSH-FE-008 と ID が重複するため DSH-FE-039 に振り直した。CountCard 側の DSH-FE-008（本行）はそのまま維持する。
...
### 追記テスト（issue-088 / レイアウト検証）

以下のテスト ID は重複解消（issue-088）またはテスト実装との整合のために追記する。

#### DashboardPage -- リダイレクトトースト表示（旧 DSH-FE-008 → 振り直し）

| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| DSH-FE-039 | 単体 | DashboardPage | location.state.toast | 正常系 | DASH-F01 | 55_ui_component/screens/dashboard.md §DashboardPage | `test_DashboardPage_ShowsRedirectToast` | location state にトースト情報 `{ toast: { severity: "error", message: "この画面にアクセスする権限がありません。" } }` が含まれている | AppToast が表示され、トーストメッセージ「この画面にアクセスする権限がありません。」が描画される（issue-088 対応） |

**備考**: DSH-FE-039 は旧 DSH-FE-008（DashboardPage 側）を振り直した ID。CountCard 側の DSH-FE-008 はそのまま維持する。

#### DashboardPage -- Grid レイアウト検証

| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| DSH-FE-003b | 単体 | DashboardPage | useCurrentUser (role: approver) | 正常系 | DASH-F01, DASH-002 | 55_ui_component/screens/dashboard.md §DashboardPage | `test_DashboardPage_Approver_ActionCardsGrid` | useCurrentUser が `{ role: 'approver' }` を返す。useDashboard が mockDashboardApprover を返す | Approver の承認待ちカードが Grid レイアウトコンテナ（approver-action-cards）内に配置される |
| DSH-FE-004b | 単体 | DashboardPage | useCurrentUser (role: accounting) | 正常系 | DASH-F01, DASH-003 | 55_ui_component/screens/dashboard.md §DashboardPage | `test_DashboardPage_Accounting_ActionCardsGrid` | useCurrentUser が `{ role: 'accounting' }` を返す。useDashboard が mockDashboardAccounting を返す | Accounting の支払待ちカードが Grid レイアウトコンテナ（accounting-action-cards）内に配置される |

#### CountCard -- Badge 配置検証

| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| DSH-FE-012b | 単体 | CountCard | showBadge: boolean | 正常系 | DASH-F01 | 55_ui_component/screens/dashboard.md §CountCard | `test_CountCard_ShowBadge_AnchorOrigin` | `<CountCard label="承認待ち" count={3} showBadge={true} />` | showBadge=true かつ count >= 1 のとき Badge の anchorOrigin が top/right である |
| DSH-FE-012c | 単体 | CountCard | showBadge: boolean | 正常系 | DASH-F01 | 55_ui_component/screens/dashboard.md §CountCard | `test_CountCard_ShowBadge_AriaLabel` | `<CountCard label="承認待ち" count={3} showBadge={true} />` | showBadge=true かつ count >= 1 のとき aria-label="要対応あり" が付与される |
