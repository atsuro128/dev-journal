...
「各フィールド未入力の場合、フォーカスアウト後に日本語エラーをリアルタイム表示」の方針に対し、金額フィールド（AppInput）の onBlur トリガによるエラー表示を検証する。既存の `ITM-FE-029`（保存ボタン押下時の金額未入力エラー）と対になるテストで、RHF `mode: 'onBlur'` において金額を空のままフォーカスアウトした際にリアルタイムでエラーメッセージが表示されることを保証する（V2 フォーカスアウト/保存時の onBlur タイミング検証）。
