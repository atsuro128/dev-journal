...
| V2 | 金額は空でないこと | ITM-FE-029, ITM-FE-108 |
      ...
      pages/
        reports/
          __tests__/
            ...
            ItemForm.test.tsx          -- ITM-FE-026 ~ 047, ITM-FE-099 ~ 106, ITM-FE-108
      hooks/
        __tests__/
...
#### ItemForm -- 金額空 onBlur 日本語エラー表示（V2）

issue 118 の完了条件「各フィールド未入力の場合、フォーカスアウト後に日本語エラーをリアルタイム表示」のうち、金額フィールド（AppInput）の onBlur トリガによるエラー表示を検証する。既存の `ITM-FE-029`（保存ボタン押下時の金額未入力エラー）と対になるテストで、RHF `mode: 'onBlur'` において金額を空のままフォーカスアウトした際にリアルタイムでエラーメッセージが表示されることを保証する（V2 フォーカスアウト/保存時の onBlur タイミング検証）。

| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| ITM-FE-108 | 単体 | ItemForm | control: Control（RHF、mode: 'onBlur'） | 異常系（V2） | ITM-002 | 55_ui_component/screens/report-detail.md §ItemForm | `shows_validation_error_when_amount_empty_on_blur` | mode='add', categories=mockCategories。金額フィールドを空のままフォーカスアウトする | RHF の onBlur バリデーションが発火し、金額 AppInput 直下に「金額を入力してください」の日本語エラーメッセージが表示される（保存ボタン押下前にリアルタイム表示される点が ITM-FE-029 との差分） |
...
- 他フィールド（日付・摘要）の onBlur トリガ相当テストは本スコープ外（日付は AppDatePicker の ADT-004 / ADT-005 でコンポーネント側を、摘要はネイティブ MUI TextField の onBlur を使用するため、issue 118 の追加設計は不要）。必要になった時点で別途採番する。
