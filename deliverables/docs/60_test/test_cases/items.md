...
| V5 | カテゴリは選択されていること | ITM-FE-033, ITM-FE-107 |
      ...
      pages/
        reports/
          __tests__/
            ...
            ItemForm.test.tsx          -- ITM-FE-026 ~ 047, ITM-FE-099 ~ 107
      hooks/
        __tests__/
...
#### ItemForm -- カテゴリ onBlur 日本語エラー表示（issue 118 スコープ拡張）

issue 118 の完了条件「各フィールド未入力の場合、フォーカスアウト後に日本語エラーをリアルタイム表示」のうち、カテゴリ（AppSelect）の onBlur トリガによるエラー表示を検証する。既存の `ITM-FE-033`（保存ボタン押下時のカテゴリ未選択エラー）と対になるテストで、RHF `mode: 'onBlur'` において `Controller` の `field.onBlur` が `AppSelect` の `onBlur` prop に伝播していることを保証する。

**前提**: `AppSelect` が `onBlur?: () => void` Props をサポートすること（`55_ui_component/common-components.md §AppSelect` 参照、`ASL-001` / `ASL-002` でコンポーネント単体の契約を保証）。

| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| ITM-FE-107 | 単体 | ItemForm | control: Control（RHF、mode: 'onBlur'） | 異常系（V5） | ITM-003 | 55_ui_component/screens/report-detail.md §ItemForm, 55_ui_component/common-components.md §AppSelect | `shows_validation_error_when_category_not_selected_on_blur` | mode='add', categories=mockCategories。カテゴリフィールドを未選択のままフォーカスアウトする | RHF の onBlur バリデーションが発火し、カテゴリ AppSelect 直下に「カテゴリを選択してください」の日本語エラーメッセージが表示される（AppSelect の `onBlur` prop が Controller の `field.onBlur` に接続されていることを保証。保存ボタン押下前にリアルタイム表示される点が ITM-FE-033 との差分） |

**備考**:
- PR #76 のテストコード内で `onBlur-5` と暫定的に識別されているテストケースは、本節の `ITM-FE-107` に付け替えること。
- 他フィールド（日付・金額・摘要）の onBlur トリガ相当テストは本スコープ外（日付は AppDatePicker の ADT-004 / ADT-005 でコンポーネント側を、金額・摘要はネイティブ MUI TextField の onBlur を使用するため、issue 118 の追加設計は不要）。必要になった時点で別途採番する。
