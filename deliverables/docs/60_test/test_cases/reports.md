...
| RPT-091 | 統合 | handler | 正常系 | RPT-F02, NFR-PERF-004 | openapi.yaml#listMyReports | `TestListMyReports_Pagination` | Actor: Test Member（テナントA）。Test Member の自分のレポートが 2 件以上存在する状態で `GET /api/reports?per_page=1` を実行（issue #147） | 200 OK。`data` の件数が 1。`pagination.total_pages >= 2`、`pagination.per_page == 1`（per_page 動作の保証。WFL-005 と同パターン） |
...
| listMyReports（per_page、issue #147） | RPT-091 | 1 |
...
| 追記テスト（issue #141 対象期間バリデーション 両フィールド発火） | RPT-FE-107, RPT-FE-110 | 2 |
| 追記テスト（issue #144 ReportInfoCard ラベル + 作成日フォーマット） | RPT-FE-108〜RPT-FE-109, RPT-FE-108-A〜RPT-FE-109-A | 4 |
| 追記テスト（issue #147 per_page UI セレクタ Page 結合） | RPT-FE-111〜RPT-FE-114 | 4 |
| 追記テスト（issue #147 再オープン 中間ラッパー paginationFooter prop + 空状態フッター常時表示） | RPT-FE-115〜RPT-FE-116 | 2 |
| AppDatePicker 共通 UI 単体テスト（issue 119/120） | ADT-001〜ADT-007 | 7 |
| AppSelect 共通 UI 単体テスト（issue 118 スコープ拡張） | ASL-001〜ASL-002 | 2 |
| PageSizeSelector 共通 UI 単体テスト（issue #147） | PSS-001〜PSS-005 | 5 |
| AppPaginationFooter 共通 UI 単体テスト（issue #147 + 再々オープン A2 案） | APF-001〜APF-011 | 11 |
| AppDataGrid 共通 UI 単体テスト（issue #147 再オープン slots 合成挙動検証） | ADG-001〜ADG-004 | 4 |
...
| RPT-FE-041 | 単体 | ReportPeriodField | periodEndError?: string | 異常系 | RPT-F01, RPT-003 | 55_ui_component/screens/report-create.md §ReportPeriodField, §4 V5-E | `test_ReportPeriodField_shows_end_error` | periodEndError = "終了日は開始日以降を指定してください" | 終了日の AppDatePicker にエラーメッセージ「終了日は開始日以降を指定してください」が表示される（issue #141 でフィールド主語別文言に統一） |
...
##### PageSizeSelector 共通 UI 単体テスト（issue #147）

`PageSizeSelector` は `components/ui/` 配下の共通 UI コンポーネントで、表示件数（per_page）を選択するセレクタを提供する。SCR-RPT-001 / SCR-ADM-001 / SCR-WFL-001 / SCR-WFL-002 の 4 一覧画面で `AppPaginationFooter` 経由で横断利用される。AppDatePicker / AppSelect と同パターンで、独自プレフィックス `PSS-` を採用し本ファイルに集約する（採番方針は issue #147 Q1 確定事項）。
...
##### AppPaginationFooter 共通 UI 単体テスト（issue #147）

`AppPaginationFooter` は `AppPagination` と `PageSizeSelector` を 1 行に合成する共通 UI コンポーネント。SCR-RPT-001 / SCR-ADM-001 / SCR-WFL-001 / SCR-WFL-002 の 4 一覧画面で横断利用される。独自プレフィックス `APF-` を採用し本ファイルに集約する（issue #147 Q1）。
...
| APF-002 | 単体 | AppPaginationFooter | totalPages: number | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter | `test_AppPaginationFooter_always_renders_when_totalPages_zero` | `totalPages=0`, `currentPage=1`, `perPage=20`（空一覧ケース） | フッターが描画され、`AppPagination` がページ番号「1」を 1 つ表示する（`count={Math.max(0, 1)} = 1`、issue #147 Q3 常時表示）。`PageSizeSelector` も描画される |
| APF-003 | 単体 | AppPaginationFooter | totalPages: number | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter | `test_AppPaginationFooter_always_renders_when_totalPages_one` | `totalPages=1`, `currentPage=1`, `perPage=20` | フッターが非表示にならず、ページ番号「1」と表示件数セレクタが描画される（旧仕様の「1 ページ未満時非表示」が撤廃されたことを保証、issue #147 Q3） |
...
| APF-011 | 単体 | AppPaginationFooter | sx prop（最小高さ・余白、A2 案視覚回帰防止） | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter §レイアウト仕様 | `test_AppPaginationFooter_applies_min_height_and_padding` | フッターをレンダリングしてルート要素を取得 | ルート `Box` の `sx` または computed style に `minHeight: 52` + `px: 2` + `py: 0.5` 相当の設定が含まれる（issue #147 再々オープン理由 2 解消: PageSizeSelector の MUI Select 枠線がフッター高さを支配しないようにする）。jsdom 上では `sx` prop の値検証で代替する。併せて `PageSizeSelector` の FormControl が `margin="none"` + `sx={{ my: 0 }}` を持つことも検証する（高さ支配回帰防止の二重保証） |

**備考**:
- `PSS-` / `APF-` プレフィックスは既存テスト ID（RPT-/RPT-FE-/ADT-/ASL-/WFL-/WFL-FE-/APR-FE-/PAY-FE-/TNT-/TNT-FE-/ITM-/ITM-FE- 等）と衝突しないことを確認済み（issue #147 Q1 で新設）。
...
- APF-008〜011 は issue #147 再々オープン（A2 案: 見栄えを MUI 標準に寄せる）で追加。境界線描画（再々オープン理由 1 解消）/ 件数表示の算出ロジック（端数ページ・0 件時）/ `totalCount` 未指定時の後方互換 / 最小高さと余白による Select 高さ支配回帰防止（再々オープン理由 2 解消）を保証する。

---

#### FE-7. 追記テスト（issue #141: 対象期間バリデーション 両フィールド発火 + フィールド別文言）

issue #141 対応として、開始日 blur で V5 が発火し開始日フィールド直下に主語別文言が表示されるケースを採番する。既存 RPT-FE-041（終了日側）の対となるテストで、Zod refine の `path: ['periodStart']` / `path: ['periodEnd']` 2 経路と、両フィールド `onBlur` での `trigger(['periodStart', 'periodEnd'])` を保証する。
...
- 実装側では `expense-saas/frontend/src/components/report/ReportForm.tsx`（または `ReportPeriodField`）の Zod スキーマで refine を 2 つに分割する。両フィールドの onBlur では `getValues()` で両方入力済みを判定し、両方入力済みの場合のみ `trigger(['periodStart', 'periodEnd'])` を呼び出して V5 を相互再評価する。片方のみ入力済みの場合は当該フィールドの通常バリデーションのみ走らせ、未操作側に V3/V4 の必須エラーが先出しされないようにする（PR #95 codex 指摘対応で追加された RPT-FE-110 の前提仕様）。

---

#### FE-8. 追記テスト（issue #144: ReportInfoCard 常時表示 4 項目のラベル + 作成日フォーマット）

issue #144「ReportInfoCard 常時表示 4 項目のラベル欠落 + 作成日フォーマット設計乖離」対応。`50_detail_design/screens/report-detail.md` §3 常時表示項目表の改訂（ラベル付き表記を正本化、作成日は `YYYY/MM/DD HH:mm`）を保証するため、`ReportBasicInfo` のコンポーネント単体テストを追加する。既存 `RPT-FE-071`（renders_all_fields）は描画存在のみを検証しているため、ラベル文字列とフォーマットを直接アサートする本ケース（RPT-FE-108 / RPT-FE-109）を追加する。

ID 採番方針: 並行 issue #141 が `RPT-FE-107` を予約済みのため、本 issue では `RPT-FE-108` 以降を使用する。
...
- 表記揺れ注意: 期待結果のラベルは「作成日:」で固定（時刻付き表示でも「作成日時:」とはしない）。ワークフロー系の「提出日:」「承認日:」「却下日:」「支払完了日:」と命名規則を揃える（issue #144 推奨。「支払完了日」はドメインモデル `paid_at` / Step 5.5 §ReportWorkflowInfo の既存正本に準拠）。
- 既存 `RPT-FE-071` は変更不要（描画存在の検証として維持）。RPT-FE-108 / RPT-FE-109 は値の表記仕様（ラベル + フォーマット）を直接アサートする補強ケースとして追加。
- RPT-FE-108-A / RPT-FE-109-A は枝番表記（過去 RPT-FE-090-A と同パターン: 同一意味のテスト・別経路）。検証経路として `ReportInfoCard.tsx`（ReportDetailPage が実画面で直接描画する本体）を対象に取り、`ReportBasicInfo.tsx` が現状の実装で参照されていない問題（インライン重複は別 issue 推奨）の影響を受けずにラベル + フォーマットの実画面回帰を保証する。
- 作成日フォーマットの JST 固定: 実装側 `ReportInfoCard.tsx` の `formatDateTimeJa` および `ReportBasicInfo.tsx` の `formattedCreatedAt` で `timeZone: 'Asia/Tokyo'` を明示し、実行環境の TZ（CI 上の TZ=UTC 等）に依存しないことを担保する（PR #94 codex 再指摘対応）。

---

#### FE-9. 追記テスト（issue #147: per_page UI セレクタ + URL 駆動 Page 結合）

issue #147 対応として、`ReportListPage`（SCR-RPT-001）における URL ⇔ UI ⇔ API 連動経路を Page 結合テストとして採番する。`PageSizeSelector` / `AppPaginationFooter` の単体テストは `PSS-001〜005` / `APF-001〜007` に集約済み（FE-6 節 AppDatePicker / AppSelect 共通 UI テストの並びを踏襲）。本節はそれらを `ReportListPage` 経由で統合的に検証するケース。
...
- フッター常時表示（Q3）の検証は `APF-002 / APF-003` に集約しているため本節では再検証しない。
- `PageSizeSelector` の動的選択肢挙動の単体検証は `PSS-002 / PSS-003` に集約。本節 RPT-FE-113 は Page 経由（URL → Hook → Selector）の統合確認。

---

#### FE-10. 追記テスト（issue #147 再オープン: 中間ラッパー paginationFooter prop + 空状態フッター常時表示）

issue #147 再オープン対応で、中間ラッパー（`ReportListTable`）に `paginationFooter?: ReactNode` prop を追加し、`AppDataGrid` の `slots.footer` 経由で DataGrid フッターコンテナに統合する D-1 + ②a 設計を採用した。空状態時もフッターを常時表示する Q3 仕様の回帰防止検証を含む。

採番方針: 既存 FE 系最大 ID は `RPT-FE-114`（FE-9 節）。次番 `RPT-FE-115` から採用する。

| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---------|-----------|------------------|------------------|---------|-----------|-----------|---------------|-----------------|---------|
| RPT-FE-115 | 単体 | ReportListTable | `paginationFooter?: ReactNode` prop | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter 配置・利用前提（パターン ②a）, 50_detail_design/screens/report-list.md §5 | `test_ReportListTable_paginationFooter_prop_renders_in_datagrid_footer_slot` | reports に複数件のデータを渡し、`paginationFooter={<div data-testid="custom-footer">FOOTER</div>}` を渡す | AppDataGrid の `slots.footer` として `paginationFooter` で渡した要素が DataGrid フッターコンテナ内に描画される（`data-testid="custom-footer"` で取得可能） |
| RPT-FE-116 | 単体 | ReportListTable | `paginationFooter?: ReactNode` prop, 空状態 | 正常系 | RPT-F02 | issue #147 §確定方針 Q3, 55_ui_component/common-components.md §AppPaginationFooter L360 常時表示 | `test_ReportListTable_paginationFooter_renders_even_when_empty` | reports = [] かつ `paginationFooter={<div data-testid="custom-footer">FOOTER</div>}` を渡す | 空状態でも AppDataGrid が描画され、`slots.noRowsOverlay` 経由で EmptyState が表示されつつ、`slots.footer` の `paginationFooter` も同時に表示される（issue #147 Q3 リグレッション防止） |
...
| 追記テスト（issue 120: ReportForm 日本語エラー） | RPT-FE-105 -- RPT-FE-106 | 2 |
| 追記テスト（issue #141: 対象期間バリデーション 両フィールド発火 + 必須先出し回避） | RPT-FE-107, RPT-FE-110 | 2 |
| 追記テスト（issue #144: ReportInfoCard ラベル + 作成日フォーマット + JST 固定） | RPT-FE-108 -- RPT-FE-109, RPT-FE-108-A, RPT-FE-109-A | 4 |
| 追記テスト（issue #147: per_page UI セレクタ Page 結合） | RPT-FE-111 -- RPT-FE-114 | 4 |
| 追記テスト（issue #147 再オープン: 中間ラッパー paginationFooter prop + 空状態フッター常時表示） | RPT-FE-115 -- RPT-FE-116 | 2 |
| AppDatePicker 共通 UI 単体テスト（issue 119/120） | ADT-001 -- ADT-007 | 7 |
| AppSelect 共通 UI 単体テスト（issue 118 スコープ拡張） | ASL-001 -- ASL-002 | 2 |
| PageSizeSelector 共通 UI 単体テスト（issue #147） | PSS-001 -- PSS-005 | 5 |
| AppPaginationFooter 共通 UI 単体テスト（issue #147 + 再々オープン A2 案: 境界線 / 件数表示 / 後方互換 / 高さ支配回帰防止） | APF-001 -- APF-011 | 11 |
| AppDataGrid 共通 UI 単体テスト（issue #147 再オープン: slots 合成挙動検証） | ADG-001 -- ADG-004 | 4 |
| **FE 合計** | **RPT-FE-001 -- RPT-FE-116, RPT-FE-090-A, RPT-FE-108-A, RPT-FE-109-A, ADT-001 -- ADT-007, ASL-001 -- ASL-002, PSS-001 -- PSS-005, APF-001 -- APF-011, ADG-001 -- ADG-004** | **148** |
