...
| RPT-091 | 統合 | handler | 正常系 | RPT-F02, NFR-PERF-004 | openapi.yaml#listMyReports | `TestListMyReports_Pagination` | Actor: Test Member（テナントA）。Test Member の自分のレポートが 2 件以上存在する状態で `GET /api/reports?per_page=1` を実行 | 200 OK。`data` の件数が 1。`pagination.total_pages >= 2`、`pagination.per_page == 1`（per_page 動作の保証。WFL-005 と同パターン） |
...
| RPT-010 | 統合 | handler | 異常系 | RPT-F01, RPT-003 | openapi.yaml#createReport, db_schema.md#expense_reports#CHECK | `TestCreateReport_ValidationError_PeriodRange` | Actor: Test Member。`period_start="2026-03-31"`, `period_end="2026-03-01"`（終了が開始より前） | 422 VALIDATION_ERROR。`details[].field = "period_end"` を含む（`period_end` は `period_start` 以降である必要がある） |
...
| listMyReports（per_page） | RPT-091 | 1 |
| **BE 合計** | **RPT-001〜RPT-091** | **91** |
...
| 追記テスト（ReportPeriodField onBlur） | RPT-FE-103〜RPT-FE-104 | 2 |
| 追記テスト（ReportForm 日本語エラー） | RPT-FE-105〜RPT-FE-106 | 2 |
| 追記テスト（対象期間バリデーション 両フィールド発火） | RPT-FE-107, RPT-FE-110 | 2 |
...
|---|---|---|---|---|---|---|---|---|---|
| RPT-FE-001 | 単体 | ReportListPage | useMyReports | 正常系 | RPT-F02 | 55_ui_component/screens/report-list.md §ReportListPage | `test_ReportListPage_renders_report_list` | useMyReports が 3 件のレポートデータを返す | ReportListHeader, ReportListFilter, ReportListTable, AppPaginationFooter が描画される |
...
| RPT-FE-041 | 単体 | ReportPeriodField | periodEndError?: string | 異常系 | RPT-F01, RPT-003 | 55_ui_component/screens/report-create.md §ReportPeriodField, §4 V5-E | `test_ReportPeriodField_shows_end_error` | periodEndError = "終了日は開始日以降を指定してください" | 終了日の AppDatePicker にエラーメッセージ「終了日は開始日以降を指定してください」が表示される（フィールド主語別文言に統一） |
...
**備考**: ReportDetailPage 側の明細行クリックプリフィルテストは、OwnerActions 側の RPT-FE-090 と ID が重複するため RPT-FE-090-A に振り直した。OwnerActions 側の RPT-FE-090（本行）はそのまま維持する。
...
#### FE-6. 追記テスト（onBlur 伝播 / AppDatePicker 型統一）

回帰テスト。既存 `RPT-FE-043`/`RPT-FE-044`（ReportFormActions 用）と `RPT-FE-059`/`RPT-FE-060`（useReport 用）に ID 衝突していたため、以下の新 ID に振り直す。AppDatePicker の共通 UI 単体テストには新プレフィックス `ADT-` を採用する（共通 UI コンポーネント用）。

##### ReportPeriodField onBlur 伝播
...
| ADT-002 | 単体 | AppDatePicker | onChange: (value: string) => void | 正常系 | RPT-F01 | 55_ui_component/common-components.md §AppDatePicker | `test_AppDatePicker_onChange_receives_string_value` | 入力フィールドに `"2026-03-01"` を入力する | onChange が `"2026-03-01"` （`string`）で呼び出される（旧仕様の `string \| null` を廃止し、必ず `string` を渡すことを保証） |
| ADT-003 | 単体 | AppDatePicker | onChange: (value: string) => void | 正常系 | RPT-F01 | 55_ui_component/common-components.md §AppDatePicker | `test_AppDatePicker_onChange_empty_string_on_clear` | 値を入力した状態からフィールドをクリアする | onChange が空文字 `""` （`null` ではない）で呼び出される |
| ADT-004 | 単体 | AppDatePicker | onBlur?: () => void | 正常系 | RPT-F01 | 55_ui_component/common-components.md §AppDatePicker | `test_AppDatePicker_calls_onBlur_on_focusout` | `onBlur` モックを渡してフィールドからフォーカスアウトする | `onBlur` コールバックが 1 回呼び出される（RHF Controller の `field.onBlur` に伝播させることを保証） |
...
**備考**:
- 旧 ID `RPT-FE-043` / `RPT-FE-044` は `ReportFormActions`（行 442〜443）、旧 ID `RPT-FE-059` / `RPT-FE-060` は `useReport`（行 465〜466）で既に採番済みのため変更しない。該当テストコードは本節の新 ID（`RPT-FE-103〜106`）に付け替えること。
- AppDatePicker の新プレフィックス `ADT-` は既存テスト ID と衝突しないことを確認済み（`grep -rn 'ADT-0' 60_test/test_cases/` で該当なし）。

##### AppSelect 共通 UI 単体テスト
...
| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| ASL-001 | 単体 | AppSelect | onBlur?: () => void | 正常系 | ITM-F01 | 55_ui_component/common-components.md §AppSelect | `test_AppSelect_calls_onBlur_on_focusout` | `onBlur` モックを渡してフィールドからフォーカスアウトする | `onBlur` コールバックが 1 回呼び出される（RHF Controller の `field.onBlur` に伝播させることを保証） |
...
**備考**:
- AppSelect の新プレフィックス `ASL-` は既存テスト ID と衝突しないことを確認済み。
- `value` / `onChange` / `options` / `errorMessage` / `disabled` などの既存 Props の単体テストは本節では採番しない（ADT-001〜003・006〜007 と同等の網羅は必要になった時点で別途追加する）。本節は CI 失敗（`onBlur-5` 相当）を解消するために必要な最小範囲に限定する。

##### PageSizeSelector 共通 UI 単体テスト

`PageSizeSelector` は `components/ui/` 配下の共通 UI コンポーネントで、表示件数（per_page）を選択するセレクタを提供する。SCR-RPT-001 / SCR-ADM-001 / SCR-WFL-001 / SCR-WFL-002 の 4 一覧画面で `AppPaginationFooter` 経由で横断利用される。AppDatePicker / AppSelect と同パターンで、独自プレフィックス `PSS-` を採用し本ファイルに集約する。

| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| PSS-001 | 単体 | PageSizeSelector | perPage: number, standardOptions?: number[] | 正常系 | RPT-F02 | 55_ui_component/common-components.md §PageSizeSelector | `test_PageSizeSelector_renders_standard_options` | `perPage=20`, `standardOptions` 省略（デフォルト `[10,20,50,100]`） | 「表示件数:」ラベルと Select が描画され、選択肢に `10, 20, 50, 100` の 4 件が含まれる。現在値として `20` が選択されている |
| PSS-002 | 単体 | PageSizeSelector | perPage: number, standardOptions?: number[] | 正常系 | RPT-F02 | 55_ui_component/common-components.md §PageSizeSelector | `test_PageSizeSelector_appends_non_standard_perPage_to_options` | `perPage=1`, `standardOptions=[10,20,50,100]`（URL 由来の標準外値ケース） | 選択肢が昇順で `[1, 10, 20, 50, 100]` の 5 件になり、現在値として `1` が選択されている（動的選択肢挙動の保証） |
| PSS-003 | 単体 | PageSizeSelector | perPage: number, standardOptions?: number[] | 正常系 | RPT-F02 | 55_ui_component/common-components.md §PageSizeSelector | `test_PageSizeSelector_dedupes_when_perPage_already_in_standard` | `perPage=20`, `standardOptions=[10,20,50,100]` | 選択肢は `[10, 20, 50, 100]` のまま 4 件で、`20` が重複して追加されない（重複ガード `Set` 等で除去、MUI MenuItem `key` warning 回避） |
| PSS-004 | 単体 | PageSizeSelector | onPerPageChange: (size: number) => void | 正常系 | RPT-F02 | 55_ui_component/common-components.md §PageSizeSelector | `test_PageSizeSelector_calls_onPerPageChange_with_number` | `perPage=20` の状態で「50」を選択 | `onPerPageChange` が **数値 `50`**（文字列 `"50"` ではない）で 1 回呼び出される |
| PSS-005 | 単体 | PageSizeSelector | disabled?: boolean | 正常系 | RPT-F02 | 55_ui_component/common-components.md §PageSizeSelector | `test_PageSizeSelector_disables_when_disabled_true` | `disabled={true}`, `perPage=20` | Select 要素が disabled 状態になり、クリック・キーボード操作で開かない |

##### AppPaginationFooter 共通 UI 単体テスト

`AppPaginationFooter` は `AppPagination` と `PageSizeSelector` を 1 行に合成する共通 UI コンポーネント。SCR-RPT-001 / SCR-ADM-001 / SCR-WFL-001 / SCR-WFL-002 の 4 一覧画面で横断利用される。独自プレフィックス `APF-` を採用し本ファイルに集約する。

| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| APF-001 | 単体 | AppPaginationFooter | currentPage, totalPages, perPage | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter | `test_AppPaginationFooter_composes_pagination_and_selector` | `currentPage=1, totalPages=3, perPage=20, onPageChange, onPerPageChange` を渡す | 内部に `AppPagination`（ページ番号 1〜3 が描画される）と `PageSizeSelector`（現在値 20）が並置される |
| APF-002 | 単体 | AppPaginationFooter | totalPages: number | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter | `test_AppPaginationFooter_always_renders_when_totalPages_zero` | `totalPages=0`, `currentPage=1`, `perPage=20`（空一覧ケース） | フッターが描画され、`AppPagination` がページ番号「1」を 1 つ表示する（`count={Math.max(0, 1)} = 1`、空状態でも常時表示）。`PageSizeSelector` も描画される |
| APF-003 | 単体 | AppPaginationFooter | totalPages: number | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter | `test_AppPaginationFooter_always_renders_when_totalPages_one` | `totalPages=1`, `currentPage=1`, `perPage=20` | フッターが非表示にならず、ページ番号「1」と表示件数セレクタが描画される（旧仕様の「1 ページ未満時非表示」が撤廃されたことを保証） |
| APF-004 | 単体 | AppPaginationFooter | sx prop（レスポンシブ） | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter | `test_AppPaginationFooter_applies_responsive_flexDirection` | フッターをレンダリングしてルート要素を取得 | ルート `Box` の `sx` または `style` に `flexDirection={{ xs: 'column', sm: 'row' }}` 相当の設定が含まれる（jsdom では実 viewport 切替不可のため `sx` prop の存在確認で代替） |
| APF-005 | 単体 | AppPaginationFooter | disabled?: boolean | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter | `test_AppPaginationFooter_disables_inner_components` | `disabled={true}` を渡す | 内部 `AppPagination` および `PageSizeSelector` の双方に `disabled=true` が伝播する（ページ送り・件数変更が共に不可になる） |
| APF-006 | 単体 | AppPaginationFooter | onPageChange: (page: number) => void | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter | `test_AppPaginationFooter_forwards_onPageChange` | `currentPage=1, totalPages=3` でページ「2」をクリック | `onPageChange` が `2` で 1 回呼び出される |
| APF-007 | 単体 | AppPaginationFooter | onPerPageChange: (size: number) => void | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter | `test_AppPaginationFooter_forwards_onPerPageChange` | `perPage=20` の状態で「50」を選択 | `onPerPageChange` が `50` で 1 回呼び出される（page リセットの責務は呼び出し側 = ページコンポーネントが負うため、本コンポーネントテストでは page リセットは検証しない） |
| APF-008 | 単体 | AppPaginationFooter | sx prop（境界線） | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter §レイアウト仕様 | `test_AppPaginationFooter_renders_border_top` | フッターをレンダリングしてルート要素を取得 | ルート `Box` の `sx` または computed style に `borderTop: '1px solid'` + `borderColor: 'divider'` 相当の設定が含まれる（DataGrid 標準フッター border-top と揃える）。jsdom 上では `sx` prop の値検証で代替する |
| APF-009 | 単体 | AppPaginationFooter | totalCount?: number | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter §件数表示フォーマット | `test_AppPaginationFooter_renders_count_display_from_totalCount` | `currentPage=2, perPage=10, totalCount=37, totalPages=4` を渡す | フッター左側に「11 - 20 / 全 37 件」が描画される（`start = (2-1)*10 + 1 = 11`、`end = min(2*10, 37) = 20`）。端数ページ: `currentPage=4, perPage=10, totalCount=37` の場合は「31 - 37 / 全 37 件」（end が totalCount で打ち止め）。0 件時: `totalCount=0` の場合は「0 - 0 / 全 0 件」を表示する（フッター常時表示方針と整合） |
| APF-010 | 単体 | AppPaginationFooter | totalCount?: number（未指定ケース） | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter §件数表示フォーマット | `test_AppPaginationFooter_hides_count_display_when_totalCount_undefined` | `totalCount` prop を渡さない（`undefined`）状態でフッターをレンダリング | 件数表示の DOM 要素が描画されない（後方互換、既存利用箇所への影響なし）。`AppPagination` と `PageSizeSelector` は通常通り描画される |
| APF-011 | 単体 | AppPaginationFooter | sx prop（最小高さ・余白、視覚回帰防止） | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter §レイアウト仕様 | `test_AppPaginationFooter_applies_min_height_and_padding` | フッターをレンダリングしてルート要素を取得 | ルート `Box` の `sx` または computed style に `minHeight: 52` + `px: 2` + `py: 0.5` 相当の設定が含まれる（PageSizeSelector の MUI Select 枠線がフッター高さを支配しないようにする）。jsdom 上では `sx` prop の値検証で代替する。併せて `PageSizeSelector` の FormControl が `margin="none"` + `sx={{ my: 0 }}` を持つことも検証する（高さ支配回帰防止の二重保証） |

**備考**:
- `PSS-` / `APF-` プレフィックスは既存テスト ID（RPT-/RPT-FE-/ADT-/ASL-/WFL-/WFL-FE-/APR-FE-/PAY-FE-/TNT-/TNT-FE-/ITM-/ITM-FE- 等）と衝突しないことを確認済み。
- `PageSizeSelector` / `AppPaginationFooter` の Page 統合経路（URL 反映、page リセット）は `RPT-FE-111〜114` / `TNT-FE-048〜051` / `WFL-FE-083〜090` の各 Page 結合テストでカバーする。
- APF-008〜011 は見栄えを MUI 標準に寄せる対応で追加。境界線描画 / 件数表示の算出ロジック（端数ページ・0 件時）/ `totalCount` 未指定時の後方互換 / 最小高さと余白による Select 高さ支配回帰防止を保証する。

---

#### FE-7. 追記テスト（対象期間バリデーション 両フィールド発火 + フィールド別文言）

開始日 blur で V5 が発火し開始日フィールド直下に主語別文言が表示されるケースを採番する。既存 RPT-FE-041（終了日側）の対となるテストで、Zod refine の `path: ['periodStart']` / `path: ['periodEnd']` 2 経路と、両フィールド `onBlur` での `trigger(['periodStart', 'periodEnd'])` を保証する。

| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| RPT-FE-107 | 単体 | ReportForm | reportCreateSchema（Zod refine: periodStart / periodEnd 2 経路）, RHF mode: 'onBlur' | 異常系 | RPT-F01, RPT-003 | 55_ui_component/screens/report-create.md §ReportForm, 50_detail_design/screens/report-create.md §4 V5-S | `test_ReportForm_onBlur_shows_periodStart_period_violation_error` | periodStart = "2026-04-30"、periodEnd = "2026-04-01" を入力した状態で、開始日フィールドのみをフォーカスアウトする | 開始日の AppDatePicker 直下に「開始日は終了日以前を指定してください」が赤字で表示される。終了日側にも RPT-FE-041 と同じく「終了日は開始日以降を指定してください」が同時に表示される（trigger(['periodStart', 'periodEnd']) により両経路が再評価されることを保証） |
| RPT-FE-110 | 単体 | ReportPeriodField | trigger / getValues（両フィールド入力済み判定ガード）, RHF mode: 'onBlur' | 正常系 | RPT-F01 | 55_ui_component/screens/report-create.md §ReportPeriodField, 50_detail_design/screens/report-create.md §4 V3/V4 | `test_ReportPeriodField_no_required_error_leak_to_other_field_on_partial_input` | title = "テスト"、periodStart = "2026-04-01"、periodEnd = "" の状態で開始日フィールドのみフォーカスアウトする | 開始日の通常バリデーション（V3 + V5-S）のみ走り、未操作の終了日に「終了日を入力してください」（V4）が先出しされない。trigger は両フィールド入力済みの場合のみ呼び出されるため、片方のみ入力時は当該フィールド以外が再評価されない |

**備考**:
- 採番根拠: 本タスク以前の reports.md 内 RPT-FE 系最大 ID は `RPT-FE-106`（FE-6 節）。`RPT-FE-090-A` は別系統の枝番のため通番外。よって次番 `RPT-FE-107` を採用した。
- 既存 RPT-FE-041 は終了日側のエラー文言ケースとして「終了日は開始日以降を指定してください」に統一済み。RPT-FE-107 はその対称ケース（開始日側）。
- 実装側では `expense-saas/frontend/src/components/report/ReportForm.tsx`（または `ReportPeriodField`）の Zod スキーマで refine を 2 つに分割する。両フィールドの onBlur では `getValues()` で両方入力済みを判定し、両方入力済みの場合のみ `trigger(['periodStart', 'periodEnd'])` を呼び出して V5 を相互再評価する。片方のみ入力済みの場合は当該フィールドの通常バリデーションのみ走らせ、未操作側に V3/V4 の必須エラーが先出しされないようにする（RPT-FE-110 の前提仕様）。

---

#### FE-8. 追記テスト（ReportInfoCard 常時表示 4 項目のラベル + 作成日フォーマット）

「ReportInfoCard 常時表示 4 項目のラベル欠落 + 作成日フォーマット設計乖離」対応。`50_detail_design/screens/report-detail.md` §3 常時表示項目表の改訂（ラベル付き表記を正本化、作成日は `YYYY/MM/DD HH:mm`）を保証するため、`ReportBasicInfo` のコンポーネント単体テストを追加する。既存 `RPT-FE-071`（renders_all_fields）は描画存在のみを検証しているため、ラベル文字列とフォーマットを直接アサートする本ケース（RPT-FE-108 / RPT-FE-109）を追加する。

ID 採番方針: 並行作業が `RPT-FE-107` を予約済みのため、本セットでは `RPT-FE-108` 以降を使用する。

| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| RPT-FE-108 | 単体 | ReportBasicInfo | periodStart, periodEnd, totalAmount, submitterName, createdAt（4 項目ラベル検証） | 正常系 | RPT-F03 | 55_ui_component/screens/report-detail.md §ReportBasicInfo, 50_detail_design/screens/report-detail.md §3 | `test_ReportBasicInfo_renders_all_four_labels` | title = "出張費"、status = "draft"、periodStart = "2026-03-01"、periodEnd = "2026-03-31"、totalAmount = 50000、submitterName = "テスト太郎"、createdAt = "2026-04-23T15:56:00+09:00" を渡す | カード内に「対象期間:」「合計金額:」「作成者:」「作成日:」の 4 つのラベル文字列がそれぞれ 1 回以上出現する（タイトル・ステータスバッジはラベルなしのため検証対象外）。各ラベルは値の前に表示されること |
| RPT-FE-109 | 単体 | ReportBasicInfo | createdAt: string（作成日フォーマット検証） | 正常系 | RPT-F03 | 55_ui_component/screens/report-detail.md §ReportBasicInfo, 50_detail_design/screens/report-detail.md §3 | `test_ReportBasicInfo_formats_created_at_with_time` | createdAt = "2026-04-23T15:56:00+09:00"（または UTC 換算で同等の ISO 8601 文字列）を渡す | 「作成日:」ラベル直後に `2026/04/23 15:56` が `YYYY/MM/DD HH:mm` 形式（JST 固定 = `timeZone: 'Asia/Tokyo'` 明示）で表示される。`YYYY/MM/DD` 単独表記（時刻なし）は出現しないこと（`formatDateTimeJa` 相当） |
| RPT-FE-108-A | 単体 | ReportInfoCard | report: ExpenseReportDetail（実画面で実際に描画されるラッパー経由検証） | 正常系 | RPT-F03 | 55_ui_component/screens/report-detail.md §ReportInfoCard, 50_detail_design/screens/report-detail.md §2 + §3 | `test_ReportInfoCard_renders_all_four_labels_via_inline_basic_info` | RPT-FE-108 と同等の入力を ExpenseReportDetail 形式で渡す | RPT-FE-108 と同じく「対象期間:」「合計金額:」「作成者:」「作成日:」が UI 上に出現する。実画面経路（ReportDetailPage → ReportInfoCard）でのラベル回帰を保証する（ReportInfoCard が ReportBasicInfo を import せずインライン記述している実装実態に合わせた補強テスト） |
| RPT-FE-109-A | 単体 | ReportInfoCard | report: ExpenseReportDetail（同上） | 正常系 | RPT-F03 | 55_ui_component/screens/report-detail.md §ReportInfoCard, 50_detail_design/screens/report-detail.md §2 + §3 | `test_ReportInfoCard_formats_created_at_with_time_via_inline_basic_info` | createdAt = "2026-04-23T15:56:00+09:00" を含む ExpenseReportDetail | RPT-FE-109 と同じく作成日が `YYYY/MM/DD HH:mm` 形式（JST 固定）で表示される。実画面経路でのフォーマット回帰を保証する |

**備考**:
- 本テストの実装ファイル候補は既存の `expense-saas/frontend/src/pages/reports/__tests__/ReportBasicInfo.test.tsx`（RPT-FE-071〜RPT-FE-072 と同じファイル）。
- ラベル文字列の検証は React Testing Library の `screen.getByText(/対象期間:/)` 等の正規表現マッチを推奨（ラベルと値が同一要素に含まれる場合・分離する場合の双方に対応するため）。
- 作成日フォーマット仕様の正本は `50_detail_design/screens/report-detail.md` §3 常時表示項目表（`YYYY/MM/DD HH:mm`）。本ファイルでは具体例（`2026/04/23 15:56`）のみ示し、フォーマット定義は再掲しない。
- 表記揺れ注意: 期待結果のラベルは「作成日:」で固定（時刻付き表示でも「作成日時:」とはしない）。ワークフロー系の「提出日:」「承認日:」「却下日:」「支払完了日:」と命名規則を揃える（「支払完了日」はドメインモデル `paid_at` / Step 5.5 §ReportWorkflowInfo の既存正本に準拠）。
- 既存 `RPT-FE-071` は変更不要（描画存在の検証として維持）。RPT-FE-108 / RPT-FE-109 は値の表記仕様（ラベル + フォーマット）を直接アサートする補強ケースとして追加。
- RPT-FE-108-A / RPT-FE-109-A は枝番表記（過去 RPT-FE-090-A と同パターン: 同一意味のテスト・別経路）。検証経路として `ReportInfoCard.tsx`（ReportDetailPage が実画面で直接描画する本体）を対象に取り、`ReportBasicInfo.tsx` が現状の実装で参照されていない問題の影響を受けずにラベル + フォーマットの実画面回帰を保証する。
- 作成日フォーマットの JST 固定: 実装側 `ReportInfoCard.tsx` の `formatDateTimeJa` および `ReportBasicInfo.tsx` の `formattedCreatedAt` で `timeZone: 'Asia/Tokyo'` を明示し、実行環境の TZ（CI 上の TZ=UTC 等）に依存しないことを担保する。

---

#### FE-9. 追記テスト（per_page UI セレクタ + URL 駆動 Page 結合）

`ReportListPage`（SCR-RPT-001）における URL ⇔ UI ⇔ API 連動経路を Page 結合テストとして採番する。`PageSizeSelector` / `AppPaginationFooter` の単体テストは `PSS-001〜005` / `APF-001〜007` に集約済み（FE-6 節 AppDatePicker / AppSelect 共通 UI テストの並びを踏襲）。本節はそれらを `ReportListPage` 経由で統合的に検証するケース。

採番方針: 既存 FE 系最大 ID は `RPT-FE-110`（FE-7 節）。`RPT-FE-108-A / RPT-FE-109-A` は枝番のため通番外。よって次番 `RPT-FE-111` から採用する。

| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| RPT-FE-111 | 単体 | ReportListPage | useMyReports（MSW）、useSearchParams | 正常系 | RPT-F02 | 50_detail_design/screens/report-list.md §5, 55_ui_component/state-management.md §3.1 | `test_ReportListPage_url_per_page_reflects_to_selector_and_api` | `/reports?per_page=10` で開く。MSW で `useMyReports` が 10 件 + `pagination.total_pages > 1` を返すよう設定 | テーブルに 10 件のみ描画される。フッターの `PageSizeSelector` が「10」を表示する。`useMyReports` への引数に `per_page: 10` が渡り、API URL に `?per_page=10` が含まれる |
| RPT-FE-112 | 単体 | ReportListPage | useMyReports（MSW）、useSearchParams | 正常系 | RPT-F02 | 50_detail_design/screens/report-list.md §5, 55_ui_component/state-management.md §3.1 | `test_ReportListPage_selector_change_updates_url_and_resets_page` | `/reports?page=3&per_page=10` で開いた状態で `PageSizeSelector` から「50」を選択 | URL が `/reports?page=1&per_page=50` に更新される（`page=1` リセット）。`setSearchParams` は 1 回のコールに集約される（race 回避）。`PageSizeSelector` の現在値が「50」に更新される |
| RPT-FE-113 | 単体 | ReportListPage | useMyReports、useSearchParams | 正常系 | RPT-F02 | 50_detail_design/screens/report-list.md §5, 55_ui_component/state-management.md §3.1 | `test_ReportListPage_url_non_standard_per_page_appends_to_options` | `/reports?per_page=1` で開く（URL に標準外値） | `PageSizeSelector` の選択肢が `[1, 10, 20, 50, 100]` に動的拡張され、現在値「1」が選択される（Hook 側にも `per_page=1` が渡る） |
| RPT-FE-114 | 単体 | ReportListPage | useMyReports、useSearchParams | 正常系 | RPT-F02 | 50_detail_design/screens/report-list.md §5, 55_ui_component/state-management.md §3.1 | `test_ReportListPage_url_invalid_per_page_falls_back_to_20` | `/reports?per_page=abc` で開く（NaN ケース） / `?per_page=-5` で開く（負数ケース） の 2 サブケース | 両サブケースとも `useMyReports` への引数 `per_page` が `20`（FE フォールバック）になり、`PageSizeSelector` も「20」を表示する（FE 側 fail-soft）。範囲内不正値（0, 101 等）は本ケースのスコープ外（API クエリにそのまま乗せて BE バリデーションで 422 検証は BE 側 RPT-091 / TNT-012 等の責務） |

**備考**:
- 採番根拠: `RPT-FE-110` の次番から開始。`RPT-FE-108-A` 等の枝番は通番外。
- 実装ファイル候補: `expense-saas/frontend/src/pages/reports/__tests__/ReportListPage.test.tsx`（既存 RPT-FE-001〜007 と同ファイル）。
- フッター常時表示の検証は `APF-002 / APF-003` に集約しているため本節では再検証しない。
- `PageSizeSelector` の動的選択肢挙動の単体検証は `PSS-002 / PSS-003` に集約。本節 RPT-FE-113 は Page 経由（URL → Hook → Selector）の統合確認。

---

#### FE-10. 追記テスト（中間ラッパー paginationFooter prop + 空状態フッター常時表示）

中間ラッパー（`ReportListTable`）に `paginationFooter?: ReactNode` prop を追加し、`AppDataGrid` の `slots.footer` 経由で DataGrid フッターコンテナに統合する設計を採用した。空状態時もフッターを常時表示する仕様の回帰防止検証を含む。

採番方針: 既存 FE 系最大 ID は `RPT-FE-114`（FE-9 節）。次番 `RPT-FE-115` から採用する。

| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---------|-----------|------------------|------------------|---------|-----------|-----------|---------------|-----------------|---------|
| RPT-FE-115 | 単体 | ReportListTable | `paginationFooter?: ReactNode` prop | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter 配置・利用前提, 50_detail_design/screens/report-list.md §5 | `test_ReportListTable_paginationFooter_prop_renders_in_datagrid_footer_slot` | reports に複数件のデータを渡し、`paginationFooter={<div data-testid="custom-footer">FOOTER</div>}` を渡す | AppDataGrid の `slots.footer` として `paginationFooter` で渡した要素が DataGrid フッターコンテナ内に描画される（`data-testid="custom-footer"` で取得可能） |
| RPT-FE-116 | 単体 | ReportListTable | `paginationFooter?: ReactNode` prop, 空状態 | 正常系 | RPT-F02 | 55_ui_component/common-components.md §AppPaginationFooter L360 常時表示 | `test_ReportListTable_paginationFooter_renders_even_when_empty` | reports = [] かつ `paginationFooter={<div data-testid="custom-footer">FOOTER</div>}` を渡す | 空状態でも AppDataGrid が描画され、`slots.noRowsOverlay` 経由で EmptyState が表示されつつ、`slots.footer` の `paginationFooter` も同時に表示される（リグレッション防止） |

**備考**:
- 採番根拠: `RPT-FE-114` の次番から開始。
- 実装ファイル: `expense-saas/frontend/src/pages/reports/__tests__/ReportListTable.test.tsx`（既存ファイル拡張）。
- 空状態の AppDataGrid 描画は `slots.noRowsOverlay` に EmptyState を渡す形に変更（中間ラッパーの早期 return 撤去）。既存 RPT-FE-019 も本対応で AppDataGrid 描画 + noRowsOverlay 経由 EmptyState 表示を期待する形に更新。
...
| 追記テスト（onBlur 伝播） | RPT-FE-103 -- RPT-FE-104 | 2 |
| 追記テスト（ReportForm 日本語エラー） | RPT-FE-105 -- RPT-FE-106 | 2 |
| 追記テスト（対象期間バリデーション 両フィールド発火 + 必須先出し回避） | RPT-FE-107, RPT-FE-110 | 2 |
| 追記テスト（ReportInfoCard ラベル + 作成日フォーマット + JST 固定） | RPT-FE-108 -- RPT-FE-109, RPT-FE-108-A, RPT-FE-109-A | 4 |
| 追記テスト（per_page UI セレクタ Page 結合） | RPT-FE-111 -- RPT-FE-114 | 4 |
| 追記テスト（中間ラッパー paginationFooter prop + 空状態フッター常時表示） | RPT-FE-115 -- RPT-FE-116 | 2 |
| AppDatePicker 共通 UI 単体テスト | ADT-001 -- ADT-007 | 7 |
| AppSelect 共通 UI 単体テスト | ASL-001 -- ASL-002 | 2 |
| PageSizeSelector 共通 UI 単体テスト | PSS-001 -- PSS-005 | 5 |
| AppPaginationFooter 共通 UI 単体テスト（境界線 / 件数表示 / 後方互換 / 高さ支配回帰防止） | APF-001 -- APF-011 | 11 |
| AppDataGrid 共通 UI 単体テスト（slots 合成挙動検証） | ADG-001 -- ADG-004 | 4 |
...
expense-saas/frontend/src/
  pages/reports/
    __tests__/
      ...
      ReportInfoCard.test.tsx        # RPT-FE-070, RPT-FE-108-A, RPT-FE-109-A
      ReportBasicInfo.test.tsx       # RPT-FE-071〜RPT-FE-072, RPT-FE-108〜RPT-FE-109
  ...
  components/report/
    __tests__/
      ...
      ReportPeriodField.test.tsx     # RPT-FE-039〜RPT-FE-042, RPT-FE-103〜RPT-FE-104, RPT-FE-107, RPT-FE-110
