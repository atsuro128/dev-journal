...
| 追記テスト（issue #141 対象期間バリデーション 両フィールド発火） | RPT-FE-107 | 1 |
| 追記テスト（issue #144 ReportInfoCard ラベル + 作成日フォーマット） | RPT-FE-108〜RPT-FE-109 | 2 |
| AppDatePicker 共通 UI 単体テスト（issue 119/120） | ADT-001〜ADT-007 | 7 |
| AppSelect 共通 UI 単体テスト（issue 118 スコープ拡張） | ASL-001〜ASL-002 | 2 |
| **FE 合計** | **RPT-FE-001〜RPT-FE-109, RPT-FE-090-A, ADT-001〜ADT-007, ASL-001〜ASL-002** | **119** |
...
- 実装側では `expense-saas/frontend/src/components/report/ReportForm.tsx`（または `ReportPeriodField`）の Zod スキーマで refine を 2 つに分割し、両フィールドの onBlur で `trigger(['periodStart', 'periodEnd'])` を呼び出すこと。
...
| RPT-FE-109 | 単体 | ReportBasicInfo | createdAt: string（作成日フォーマット検証） | 正常系 | RPT-F03 | 55_ui_component/screens/report-detail.md §ReportBasicInfo, 50_detail_design/screens/report-detail.md §3 | `test_ReportBasicInfo_formats_created_at_with_time` | createdAt = "2026-04-23T15:56:00+09:00"（または UTC 換算で同等の ISO 8601 文字列）を渡す | 「作成日:」ラベル直後に `2026/04/23 15:56` が `YYYY/MM/DD HH:mm` 形式で表示される。`YYYY/MM/DD` 単独表記（時刻なし）は出現しないこと。タイムゾーンは JST 表示（`formatDateTimeJa` 相当） |
...
expense-saas/frontend/src/
  ...
  components/report/
    __tests__/
      ReportForm.test.tsx            # RPT-FE-028〜RPT-FE-036, RPT-FE-058, RPT-FE-105〜RPT-FE-107
