...
| AppPaginationFooter 共通 UI 単体テスト（issue #147 + 再々オープン A2 案） | APF-001〜APF-010 | 10 |
| AppDataGrid 共通 UI 単体テスト（issue #147 再オープン slots 合成挙動検証） | ADG-001〜ADG-004 | 4 |
| **FE 合計** | **RPT-FE-001〜RPT-FE-116, RPT-FE-090-A, RPT-FE-108-A, RPT-FE-109-A, ADT-001〜ADT-007, ASL-001〜ASL-002, PSS-001〜PSS-005, APF-001〜APF-010, ADG-001〜ADG-004** | **147** |
...
- APF-008 / APF-009 / APF-010 は issue #147 再々オープン（A2 案: 見栄えを MUI 標準に寄せる）で追加。境界線描画 / 件数表示の算出ロジック（端数ページ・0 件時）/ `totalCount` 未指定時の後方互換を保証する。
...
| AppSelect 共通 UI 単体テスト（issue 118 スコープ拡張） | ASL-001 -- ASL-002 | 2 |
| PageSizeSelector 共通 UI 単体テスト（issue #147） | PSS-001 -- PSS-005 | 5 |
| AppPaginationFooter 共通 UI 単体テスト（issue #147 + 再々オープン A2 案: 境界線 / 件数表示 / 後方互換） | APF-001 -- APF-010 | 10 |
| AppDataGrid 共通 UI 単体テスト（issue #147 再オープン: slots 合成挙動検証） | ADG-001 -- ADG-004 | 4 |
| **FE 合計** | **RPT-FE-001 -- RPT-FE-116, RPT-FE-090-A, RPT-FE-108-A, RPT-FE-109-A, ADT-001 -- ADT-007, ASL-001 -- ASL-002, PSS-001 -- PSS-005, APF-001 -- APF-010, ADG-001 -- ADG-004** | **147** |
