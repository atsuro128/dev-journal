...
| テストID | テストレベル | レイヤー | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 | 優先度 |
|---------|------------|--------|---------|-----------|-----------|--------------|-----------------|---------|--------|
| ATT-020 | 統合 | handler | 正常系 | ATT-F02 | openapi.yaml#listAttachments | `TestListAttachments_Success_Owner` | 前提: `report_draft` + `att_draft_jpeg`、認証: Test Member（所有者） | 200 OK。`data` 配列に `att_draft_jpeg` の情報を含む。各要素に `id`, `item_id`, `file_name`, `file_size`, `mime_type`, `created_at` を含み、`download_url` を含まない | 高 |
...
| ATT-025 | 統合 | handler | 正常系 | ATT-F02 | openapi.yaml#listAttachments, files.md#5 | `TestListAttachments_NoDownloadUrl` | 前提: `report_draft` + `att_draft_jpeg`、認証: Test Member（所有者） | 200 OK。レスポンス body の各要素に `download_url` フィールドが含まれない（files.md §5） | 高 |
...
| テストID | テストレベル | レイヤー | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 | 優先度 |
|---------|------------|--------|---------|-----------|-----------|--------------|-----------------|---------|--------|
| ATT-030 | 統合 | handler | 正常系 | ATT-F03, ATT-010 | openapi.yaml#getAttachmentDownload, files.md#署名付きURL | `TestGetAttachmentDownload_Success_Owner` | 前提: `report_draft` + `att_draft_jpeg`、認証: Test Member（所有者） | 200 OK。`data.download_url`（S3署名付きURL）、`data.file_name`（元ファイル名）、`data.mime_type`（image/jpeg）、`data.file_size`、`data.expires_at` を含む | 高 |
...
| ATT-025 | listAttachments | 正常系（download_urlなし） | 正常系 | ATT-F02 | openapi.yaml#listAttachments, files.md#5 | 高 |
...
| `useAttachmentDownload` | `GET /api/reports/:id/items/:itemId/attachments/:attId` | `55_ui_component/state-management.md §useAttachmentDownload` |
...
- `55_ui_component/state-management.md` -- useAttachments, useAttachmentDownload, useUploadAttachment, useDeleteAttachment の入出力・クエリキー・キャッシュ無効化
...
| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| ATT-FE-007 | 単体 | AttachmentList | `attachments: Attachment[]` | 正常系 | ATT-F02 | `55_ui_component/screens/report-detail.md §AttachmentList` | `renders_attachment_list_with_file_info` | Props: `attachments=[{ id: "att-1", file_name: "receipt.jpg", file_size: 245760, mime_type: "image/jpeg" }]`, `canDelete=true`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId=null` | ファイル名「receipt.jpg」とファイルサイズが表示される |
| ATT-FE-008 | 単体 | AttachmentList | `attachments: Attachment[]` | 正常系 | ATT-F02 | `55_ui_component/screens/report-detail.md §AttachmentList` | `renders_empty_list` | Props: `attachments=[]`, `canDelete=true`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId=null` | 添付ファイルの行要素が描画されない |
| ATT-FE-009 | 単体 | AttachmentList | `attachments: Attachment[]` | 正常系 | ATT-F02 | `55_ui_component/screens/report-detail.md §AttachmentList` | `renders_multiple_attachments` | Props: `attachments=[att-1, att-2, att-3]`（3件）、`canDelete=true`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId=null` | 3件の添付ファイルがそれぞれファイル名・ファイルサイズとともに表示される |
| ATT-FE-010 | 単体 | AttachmentList | `onDownload: (attachmentId: string) => void` | 正常系 | ATT-F03 | `55_ui_component/screens/report-detail.md §AttachmentList` | `calls_onDownload_on_filename_click` | Props: `attachments=[{ id: "att-1", file_name: "receipt.jpg", ... }]`, `canDelete=true`, `onDownload=mockFn`, `onDelete=jest.fn()`, `deletingId=null`。ファイル名「receipt.jpg」をクリック | `onDownload` が `"att-1"` を引数として呼び出される |
| ATT-FE-011 | 単体 | AttachmentList | `canDelete: boolean` | 認可 | ATT-020 | `55_ui_component/screens/report-detail.md §AttachmentList` | `shows_delete_button_when_canDelete_true` | Props: `attachments=[att-1]`, `canDelete=true`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId=null` | 各添付行に削除ボタンが表示される |
| ATT-FE-012 | 単体 | AttachmentList | `canDelete: boolean` | 認可 | ATT-020 | `55_ui_component/screens/report-detail.md §AttachmentList` | `hides_delete_button_when_canDelete_false` | Props: `attachments=[att-1]`, `canDelete=false`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId=null` | 削除ボタンが表示されない。ファイル名のダウンロードリンクは表示される |
| ATT-FE-013 | 単体 | AttachmentList | `onDelete: (attachmentId: string) => void` | 正常系 | ATT-F04 | `55_ui_component/screens/report-detail.md §AttachmentList` | `calls_onDelete_on_delete_button_click` | Props: `attachments=[{ id: "att-1", ... }]`, `canDelete=true`, `onDownload=jest.fn()`, `onDelete=mockFn`, `deletingId=null`。削除ボタンをクリック | `onDelete` が `"att-1"` を引数として呼び出される |
| ATT-FE-014 | 単体 | AttachmentList | `deletingId: string \| null` | 正常系 | ATT-F04 | `55_ui_component/screens/report-detail.md §AttachmentList` | `greys_out_deleting_attachment` | Props: `attachments=[{ id: "att-1", ... }, { id: "att-2", ... }]`, `canDelete=true`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId="att-1"` | `att-1` の行がグレーアウト表示される。`att-2` の行は通常表示 |
| ATT-FE-015 | 単体 | AttachmentList | `deletingId: string \| null` | 正常系 | ATT-F04 | `55_ui_component/screens/report-detail.md §AttachmentList` | `disables_delete_button_for_deleting_attachment` | Props: `attachments=[{ id: "att-1", ... }]`, `canDelete=true`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId="att-1"` | `att-1` の削除ボタンが disabled になる |
...
### 7-5. Hook テスト（useAttachmentDownload）

| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| ATT-FE-033 | 単体 | - | `useAttachmentDownload` | 正常系 | ATT-F03, ATT-010 | `55_ui_component/state-management.md §useAttachmentDownload` | `useAttachmentDownload_returns_signed_url` | Hook 引数: `{ reportId: "rpt-1", itemId: "item-1", attId: "att-1" }`。API モック: 200 OK、`data: { download_url: "https://s3.example.com/...", file_name: "receipt.jpg", mime_type: "image/jpeg", file_size: 245760, expires_at: "..." }` | `data.data.download_url` が署名付き URL を含む |
| ATT-FE-034 | 単体 | - | `useAttachmentDownload` | 正常系 | ATT-012 | `55_ui_component/state-management.md §useAttachmentDownload` | `useAttachmentDownload_staleTime_is_zero` | Hook 引数: `{ reportId: "rpt-1", itemId: "item-1", attId: "att-1" }` | staleTime が 0 であること（署名付き URL は毎回取得） |
| ATT-FE-035 | 単体 | - | `useAttachmentDownload` | 異常系 | ATT-F03 | `55_ui_component/state-management.md §useAttachmentDownload` | `useAttachmentDownload_handles_404` | Hook 引数: `{ reportId: "rpt-1", itemId: "item-1", attId: "nonexistent" }`。API モック: 404 RESOURCE_NOT_FOUND | `isError` が true。`error.status` が 404 |
...
| ATT-FE-049 | 統合 | AttachmentArea | `useAttachmentDownload` | 正常系 | ATT-F03, ATT-010 | `55_ui_component/screens/report-detail.md §AttachmentArea` | `download_opens_signed_url` | Props: `reportId="rpt-1"`, `itemId="item-1"`, `canModify=true`。添付1件。ファイル名をクリック。API モック: 200 OK、`data.download_url` に署名付き URL | 署名付き URL でブラウザがファイルを開く/ダウンロードする |
...
| ATT-FE-010 | AttachmentList | 正常系（ダウンロードコールバック） | 高 |
...
| ATT-FE-033 | useAttachmentDownload | 正常系（署名付き URL 取得） | 高 |
| ATT-FE-034 | useAttachmentDownload | 正常系（staleTime=0 検証） | 中 |
| ATT-FE-035 | useAttachmentDownload | 異常系（404 ハンドリング） | 高 |
...
| ATT-FE-049 | AttachmentArea 統合 | 正常系（ダウンロード署名付き URL） | 高 |
...
#### useAttachmentDownload -- 403 ハンドリング

| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| ATT-FE-035b | 単体 | - | `useAttachmentDownload` | 異常系 | ATT-F03, ATT-011 | `55_ui_component/state-management.md §useAttachmentDownload` | `useAttachmentDownload_handles_403` | Hook 引数: `{ reportId: "rpt-1", itemId: "item-1", attId: "att-1" }`。API モック: 403 FORBIDDEN | `isError` が true。`error.status` が 403 |
...
- **Hook モック**: コンポーネント単体テスト（ATT-FE-001〜028）では `useAttachments`, `useUploadAttachment`, `useDeleteAttachment`, `useAttachmentDownload` を vi.mock でモックする
...
- `useAttachmentDownload` の staleTime=0 検証（ATT-FE-034）は QueryClient のクエリキャッシュ設定を直接確認する
