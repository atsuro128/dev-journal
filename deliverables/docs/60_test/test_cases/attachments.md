...
| テストID | テストレベル | 対象コンポーネント | 対象 Props / Hook | 保証種別 | 対応要件ID | 対応設計ID | テスト関数名候補 | 入力（前提条件含む） | 期待結果 |
|---|---|---|---|---|---|---|---|---|---|
| ATT-FE-007 | 単体 | AttachmentList | `attachments: Attachment[]` | 正常系 | ATT-F02 | `55_ui_component/screens/report-detail.md §AttachmentList` | `renders_attachment_list_with_file_info` | Props: `attachments=[{ id: "att-1", file_name: "receipt.jpg", file_size: 245760, mime_type: "image/jpeg" }]`, `canDelete=true`, `onPreview=jest.fn()`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId=null` | ファイル名「receipt.jpg」とファイルサイズが表示される |
| ATT-FE-008 | 単体 | AttachmentList | `attachments: Attachment[]` | 正常系 | ATT-F02 | `55_ui_component/screens/report-detail.md §AttachmentList` | `renders_empty_list` | Props: `attachments=[]`, `canDelete=true`, `onPreview=jest.fn()`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId=null` | 添付ファイルの行要素が描画されない |
| ATT-FE-009 | 単体 | AttachmentList | `attachments: Attachment[]` | 正常系 | ATT-F02 | `55_ui_component/screens/report-detail.md §AttachmentList` | `renders_multiple_attachments` | Props: `attachments=[att-1, att-2, att-3]`（3件）、`canDelete=true`, `onPreview=jest.fn()`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId=null` | 3件の添付ファイルがそれぞれファイル名・ファイルサイズとともに表示される |
| ATT-FE-010 | 単体 | AttachmentList | `onPreview: (attachmentId: string) => void` | 正常系 | ATT-F03 | `55_ui_component/screens/report-detail.md §AttachmentList` | `calls_onPreview_on_filename_click` | Props: `attachments=[{ id: "att-1", file_name: "receipt.jpg", ... }]`, `canDelete=true`, `onPreview=mockFn`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId=null`。ファイル名「receipt.jpg」をクリック | `onPreview` が `"att-1"` を引数として呼び出される |
| ATT-FE-010b | 単体 | AttachmentList | `onDownload: (attachmentId: string) => void` | 正常系 | ATT-F03 | `55_ui_component/screens/report-detail.md §AttachmentList` | `calls_onDownload_on_download_icon_click` | Props: `attachments=[{ id: "att-1", file_name: "receipt.jpg", ... }]`, `canDelete=true`, `onPreview=jest.fn()`, `onDownload=mockFn`, `onDelete=jest.fn()`, `deletingId=null`。↓ アイコンをクリック | `onDownload` が `"att-1"` を引数として呼び出される |
| ATT-FE-011 | 単体 | AttachmentList | `canDelete: boolean` | 認可 | ATT-020 | `55_ui_component/screens/report-detail.md §AttachmentList` | `shows_delete_button_when_canDelete_true` | Props: `attachments=[att-1]`, `canDelete=true`, `onPreview=jest.fn()`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId=null` | 各添付行に削除ボタンが表示される |
| ATT-FE-012 | 単体 | AttachmentList | `canDelete: boolean` | 認可 | ATT-020 | `55_ui_component/screens/report-detail.md §AttachmentList` | `hides_delete_button_when_canDelete_false` | Props: `attachments=[att-1]`, `canDelete=false`, `onPreview=jest.fn()`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId=null` | 削除ボタンが表示されない。ファイル名のダウンロードリンクは表示される |
| ATT-FE-013 | 単体 | AttachmentList | `onDelete: (attachmentId: string) => void` | 正常系 | ATT-F04 | `55_ui_component/screens/report-detail.md §AttachmentList` | `calls_onDelete_on_delete_button_click` | Props: `attachments=[{ id: "att-1", ... }]`, `canDelete=true`, `onPreview=jest.fn()`, `onDownload=jest.fn()`, `onDelete=mockFn`, `deletingId=null`。削除ボタンをクリック | `onDelete` が `"att-1"` を引数として呼び出される |
| ATT-FE-014 | 単体 | AttachmentList | `deletingId: string \| null` | 正常系 | ATT-F04 | `55_ui_component/screens/report-detail.md §AttachmentList` | `greys_out_deleting_attachment` | Props: `attachments=[{ id: "att-1", ... }, { id: "att-2", ... }]`, `canDelete=true`, `onPreview=jest.fn()`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId="att-1"` | `att-1` の行がグレーアウト表示される。`att-2` の行は通常表示 |
| ATT-FE-015 | 単体 | AttachmentList | `deletingId: string \| null` | 正常系 | ATT-F04 | `55_ui_component/screens/report-detail.md §AttachmentList` | `disables_delete_button_for_deleting_attachment` | Props: `attachments=[{ id: "att-1", ... }]`, `canDelete=true`, `onPreview=jest.fn()`, `onDownload=jest.fn()`, `onDelete=jest.fn()`, `deletingId="att-1"` | `att-1` の削除ボタンが disabled になる |
...
| ATT-FE-049 | 統合 | AttachmentArea | `useAttachmentPreviewUrl` / `useAttachmentDownloadUrl` | 正常系 | ATT-F03, ATT-010 | `55_ui_component/screens/report-detail.md §AttachmentArea` | `preview_and_download_open_signed_url` | Props: `reportId="rpt-1"`, `itemId="item-1"`, `canModify=true`。添付1件。1. ファイル名をクリック: API モック 200 OK、`data.url` に署名付き URL（inline）。2. ↓ アイコンをクリック: API モック 200 OK、`data.url` に署名付き URL（attachment） | 1. プレビュー: `window.open('about:blank')` 後に `location.href` が署名付き URL に差し替えられる。2. ダウンロード: 同様のパターンでダウンロードが開始される |
