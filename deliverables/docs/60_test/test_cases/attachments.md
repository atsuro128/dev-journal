...
| ATT-FE-010b | 単体 | AttachmentList（内部の AttachmentItemRow） | `useAttachmentDownloadUrl` | 正常系 | ATT-F03 | `55_ui_component/screens/report-detail.md §AttachmentList`, `50_detail_design/files.md §4.5` | `opens_download_via_window_open_on_download_icon_click` | Props: `reportId="rpt-1"`, `itemId="item-1"`, `attachments=[{ id: "att-1", file_name: "receipt.jpg", ... }]`, `canDelete=true`, `onDelete=jest.fn()`, `deletingId=null`, `onError=jest.fn()`。`useAttachmentDownloadUrl` をモックし `refetch` が `{ data: { data: { url: "https://s3.example.com/..." } } }` を返すように設定。`window.open` をスパイし `newWindow` モックを返すよう設定。↓ アイコンをクリック | `window.open('about:blank', '_blank')` がクリック同期で呼ばれ、続いて `useAttachmentDownloadUrl` の `refetch()` が 1 回呼ばれる。取得成功後に `newWindow.location.href` が取得した署名付き URL に差し替えられる |
...
| ATT-FE-049 | 統合 | AttachmentArea + AttachmentList（内部 AttachmentItemRow） | `useAttachmentPreviewUrl` / `useAttachmentDownloadUrl` | 正常系 | ATT-F03, ATT-010 | `55_ui_component/screens/report-detail.md §AttachmentList`, `50_detail_design/files.md §4.5` | `preview_and_download_open_signed_url` | Props: `reportId="rpt-1"`, `itemId="item-1"`, `canModify=true`。添付1件。`window.open` をスパイし `newWindow` モックを返すよう設定。1. ファイル名をクリック: プレビュー API モック 200 OK、`data.url` に署名付き URL（inline）。2. ↓ アイコンをクリック: ダウンロード API モック 200 OK、`data.url` に署名付き URL（attachment） | 1. プレビュー: `window.open('about:blank', '_blank')` がクリック同期で呼ばれ、AttachmentItemRow 内の `useAttachmentPreviewUrl.refetch()` 成功後に `newWindow.location.href` が署名付き URL に差し替えられる。2. ダウンロード: `useAttachmentDownloadUrl.refetch()` で同様のパターンでダウンロードが開始される |
...
| ATT-FE-049 | AttachmentArea + AttachmentList 統合 | 正常系（プレビュー・ダウンロード署名付き URL、クリック同期 window.open） | 高 |
