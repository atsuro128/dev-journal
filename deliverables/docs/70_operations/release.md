...
# d. systemd unit のイメージタグ参照を更新（または環境変数化済みなら不要）
$ sudo sed -i 's|expense-saas:.*|expense-saas:<TAG>|' /etc/systemd/system/expense-app.service
...
| 2 | EC2 上のアプリコンテナが正常に起動している | SSM 経由で EC2 に接続し `sudo docker ps` で expense-app コンテナが Up 状態であることを確認、`sudo systemctl status expense-app` で active (running) を確認 | [ ] |
| 3 | 旧コンテナが停止している | `sudo docker ps -a` で expense-app の旧コンテナが Exited (0) で停止していることを確認 | [ ] |
...
| # | ロールバック条件 | 判断基準 |
|---|----------------|---------|
| (a) | `systemctl restart` が連続失敗する | EC2 上で `sudo systemctl restart expense-app` を 3 回連続実行しても `sudo systemctl status expense-app` が `active (running)` にならない |
...
4. systemd unit のイメージタグ参照を前バージョンに戻す
   $ sudo sed -i 's|expense-saas:.*|expense-saas:<前TAG>|' /etc/systemd/system/expense-app.service
   $ sudo systemctl daemon-reload
...
6. ロールバック完了確認
   $ sudo systemctl status expense-app   # active (running) であること
   $ sudo docker ps                      # expense-app コンテナが Up 状態
   $ curl -fsS http://localhost:8080/health
