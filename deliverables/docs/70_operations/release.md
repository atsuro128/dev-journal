...
| # | 確認項目 | 確認方法 | 合否 |
|---|---------|---------|------|
| 1 | CI パイプラインが全て通過している | GitHub Actions の最新ワークフロー実行結果を確認 | [ ] |
| 2 | lint エラーが 0 件である | CI の lint ステップの結果を確認 | [ ] |
| 3 | 全テストが通過している | CI の test ステップの結果を確認 | [ ] |
| 4 | Docker イメージのビルドが成功している | CI の build ステップの結果を確認 | [ ] |
...
1. 本番 DB へのマイグレーション実行
   マイグレーション用の踏み台（bastion）またはCI ジョブから実行する。
...
GitHub Actions の CI/CD パイプラインを通じて ECS Fargate にデプロイする。

```
[デプロイフロー]

1. GitHub Actions ワークフローのトリガー
   - main ブランチへのマージ時に自動実行、または手動トリガー

2. CI パイプライン（自動実行）
   a. コードチェックアウト
   b. Go lint（golangci-lint）
   c. Go テスト実行
   d. フロントエンドビルド（npm run build）
   e. Docker イメージビルド（Go バイナリ + SPA 静的ファイル embed）
   f. ECR へのイメージプッシュ
...
- [x] デプロイ手順が GitHub Actions + ECS ローリングアップデートの構成と整合している（architecture.md / ADR-0004）
