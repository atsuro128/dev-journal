...
| `S3_REGION` | S3 リージョン | string | `ap-northeast-1` | `ap-northeast-1` | `ap-northeast-1` |
...
2. 新しい鍵ペアに kid（Key ID）を付与する
   - kid は UUID v4 で生成する
   - 例: kid = "a1b2c3d4-e5f6-7890-abcd-ef1234567890"

3. Secrets Manager のシークレットを更新する
   新鍵を追加し、旧鍵を猶予期間用に保持する。

   $ aws secretsmanager update-secret \
       --secret-id expense-saas/prod/jwt-keys \
       --secret-string '{
         "JWT_PRIVATE_KEY": "<new_private_key_pem>",
         "JWT_PUBLIC_KEY": "<new_public_key_pem>",
         "JWT_PUBLIC_KEY_PREVIOUS": "<old_public_key_pem>"
       }'

4. ECS サービスを再デプロイして新しいシークレットを読み込む
   $ aws ecs update-service \
       --cluster expense-saas-prod \
       --service expense-saas-api \
       --force-new-deployment

5. 猶予期間（7 日間）の間、旧公開鍵での検証を継続する
   - アプリケーションは JWT ヘッダーの kid を参照し、対応する公開鍵で検証する
...
- [x] JWT 鍵ペアのローテーション手順がコマンドレベルで記載されている（security.md SS2.1 と整合）
