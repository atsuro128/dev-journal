...
Step 11-A で PR #57 により `useDeleteAttachment` 実装・テストを対称化（issue 099 対応）。設計テーブル `state-management.md` L303 側も本セッションで更新し、Upload / Delete 両方の invalidation が対称になった。

- `state-management.md` L303: `useDeleteAttachment` の invalidation に `['reports', reportId, 'items', itemId, 'attachments']` を追加
- 実装・テストは PR #57 に含まれる（別コミット）

これで上記差し戻しレビューの指摘事項（設計書・実装・テストの対称化）が全て解消された。
