...
### dev-journal（設計成果物、2 コミット）

- `5b624fd`: §AppDataGrid にカスタムフッター項追加 + §AppPaginationFooter 配置仕様を `slots.footer` 経由統合に全面改訂 + 4 画面 detail 整合 + progress.md 残存 issue 表に #147 再掲 + review-findings/112（progress.md 文言整合）resolved
- `d16a410`: テストケース ID 採番追加（RPT-FE-115/116, TNT-FE-052/053, ADG-001..004）+ サマリ更新（FE 合計 138→144 / 総合計 229→235）

### expense-saas（実装 + テスト、squash merge: PR #102 = `e3142c3`）

**実装変更**:
- `AppDataGrid.tsx`: `{...rest}` を JSX 先頭に移動し、明示 props (`slots`/`sx`/`hideFooterPagination`) が後勝ちになる構造に修正（D-1 対応）
- `AppPagination.tsx`: 内側 `mt: 2` を撤去（codex warning 対応、外側マージン一元化）
- `AppPaginationFooter.tsx`: 外側マージン `mt={2}` を撤去
- `ReportListTable.tsx` / `AllReportsTable.tsx`: `paginationFooter?: ReactNode` prop 追加 + 空状態早期 return 撤去 + `slots.noRowsOverlay` に EmptyState を渡す形に変更（B-1 対応）
- `ReportListPage.tsx` / `AllReportsPage.tsx`: テーブル外の `<AppPaginationFooter />` を撤去し、中間ラッパーの `paginationFooter` prop に渡す形に変更
- `ApprovalListPage.tsx` / `PaymentListPage.tsx`: AppDataGrid 直接利用で `slots={{ footer: () => <AppPaginationFooter ... /> }}` を渡す形に変更

**テスト**:
- 新規: `AppDataGrid.test.tsx`（ADG-001〜004: slots 合成挙動検証）
- 既存追記: `ReportListTable.test.tsx`（RPT-FE-115/116）、`AllReportsTable.test.tsx`（TNT-FE-052/053）
- 既存追従: `ReportListPage.test.tsx` / `AllReportsPage.test.tsx` / `ApprovalListPage.test.tsx` / `PaymentListPage.test.tsx`（AppDataGrid モックの slots.footer / noRowsOverlay 描画対応）

### レビュー履歴

- 内部 reviewer: 2 回 FIX → PASS（B-1 / D-1 対応後）
- codex review: 1 回 warning（AppPagination mt 残存）→ 修正 → PASS

### ローカル CI 最終結果

- lint: 0 errors / 1 既存 warning
- tsc: 0 errors
- vitest: 710/710 PASS / 99 ファイル
- build: OK

### 残対応

- SMK-081（ページ送り）再実施 — マージ後のユーザー作業
- BE テスト（本 PR は FE のみ変更のため省略可、ユーザー判断）

## 学び

- 「テーブルフッター」は MUI Table の `<TableFooter>` だけでなく DataGrid の `slots.footer`（`<div class="MuiDataGrid-footerContainer">`）も指す概念。設計書では DOM 構造レベルまで明文化しないと実装が乖離する
- `{...rest}` 順序（JSX）の後勝ち仕様は既存ライブラリラッパーで定常的に問題化する。slots/sx 合成は明示構築 → 末尾に rest を置くアンチパターンを避け、明示 props を後置するパターンを採用すべき
- 共通コンポーネントの API 拡張（中間ラッパーの `paginationFooter` prop）は単体テストでの prop 受け渡し検証 + 空状態リグレッション防止テストをセットで実装する
- 確定方針が再協議で変わった場合（TableFooter 案 → D-1 + ②a 案）、issue 本文・progress.md・設計書の三点同期を怠ると後続レビューで指摘される（本セッション codex 指摘 #112）

## 2026-04-27 再々オープン（PR #102 マージ後の SMK-081 で見栄え崩れ）

### 再々オープン理由

PR #102 マージ後の SMK-081 検証で、AppPaginationFooter の見栄え問題が発覚:

1. **フッターとリストの境界線が消失** — DataGrid 標準フッターには `border-top` があるが、`slots.footer` で差し込んだ独自 AppPaginationFooter には未指定
2. **PageSizeSelector の MUI Select 枠線がフッター高さを支配** — Select のデフォルトサイズがフッター全体の高さを引き上げている

完了条件「SMK-081 を再実施し PASS であること」が **未達** のまま resolved に移動してしまっていたため、再々オープン。

**指揮役の運用ミス**: PR マージ後すぐに resolved 移動 + progress.md 削除 + コミットを実施したが、SMK-081 PASS 確認を順序として **後回しにした** ため、検証で FAIL したにもかかわらず一旦解決扱いになる事態を招いた。本来は「PR マージ → SMK-081 PASS 確認 → resolved 移動」の順で進めるべきだった（教訓: 完了条件にユーザー手動検証が含まれる場合は、検証前に解決扱いしない）。

### ユーザー疑問への回答

ユーザーから「ページング/表示件数をフッターに入れる UI は一般的ではないか」と疑問が呈されたが、**一般的**。MUI X DataGrid のデフォルトフッター（`slots.footer` 未指定時）は標準で TablePagination（件数表示 + ページサイズ選択 + ページ送り）を含んでおり、本プロジェクトの AppPaginationFooter も思想は同じ。問題は実装の見栄え調整不足。

### 確定方針（A2 案、2026-04-27 ユーザー協議 + codex 相談で確定）

**「独自実装は維持しつつ、見た目を MUI 標準に寄せて粗を消す」**

過去の方針検討で **MUI 標準（TablePagination + paginationModel + pageSizeOptions）採用案が比較対象から漏れていた** ことが本セッションで判明。codex 相談の結果、以下の理由で A2 が推奨と確認:

- 不具合は設計破綻ではなく見た目調整不足（境界線消失 + Select 高さ支配は副作用）
- B 案（MUI 標準完全置換）は単なる UI 差し替えではなく、設計書・テスト・中間ラッパーの責務をかなり巻き戻す（PSS / APF / ADG / RPT-FE-111..116 / TNT-FE-048..053 等のテスト全面移行）
- 独自要件（番号型 UI / 中央+右レイアウト / 動的選択肢 / URL 双方向 / 常時表示）は B 案でも完全な no-custom にはならない
- MVP では「標準を使ったか」より「説明可能で安定して見えるか」の方が重要

### 追加対応内容

#### 1. 実装修正（共通コンポーネント側のみ、4 画面・中間ラッパーは原則変更不要）

- `frontend/src/components/ui/AppPaginationFooter.tsx`:
  - ルート Box に `borderTop: '1px solid'` + `borderColor: 'divider'` を追加（DataGrid 標準に揃える）
  - `minHeight` 設定（MUI 標準フッター高さ 52px 相当）+ `px` / `py` 調整で Select 枠線に高さを支配されないようにする
  - 件数表示「1-10 / 37 件」相当を左側に追加（標準っぽさ復元、表示ロジックは props で受ける形を検討。例: `currentRange?: { start: number; end: number; total: number }` 等）
- `frontend/src/components/ui/PageSizeSelector.tsx`:
  - 既存の `size="small"` を維持しつつ、必要なら `variant` 調整（`variant="standard"` 検討）/ ラベル位置・余白調整で薄い見え方にする

#### 2. 4 画面（呼び出し側）への波及（最小限）

件数表示（`currentRange`）を新 prop で受ける場合、4 画面で `pagination.total_count` / `current_page` / `per_page` から算出して渡す処理を追加。中間ラッパー（ReportListTable / AllReportsTable）にも同 prop を pass-through する形になる可能性あり。

#### 3. 設計書改訂

- `dev-journal/deliverables/docs/55_ui_component/common-components.md`:
  - §AppPaginationFooter §レイアウト仕様: 「境界線（border-top）」「最小高さ（minHeight）」「件数表示」を追加明記
  - §PageSizeSelector: variant / size / 余白方針を明記
- 4 画面 detail（`screens/{report-list, admin-all-reports, workflow-pending, workflow-payable}.md`）: §ページネーション に件数表示の仕様を追記（4 画面で同じ表記方針を共通化）
- `state-management.md`: `pagination.total_count` を件数表示に流す経路を §3.1 に追記（必要なら）

#### 4. テスト

- `AppPaginationFooter.test.tsx`（APF-001〜007）: 既存ケース維持、必要なら以下を追加
  - APF-008: 境界線が描画される（`borderTop` style 検証）
  - APF-009: 件数表示が `props.currentRange` から正しく描画される
- `PageSizeSelector.test.tsx`（PSS-001〜005）: 既存ケース維持、`size="small"` / `variant` の変更が表示に影響していないか軽く確認

### 完了条件（追加）

- SMK-081 を再実施し PASS（境界線あり / フッター高さが Select に支配されない / 件数表示が表示される / 4 画面のフッター見栄えが MUI 標準に近い）
- common-components.md §AppPaginationFooter / §PageSizeSelector が新仕様を反映
- 4 画面 detail に件数表示仕様が明記される
- 既存テスト + 追加テスト（APF-008/009 等）がローカル CI で PASS
- **SMK-081 PASS を確認してから resolved 移動を実施する**（運用ミス再発防止）

### Post-MVP 検討事項（保留）

将来、画面が増えるか DataGrid 標準寄せを全体方針にする場合、B 案（MUI 標準 TablePagination + paginationModel + pageSizeOptions + slotProps.basePagination カスタム）への移行を検討する。codex 提案の参考実装スケッチ:

```tsx
const paginationModel = { page: page - 1, pageSize: perPage };
const pageSizeOptions = Array.from(new Set([10, 20, 50, 100, perPage])).sort((a, b) => a - b);
