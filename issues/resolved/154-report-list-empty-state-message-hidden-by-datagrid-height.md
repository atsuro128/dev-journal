...
PR #114 の minHeight: 300 で EmptyState が見切れることを修正（PR #117 で 300 → 400）したが、ユーザー観察で 400 は overlayWrapperInner 275px に対し EmptyState 236.5px が centering で配置され上下 19.25px の余白が出るため過大と判明。

PR #118 で `--DataGrid-overlayHeight: 'auto'` による auto-fit を試みたが、MUI X v8 内部で `auto` が `var(--DataGrid-overlayHeight, calc(var(--height) * 2))` の数値計算文脈で無効値となり fallback (104px) に落ち、EmptyState が見切れて表が「ぺちゃんこ」になることが判明。

PR #120 で PR #118 を git revert + minHeight: 400 → 361 に再調整した。

### 採用方針（PR #120）

minHeight: rows.length === 0 ? 361 : undefined

361 = ColumnHeader 56 + EmptyState 必要量 236.5 + filler 15 + AppPaginationFooter 53 = 360.5 を丸めた値。overlayWrapperInner と EmptyState の高さが一致し centering slack 0、button-footer 間の視覚余白は EmptyState 自身の py-bottom 48px で確保される。

### 検証

SMK-006 #4 を 2026-05-02 ローカルで再々検証 → **PASS**（EmptyState 全文視認、ボタン-フッター間に自然余白あり）

### 関連 PR

- PR #117 (95fe7f3): minHeight 300 → 400
- PR #118 (b80c6c8): minHeight 削除 + `--DataGrid-overlayHeight: 'auto'` （機能せず）
- PR #120 (586523a): PR #118 revert + minHeight: 400 → 361
