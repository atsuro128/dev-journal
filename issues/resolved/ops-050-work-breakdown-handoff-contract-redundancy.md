...
提案どおり「次 Step への受け渡し契約」セクションを全ファイルから廃止した。

- work-breakdown テンプレート: セクション本体 + 必須セクション表の該当行を削除
- 全 step ファイル（pre-step, step0〜step11）: 該当セクションを削除（計14ファイル）
- step4 は「Step 5 への受け渡し契約」「次 Step への受け渡し契約」の2セクションを削除

依存関係の正本は各 Step の「上流入力」セクションに一元化された。
