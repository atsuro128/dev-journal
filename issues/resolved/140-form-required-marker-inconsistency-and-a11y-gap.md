...
採用方針: 案 A（全コンポーネントで `*` 抑止 + 全フォームに `required` prop 統一付与、HTML5 `required` 属性は input 要素にのみ付与）。

PR #99（squash commit `12b3d22`）で expense-saas 側 13 ファイル（共通 2 + フォーム 5 + テスト 6）を修正:

共通コンポーネント:
- `frontend/src/components/ui/AppTextField.tsx`: `required` を分割代入し `slotProps.htmlInput.required` 経由で input にのみ付与（MUI 自動 `*` 抑止）。旧 `inputProps` API も分割代入してマージ
- `frontend/src/components/ui/AppSelect.tsx`: `<FormControl required>` を削除し `Select.inputProps={{ required, 'aria-required': true }}` で input にのみ付与（aria-required 連携を二重指定で確保）

フォーム:
- `frontend/src/pages/reports/ItemForm.tsx`: 4 フィールド（日付 / 金額 / カテゴリ / 摘要）に `required` 追加
- `frontend/src/pages/login/LoginForm.tsx`: 2 フィールド
- `frontend/src/pages/signup/SignupForm.tsx`: 4 フィールド
- `frontend/src/pages/password-reset/PasswordResetForm.tsx`: 2 フィールド
- `frontend/src/pages/password-reset/PasswordResetRequestForm.tsx`: 1 フィールド

テスト:
- 各フォームの `__tests__` に `expect(...).toBeRequired()` 追加（6 ファイル）
- `ItemForm.test.tsx` に AppSelect（カテゴリ）の `toBeRequired()` + `toHaveAttribute('aria-required', 'true')` 回帰検証追加（reviewer 指摘 warning-1 対応、commit `c8f3df3`）

dev-journal 側 4 ファイルを commit `f0d6ace` で修正:
- `deliverables/docs/50_detail_design/ui-guidelines.md` §7「必須フィールド」: `required` prop は input 要素にのみ付与・ラベル `*` 抑止方針に書き換え
- `deliverables/docs/50_detail_design/ui-guidelines.md` §バリデーションエラー表示: コード例の `TextField` を `AppTextField` に置換
- `deliverables/docs/50_detail_design/screens/{report-create, report-edit, report-detail}.md`: レイアウト図の必須マーカー `*` 削除（テーブルの「必須」列は仕様情報として維持）

レビュー: 内部レビュー FIX（warning-1: AppSelect 回帰検証不足）→ 修正 `c8f3df3` → 再レビュー PASS + codex レビュー APPROVE 相当（実 DOM で aria-required 連携確認済み、self-PR のため `--comment` で投稿）。
