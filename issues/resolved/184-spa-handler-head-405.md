...
PR #150（expense-saas、squash commit `dd480b5`）でマージ完了。

### 実装変更

- `cmd/server/main.go`: SPA fallback ルーティングを GET 単独から GET+HEAD 両対応に拡張。
  - `r.Get("/*", spaHandler)` → `r.Method(http.MethodGet, "/*", ...)` + `r.Method(http.MethodHead, "/*", ...)` 相当の対応
  - `r.NotFound(spaHandler)` 経路も HEAD を考慮
- `HEAD /health` がヘルスハンドラ経由になるよう、ヘルスエンドポイントを GET+HEAD で個別登録（SPA fallback が `/health` の HEAD を奪って `index.html` を返してしまう問題を回避）
- `internal/spa/handler_test.go` に回帰テスト 5 件を追加（GET / HEAD / NotFound / /health / /assets）
...
### Step 11-E Phase 6 への影響

`/health` HEAD が `index.html` を返す副次バグも同時解消。Phase 6 ALB ヘルスチェック (GET /health) はもともと問題なかったが、将来 HEAD ベース監視を導入した際の汚染も予防。
