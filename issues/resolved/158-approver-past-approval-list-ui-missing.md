...
**採用方針**: 案 2 改（新規履歴画面追加、SCR-WFL-003）

**設計判断（確定）**:
- 新エンドポイント `GET /api/workflow/processed`（既存 `/api/workflow/pending` は無変更）
- 認可スコープ: `tenant_id = ? AND (approved_by = actor.user_id OR rejected_by = actor.user_id)`、Approver のみ
- URL: `/approvals/processed`、画面 ID: **SCR-WFL-003**（空き番号活用、既存 SCR-WFL-001/002 への参照は一切変更しない）
- 表示カラム: 申請者名 / タイトル / 合計金額 / 処理結果バッジ（承認/却下） / 処理日 / 現在ステータスバッジ / 遷移アイコン
- ソート: 処理日時 DESC（同値時 report_id DESC）
- サイドメニュー: Approver の「処理済み」を「承認待ち」直下に追加

**設計書改訂** (commit `790e558`):
- `40_basic_design/screens.md` §3.4 に SCR-WFL-003 行追加、§3.6 / §4.3 / §4.7 / §4.9 / §5 整合更新
- `50_detail_design/authz.md` §10.2 / §10.3 に処理済み一覧 scope 追記（§10.1 は既存表記が内包するため改訂せず）
- `50_detail_design/openapi.yaml` に `/api/workflow/processed` パスと `ProcessedReport` schema 追加
- `50_detail_design/screens/workflow-processed.md` 新規作成
- `60_test/manual_checklists/smoke_check.md` 新章 §4.12（SMK-103〜105）追加
- `55_ui_component/common-components.md` / `state-management.md` に SCR-WFL-003 / `useProcessedReports` を追記のみ

**BE 実装** (PR #111, commit `722568e` 実装 / `4d82545` テスト):
- `internal/handler/workflow.go`: `ListProcessedReports` ハンドラ + ルーター登録（Approver RBAC）
- `internal/service/workflow_service.go`: `ListProcessedReports` サービス + `ProcessedReport` DTO（decision / decided_at / current_status）
- `db/queries/expense_reports.sql` + sqlcgen: `ListProcessedReports` / `CountProcessedReports` クエリ
- `internal/repository/postgres/report_repo.go`: `ListProcessed` 実装
- 統合テスト WFL-064〜075 全 12 ケース（テナント分離 / RBAC / scope / 他 Approver 非表示 / current_status / ソート / ページネーション）

**FE 実装** (PR #111, commit `d1e8e37` 実装 / `b4993f0` テスト):
- 新規 Page `ProcessedReportsPage.tsx`（`ApprovalListPage.tsx` 雛形）
- `App.tsx` ルート `/approvals/processed` 追加（Approver ロールガード）
- `AppSidebar.tsx`（layout）に Approver 用「処理済み」項目追加（HistoryIcon、「承認待ち」直下）
- `useReports.ts` に `useProcessedReports` hook 追加（既存 hook は無変更）
- `api/types.ts` に `ProcessedReport` 型追加
- WFL-FE-061〜069 全 10 ケースの Vitest テスト

**SMK 追加項目**: SMK-103（基本表示）/ SMK-104（テナント分離）/ SMK-105（他 Approver 処理分非表示）

**残課題（後続判断、本 issue スコープ外）**: Admin / Accounting への画面開放（要件再確認時に別 issue 候補）
...
### 解決根拠
PR #111 マージ後に SCR-WFL-003（処理済みレポート一覧画面）が実装済み。2026-05-02 セッションで SMK-103（基本表示）/ SMK-104（テナント分離）/ SMK-105（他 Approver 処理分非表示）の全 3 項目が PASS。

### 関連 PR / コミット
- PR #111: BE ハンドラ + FE Page 実装（commits `722568e` / `4d82545` / `d1e8e37` / `b4993f0`）
- commit `790e558`: 設計書改訂（openapi.yaml / screens.md / smoke_check.md §4.12 追加）
