...
PR #88（commit e7b8a59）にて対応。`ItemSlidePanel.tsx` の保存成功経路（`handleAddModeSubmit` / `handleSubmit`）で `setIsFormDirty(false)` / `setPendingFiles([])` / `formResetRef.current?.()` を `afterSubmit()` 呼び出し前に実行するよう修正。保存成功後の F5 リロードでブラウザ確認ダイアログが表示されないことを確認。
