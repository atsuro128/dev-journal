...
## 追加対応ログ（2026-04-18: ダウンロード挙動 UX 修正）

### 背景・修正方針

FE PR #65（`d45e387`）で実装した `AttachmentItemRow.handleDownload` は、プレビューと同じ「クリック同期 `window.open('about:blank')` → `location.href` 差し替え」パターンを採用している。しかし Content-Disposition: attachment の URL を新タブで開くと、空白タブが残る・すぐ閉じるなど UX が悪く、ダウンロードに新タブを開く必然性がない。

ユーザー合意済み方針:

- ダウンロード側のみ master 旧実装の C パターン（動的 `<a download>` 要素 + click）に戻す
- プレビュー側は現状維持（`window.open('about:blank')` → `location.href` 差し替え）
- hook（`useAttachmentDownloadUrl`）の構造は維持（`enabled: false` + 明示的 `refetch()`）
- 決定 2 の「↓ アイコン = `window.open(download_url)`」記述は本対応で訂正

### 1. 実装修正

**対象ファイル（worktree 内パス前提）**:

| ファイル | 変更内容 |
|---------|---------|
| `frontend/src/pages/reports/AttachmentList.tsx` | `AttachmentItemRow.handleDownload`（現状 L115-135）を動的 `<a download>` パターンに差し替え。`window.open` 関連処理・空タブ close 処理を削除。クラスコメント（L4, L54-55）のダウンロード挙動に関する記述を更新 |
  ...
  refetchDownload()
    .then((res) => {
      if (res.data?.data.url) {
        // <a download> 要素を動的生成してクリック（タブを開かずダウンロード起動）。
...
- `frontend/src/pages/reports/AttachmentArea.tsx` — 現状 AttachmentArea は `AttachmentList` に `onError` を渡すのみ。変更不要
- `frontend/src/hooks/useAttachmentDownloadUrl.ts` — hook は `refetch()` で `{ url, file_name, mime_type, file_size, expires_at }` を返すのみ。変更不要
- `frontend/src/hooks/useAttachmentPreviewUrl.ts` — プレビュー側は現状維持のため変更不要
- `frontend/src/api/types.ts` — `AttachmentAccess` 型には `file_name` フィールドが既にある（L120）。変更不要
- BE 実装（`attachment.go` / `attachment_service.go` など） — BE 側は `/download` エンドポイント + `Content-Disposition: attachment` 設定で既に正しい。変更不要
...
| ファイル | 変更内容 |
|---------|---------|
| `frontend/src/pages/reports/__tests__/AttachmentList.test.tsx` | ATT-FE-010b（L206-227）を `<a download>` クリック検証に差し替え。`vi.spyOn(document, 'createElement')` または `data-testid` ベースで動的生成 `<a>` 要素のクリックを検証。冒頭の mock セットアップ（L17-28）は維持、`mockRefetchDownload` の返り値に `file_name` を追加 |
| `frontend/src/pages/reports/__tests__/AttachmentArea.integration.test.tsx` | ATT-FE-049（L294-355）を `<a download>` 方式検証に差し替え。`window.open` スパイ + `newWindow.location.href` 検証 → `<a>` 要素の `download` 属性と `href` に URL が設定されることを検証。ATT-FE-049d（L454-499、API エラー時）も `newWindow.close()` 検証から「エラートーストのみ表示」検証に変更 |

**プレビュー側テストの確認**:

- ATT-FE-010（L182-203, ファイル名クリック・`window.open` パターン） — 影響なし（プレビューは現状維持）
- ATT-FE-049b（L357-419, ファイル名クリック・`/preview` API） — 影響なし
- ATT-FE-049c（L421-451, ポップアップブロック） — 影響なし（プレビューのみ該当）

**Hook テストの確認**:

- `frontend/src/hooks/__tests__/useAttachmentDownloadUrl.test.tsx` — hook 自体は変更しないため影響なし。既存テストはパス

### 3. 設計書修正

| # | ファイル | 修正行・差分方針 |
|---|---------|---------------|
| D1 | `dev-journal/deliverables/docs/50_detail_design/screens/report-detail.md` | §7「添付ファイル一覧」L326: 「↓ アイコン...クリックでダウンロード（新タブ）」の「（新タブ）」を削除。§7「ダウンロード」 L371: 「クリック同期で空タブを開き、非同期取得した署名付き URL（`Content-Disposition: attachment`）で location を差し替える」を「非同期で署名付き URL（`Content-Disposition: attachment`）を取得し、動的生成した `<a download>` 要素のクリックでブラウザのダウンロードを開始する」に書き換え |
| D2 | `dev-journal/deliverables/docs/50_detail_design/files.md` | §4.5「フロント実装制約: 新タブ方式のポップアップブロック回避」L349-363 をプレビュー側のみの制約に限定。冒頭 L351 を「プレビューは `window.open` で新タブを開く。ダウンロードは動的 `<a download>` 要素方式を採用する（新タブを開かない）」と書き換え。L354-358 のパターン図は「プレビュー」セクションとして維持しつつ、ダウンロード用の新セクション「[ダウンロード] (1) refetch → (2) 動的 `<a>` 要素生成 → (3) click → (4) remove」を追加。L361-362 の「AttachmentItemRow の Hook 配置」記述はそのまま有効 |
| D3 | `dev-journal/deliverables/docs/55_ui_component/screens/report-detail.md` | §AttachmentList L516: 「クリック同期 `window.open` → `refetch` → `newWindow.location.href` 差し替えパターン」の記述を「プレビューはクリック同期 `window.open` パターン、ダウンロードは動的 `<a download>` 要素パターン」に分離。§AttachmentItemRow L552: 「クリック同期で `window.open('about:blank')` → ...」記述もプレビュー/ダウンロードを分けた表現に書き換え。データフロー L692-697「[添付ダウンロード]」ブロックを `<a download>` 方式に更新 |
| D4 | `dev-journal/deliverables/docs/60_test/test_cases/attachments.md` | ATT-FE-010b（L443）: 「`window.open('about:blank', '_blank')` がクリック同期で呼ばれ…`newWindow.location.href` が差し替えられる」を「`refetch()` 成功後に `<a>` 要素（`download` 属性 + 署名付き URL `href`）が生成・クリック・除去される」に書き換え。ATT-FE-049（L515）: ダウンロード側の検証を同様に更新、プレビュー側はそのまま維持。L578 のサマリー文言もあわせて調整 |
| D5 | `dev-journal/deliverables/docs/60_test/manual_checklists/smoke_check.md` | SMK-037（L121）: 期待結果の「2. ↓ アイコン押下: クリック同期で新しいタブが開き、ブラウザのダウンロードが開始される」を「2. ↓ アイコン押下: 新しいタブは開かず、ブラウザのダウンロードが開始される（`Content-Disposition: attachment`）。ファイル名・拡張子が正しい」に書き換え |

**修正対象外（影響なしを確認）**:

- `openapi.yaml` — API 契約は変わらない
- `authz.md` — 認可ロジックは変わらない
- `security.md` — CSP は `<a download>` の S3 ドメインへの遷移は影響なし（`img-src`/`frame-src` はプレビューのみ対象）
- `state-management.md` — hook の責務は変わらない

### 4. issue 102 本文の扱い

- 決定 2（L33-L34）は「合意事項」なので**改変せず維持**。代わりに本「追加対応ログ」で訂正する旨を明示
- 実装ログ（L348-）はマージ済み PR #64/#65 の履歴として残す

### 5. ブランチ戦略・PR 分割

**ブランチ**: `fix/102-download-anchor-pattern`（`integration/102-attachment-preview` から分岐）

**PR 分割案（単一 PR 推奨）**:

規模が小さく（FE 実装 1 ファイル + FE テスト 2 ファイル + 設計書 5 ファイル）、全て論理的に 1 つの UX 修正に紐づくため、1 つの PR にまとめる。

- PR タイトル: `102: ダウンロード挙動を <a download> 方式に変更（UX 修正）`
- ベース: `integration/102-attachment-preview`
- マージ順序: 本 PR のみ → integration ブランチで結合動作確認 → 最終 PR（integration → master squash）

**判断理由**: 前回（PR #64/#65）は BE/FE の DTO スキーマ変更を含む breaking change で stacked PR が必要だったが、本修正は FE 内に閉じており、設計書は実装と同期する必要がある（実装なしで設計書だけマージすると設計-実装の乖離を生む）。

### 6. 検証手順

**FE ローカル動作確認**:

- `npm run lint`, `npx tsc --noEmit`, `npm test`, `npm run build` 通過
- VS Code タスクで FE テスト実行（project-rules の通りローカルフルスイート禁止）

**手動 smoke 確認（integration ブランチ上、`docker compose up` 後）**:

| # | 観点 | 手順 | 期待結果 |
|---|------|-----|---------|
| 1 | SMK-037 ↓ アイコン | 添付レポートで ↓ アイコンクリック | 新タブは開かず、ブラウザのダウンロードが即起動。ファイル名・拡張子が元ファイル名と一致 |
| 2 | SMK-037 プレビュー（回帰） | 添付レポートでファイル名クリック | 新タブが開き、画像/PDF が `Content-Disposition: inline` で表示される（変更なし） |
| 3 | ポップアップブロック回帰 | ブラウザでポップアップ全ブロック設定 → ファイル名クリック | エラートースト「ポップアップがブロックされました。ブラウザ設定を確認してください」表示（プレビューのみ該当）。↓ アイコンはポップアップブロック影響なし |
| 4 | 認可 4 ロール × 2 操作（回帰） | Admin/Approver/Accounting/Member で各自プレビュー・ダウンロード | 既存認可ロジック維持のため全て成功（所有者・Approver submitted 範囲・Accounting approved 範囲 etc.） |
| 5 | API エラー時 | mock で `/download` API を 500 に設定 → ↓ アイコンクリック | エラートースト「ダウンロードの取得に失敗しました」表示。タブは開かない |
...
- 「最終判断は実装者に委譲」と書かない
- React hooks rules などの破れない技術制約を事前検証してから計画に落とす

**今回の技術制約（事前検証済み）**:

| 制約 | 検証結果 |
|------|---------|
| `<a download>` 属性のクロスオリジン挙動 | Chromium/Firefox は同一オリジン限定だが、S3 が `Content-Disposition: attachment` を返すためブラウザはダウンロード扱い。`download` 属性のファイル名指定は S3 側 `Content-Disposition` のほうが優先される可能性あり（`response-content-disposition` で上書き済み）。実ブラウザ確認で両方のファイル名が一致することを確認する |
| `refetch()` の Promise 解決タイミング | TanStack Query `refetch()` は Promise を返す。`.then(res => ...)` で解決後に DOM 操作する流れを維持（プレビュー側と同じ） |
| `appendChild` → `click()` → `removeChild` のタイミング | 同期的に連続実行する（ブラウザはイベントループ一周の間に download を起動するため問題なし）。master 旧実装と同じパターン |
| hook の挙動変更なし | `useAttachmentDownloadUrl` の `queryKey` / `enabled: false` / `staleTime: 0` はそのまま維持。テスト ATT-FE-033〜035 は変更なし |
