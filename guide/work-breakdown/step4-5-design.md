...
| ID | タスク | 種別 | 依存 | 状態 |
|----|--------|------|------|------|
| 4+5-A | DB スキーマ設計（全体） | 基盤 | Step 3 完了 | 未着手 |
| 4+5-B | セキュリティ設計（横断） | 基盤 | Step 3 完了 | 未着手 |
| 4+5-H | 監視・ログ詳細設計（横断） | 基盤 | 4+5-B | 未着手 |
| 4+5-I | 画面一覧・遷移の全体設計 | 基盤 | Step 3 完了 | 未着手 |
| 4+5-C-1 | 認証・ユーザー管理（画面詳細化） | 機能 | 4+5-I | 未着手 |
| 4+5-C-2 | 認証・ユーザー管理（API 定義） | 機能 | 4+5-A, 4+5-C-1 | 未着手 |
| 4+5-D-1 | 経費レポート CRUD + ダッシュボード（画面詳細化） | 機能 | 4+5-I | 未着手 |
| 4+5-D-2 | 経費レポート CRUD + ダッシュボード（API 定義） | 機能 | 4+5-A, 4+5-D-1, 4+5-C-2 | 未着手 |
| 4+5-E-1 | 添付ファイル（画面詳細化） | 機能 | 4+5-D-1 | 未着手 |
| 4+5-E-2 | 添付ファイル（API + S3 設計） | 機能 | 4+5-A, 4+5-E-1 | 未着手 |
| 4+5-F-1 | 承認フロー（画面詳細化） | 機能 | 4+5-D-1 | 未着手 |
| 4+5-F-2 | 承認フロー（API 定義） | 機能 | 4+5-A, 4+5-F-1 | 未着手 |
| 4+5-G | 認可設計 + 最終統合 | 統合 | 4+5-C-2, 4+5-D-2, 4+5-E-2, 4+5-F-2, 4+5-I | 未着手 |

### 依存グラフ

```
Step 3 完了
  ├→ 4+5-A (DB スキーマ) ─────────────────┬→ 4+5-C-2, D-2, E-2, F-2
  ├→ 4+5-B (セキュリティ) → 4+5-H (監視)  │
  └→ 4+5-I (画面一覧) ──┬→ 4+5-C-1 (認証画面) → 4+5-C-2 (認証API) ──┐
                         ├→ 4+5-D-1 (経費画面) → 4+5-D-2 (経費API) ──┤
                         │    ├→ 4+5-E-1 (添付画面) → 4+5-E-2 (添付API) ─┤
                         │    └→ 4+5-F-1 (承認画面) → 4+5-F-2 (承認API) ─┤
                         └→ 4+5-G (認可 + 最終統合) ←─────────────────────┘
```

**D-2 は C-2 に依存**: openapi.yaml の初回作成（共通定義）が C-2 で行われるため。
