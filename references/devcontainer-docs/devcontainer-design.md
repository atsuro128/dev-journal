...
- CLI:
  - Claude Code: Anthropic 公式 native installer（`https://claude.ai/install.sh` + バージョン位置引数）で `/home/node/.local/share/claude/versions/` 配下に導入
  - Codex: `@openai/codex` を npm グローバルで導入
...
- Claude Code を npm 経由で導入すると初回 `claude update` 実行時に暗黙で native installer へ差し替わるため、Dockerfile 時点で native installer に寄せてランタイム状態と一致させる
...
起動確認では、Squid の PID ファイルとプロセス生存確認だけでなく、`127.0.0.1:3127` が TCP 接続を受け付ける状態になったことも確認する。
これは、Squid プロセス生成直後に proxy port がまだ listen していない状態で egress 検証が先行し、起動失敗として扱われることを防ぐためである。
...
- `127.0.0.1:3127` の proxy が listen していること
...
`https://example.com` の拒否確認では、proxy 未起動や proxy 接続失敗による `Connection refused` を blocked 成功とは扱わない。
proxy 自体が利用不能な場合は、allowlist による拒否が成立しているか判定できないため、検証失敗として fail-closed に扱う。
...
1. proxy が `127.0.0.1:3127` で listen している
2. `https://example.com` が proxy 経由で拒否される
3. `https://chatgpt.com` が proxy 経由で到達できる
4. `https://auth.openai.com` が proxy 経由で到達できる
5. `https://api.openai.com` が proxy 経由で到達できる
6. `https://api.openai.com` への direct outbound が失敗する
7. allowlist にある全ホストが proxy 経由で到達できる
