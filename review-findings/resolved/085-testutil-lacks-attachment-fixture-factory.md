...
## 指摘概要
Step 8-8 の責務には添付ファイルを含むフィクスチャファクトリが明記されているが、`internal/testutil` にあるのは tenant / user / membership / report / item までで、attachment を生成するファクトリが存在しない。Step 9 の添付 API テストは共通基盤だけで着手できず、チケット責務と受け渡し契約を満たしていない。

## 根拠
- `dev-journal/progress-management/tickets/step8/8-8-test-infra.md:18-20`
