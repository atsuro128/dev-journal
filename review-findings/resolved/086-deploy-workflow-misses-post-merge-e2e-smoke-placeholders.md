...
## 指摘概要
Step 8-8 は「デプロイ後のワークフロー定義（E2E/スモークは枠のみ）」を成果物に含める契約だが、`deploy.yml` には lint / test / build と Docker/ECS のコメントアウト枠しかなく、main マージ後に走る E2E / レスポンスタイムスモークのジョブ定義が存在しない。テスト戦略とチケット責務が CI 実装に反映されていない。

## 根拠
- `dev-journal/progress-management/tickets/step8/8-8-cicd.md:17-19`
