...
### 確認内容
- `ai-dev-framework/templates/docs-v2/55_ui_component/screens/component-design-template.md` に `## 7. 表示条件・認可` セクションが追加されている
- 同セクションに `コンポーネント / 表示条件 / 操作可否条件 / 根拠` の4列が定義され、元指摘で求めた記録項目を満たしている
- 根拠列の記法例として `authz.md` と `screens/{name}.md §8` が示されており、上流の認可設計と画面詳細仕様へ追跡できる

### 判定理由
- 元指摘の論点だった「表示条件や認可根拠を構造化して残せない」状態は解消された
- Step 5.5 のレビュー観点である `authz.md` との整合確認を、画面別コンポーネント設計テンプレート上で実施できる状態に戻っている
